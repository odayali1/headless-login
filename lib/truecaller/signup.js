...
function proxyHint(proxyUrl) {
  const preset = getProxyPresetId() || '';
  try {
    const raw = String(proxyUrl || '');
    const u = new URL(raw.includes('://') ? raw : `http://${raw}`);
    if (u.hostname === '172.120.42.227' || /owl/i.test(preset) || /datacenter/i.test(preset)) {
      return ' Truecaller proxy is datacenter (owl). Microsoft often ignores Outlook cookies from a different IP — switch this tab to Residential IPv4.';
    }
  } catch {
    // ignore
  }
...
function passwordAbortMessage(email, summary, proxyUrl) {
  const bits = [
    `Microsoft recognized ${email} from copied Outlook cookies but still asked for a password.`,
    'The Outlook session is being used (read-only); it was not accepted for silent SSO.',
    summary?.estsauth || summary?.estsauthPersistent
      ? 'ESTSAUTH was present.'
      : 'No ESTSAUTH in the Outlook profile (MSPAuth-only).',
    summary?.mspAuthDisabled ? 'MSPAuth value is Disabled.' : '',
    summary?.hostMsa ? '' : 'No __Host-MSAAUTH cookie.',
    proxyHint(proxyUrl),
    'Truecaller will not type the password. Outlook profile was not written.',
  ];
  return bits.filter(Boolean).join(' ');
}

function abortReasonFromSnap(snap, { allowPassword = false, email = '', summary = null, proxyUrl = '' } = {}) {
  if (snap.password && !allowPassword) {
    return passwordAbortMessage(email, summary, proxyUrl);
  }
...
export async function signupMicrosoftAccount(email, { log = () => {}, jobId = 'manual', password = '' } = {}) {
  ...
  log('sso', 'Looking up Truecaller sqlite, then Outlook profile cookies (not origins)…');
  ...
  if (existingTc?.ms_cookies_json) {
    try {
      ...
      if (extracted.cookies.length >= 2) {
        ...
        summary = ssoCookieSummary(extracted.names, extracted.cookies);
  ...
  log('sso', 'Reading Outlook profile cookies (skipping MSAL origins)…');
  const sso = await readOutlookSso(email);
  const fileName = sso.file ? sso.file.split(/[/\\]/).pop() : 'not-found';
  log(
    'sso',
    `Outlook profile ${sso.bytes ? `${Math.round(sso.bytes / 1024)}kb` : 'missing'} file=${fileName} sessionOk=${!!sso.sessionOk} ssoCookies=${sso.cookies?.length || 0}`
  );
  if (!cookies.length && sso.ok) {
    cookies = sso.cookies;
    fingerprint = sso.fingerprint;
    summary = sso.summary || ssoCookieSummary(sso.names, sso.cookies);
    log(
      'sso',
      `Copied ${summary.count} Outlook SSO cookies read-only (ESTSAUTH=${summary.estsauth} persistent=${summary.estsauthPersistent} MSPAuth=${summary.mspAuth} disabled=${!!summary.mspAuthDisabled} hostMsa=${summary.hostMsa} strong=${(summary.strong || []).join(',') || 'none'}). Outlook profile not opened.`
    );
  ...
  if (summary?.mspAuthDisabled && !summary.estsauth && !summary.estsauthPersistent && !summary.hostMsa && !allowPassword) {
    throw new Error(
      'Outlook MSPAuth is Disabled and there is no ESTSAUTH / __Host-MSAAUTH. Re-login on the Outlook dashboard first. Truecaller will not type the password.'
    );
  }

  const dcHint = proxyHint(proxyUrl);
  if (dcHint) log('sso', dcHint.trim());

  upsertAccount(email, { status: 'signing_up', last_error: null });

  log('browser', 'Launching isolated Camoufox (Firefox start can take up to a minute)…');
  ...
  if (session.cookiesDropped?.some((d) => /__Host-MSAAUTH|ESTSAUTH/i.test(d))) {
    log('sso', 'Firefox dropped a Microsoft ticket cookie — silent SSO will likely fail.');
  }
  ...
  try {
    log('sso', 'Warming login.live.com so copied Microsoft cookies attach (isolated browser only)…');
    await page.goto('https://login.live.com/', { waitUntil: 'domcontentloaded', timeout: 45_000 }).catch((err) => {
      log('sso', `login.live.com warm skipped: ${err.message}`);
    });
    await sleep(1500);
    let snap = await snapshot(page);
    log('sso', `warm ${snap.url.slice(0, 140)} — ${snap.password ? 'password prompt' : snap.body.slice(0, 100)}`);
    if (snap.password && !allowPassword) {
      await screenshot(page, jobId, 'warm-password');
      throw new Error(passwordAbortMessage(email, summary, proxyUrl));
    }
    ...
    snap = await snapshot(activePage);
    ...
    while (Date.now() < deadline) {
      ...
      const danger = abortReasonFromSnap(snap, { allowPassword, email, summary, proxyUrl });
      if (danger) {
      ...
      if (isMicrosoftHost(snap.url) || PICK_ACCOUNT_RE.test(snap.body) || KMSI_RE.test(snap.body) || CONSENT_RE.test(snap.body)) {
        if (PICK_ACCOUNT_RE.test(snap.body) || /accounts\.login|login\.live\.com\/oauth20/i.test(snap.url)) {
          ...
          if (picked) {
            ...
            await sleep(2000);
            snap = await snapshot(view);
            log('page', `after pick ${snap.url.slice(0, 140)} — ${snap.password ? 'password field' : snap.body.slice(0, 100)}`);
            continue;
          }
        }
