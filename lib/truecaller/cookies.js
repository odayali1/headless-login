...
import { resolveOutlookProfilePath } from '../profile.js';
...
export function extractSsoCookies(cookies = []) {
  ...
  for (const c of cookies) {
    ...
    if (STRONG_NAMES.has(c.name) && isDisabledAuthValue(c.value)) continue;
    ...
    if (copy.expires === 0) copy.expires = -1;
    if (c.domain) copy.domain = c.domain;
    else if (c.url) copy.url = c.url;
    else copy.url = 'https://login.live.com/';
    out.push(copy);
    names.add(c.name);
  }
...
export function isLiveTicketCookie(c) {
  if (!c?.name || c.value == null || c.value === '') return false;
  if (!STRONG_NAMES.has(c.name)) return false;
  return !isDisabledAuthValue(c.value);
}

function cookieHost(c) {
  return String(c.domain || '').replace(/^\./, '') || hostFromUrl(c.url) || 'login.live.com';
}

function sessionExpires(expires) {
  if (typeof expires !== 'number' || expires === 0) return -1;
  return expires;
}

/**
 * Playwright cookie objects for Firefox.
 * hostPrefix=keep: Outlook storageState shape (domain as exported).
 * hostPrefix=url: RFC __Host- form (url, no domain) — Firefox fallback if domain is rejected.
 * Never duplicate MSPAuth onto login.live.com; that made Firefox reject the whole batch.
 */
export function toPlaywrightCookies(cookies = [], { hostPrefix = 'keep' } = {}) {
  ...
  for (const c of cookies) {
    ...
    if (STRONG_NAMES.has(c.name) && isDisabledAuthValue(c.value)) {
      dropped.push(`${c.name}:disabled`);
      continue;
    }
    ...
    const cookie = {
      ...
      value: String(c.value),
      path: c.path || '/',
      expires: sessionExpires(c.expires),
      httpOnly: !!c.httpOnly,
      secure: c.secure !== false,
      sameSite: normalizeSameSite(c.sameSite),
    };
    ...
    if (c.name.startsWith('__Host-') && hostPrefix === 'url') {
      cookie.secure = true;
      cookie.path = '/';
...
export async function readOutlookSso(email) {
  const file = await resolveOutlookProfilePath(email);
  if (!file) {
    return { ok: false, reason: 'no_outlook_profile', cookies: [], names: new Set(), bytes: 0, file: null };
  }
  let raw = '';
  let bytes = 0;
  try {
    ...
    return { ok: false, reason: 'no_outlook_profile', cookies: [], names: new Set(), bytes: 0, file };
  }
  ...
  try {
    ...
    try {
      ...
      return { ok: false, reason: 'no_outlook_profile', cookies: [], names: new Set(), bytes, file };
    }
  }
  raw = '';
  ...
  const summary = ssoCookieSummary(names, data.cookies);
