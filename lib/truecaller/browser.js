...
import { toPlaywrightCookies } from './cookies.js';
...
export async function launchTruecallerBrowser({ email, cookies = [], fingerprint, proxyUrl, log = () => {} }) {
  ...
  log('browser', 'Starting isolated proxy relay…');
  ...
  const { cookies: usable, dropped } = toPlaywrightCookies(cookies);
  const contextOpts = {};
  if (usable.length) {
    contextOpts.storageState = { cookies: usable, origins: [] };
  }
  let context;
  try {
