...
export const BUILT_IN_PROXY_PRESETS = Object.freeze([
  ...
  Object.freeze({
    ...
    name: 'Residential IPv4 rotating',
    kind: 'residential',
...
export function initSettings(db) {
  ...
  // Refresh to US-targeted URL when missing or still on the old untargeted login.
  {
    const enc = settings.get('proxy_login_residential_url_enc');
    const cur = enc ? decryptPassword(enc) || '' : '';
    const needsUs =
      !cur ||
      (/dataimpulse\.com/i.test(cur) && !/__cr\.us\b/i.test(cur));
    if (needsUs) {
      set.run('proxy_login_residential_url_enc', encryptPassword(BUILT_IN_RESIDENTIAL_IPV4_PROXY_URL));
      if (cur && /dataimpulse\.com/i.test(cur)) {
        console.warn('[settings] Login residential IPv4 → DataImpulse __cr.us (US exits)');
      }
    }
  }
...
export function matchProxyPresetId({
  ...
  if (loginResKey) {
    const byLoginUrl = BUILT_IN_PROXY_PRESETS.find(
      (p) => p.kind === 'residential' && proxyEndpointKey(p.url) === loginResKey
    );
    if (byLoginUrl) return byLoginUrl.id;
  }

  // Legacy: profile=residential used to mean full residential (now login IPv4 sticky).
