...
/** Skip Playwright HTTP token refresh — too slow on mobile relay / Wi‑Fi Split uplink. */
export function useSessionOnlyTokenPath() {
  return isMobileRelayProxy() || isIproxyWifiSplitMode();
}

/** socks | http | auto — auto tries SOCKS relay first, then HTTP fallback. */
export function getProxyPreferMode() {
  ...
  if (isIproxyWifiSplitMode()) return 'http';
  return 'auto';
}
...
export async function applyProxyToLaunchOptions(fromOptions, { forceNewRelay = false } = {}) {
  ...
  const { getPlaywrightProxyConfig, isBrowserPathSplitBlocked } = await import('./proxy-local.js');
  ...
  // HTTP/3/QUIC over UDP is dropped by many mobile proxies. Keep HTTP/2 — Outlook MSAL breaks without it.
  const prefs = {
  ...
  // Mobile relay: cap parallel tunnels — heavy SPA + split uplink stalls with dozens of CONNECTs.
  if (cfg.mode === 'socks-relay' || (cfg.mode === 'http-direct' && isMobileRelayProxy())) {
    const splitSerial = isBrowserPathSplitBlocked();
    Object.assign(prefs, splitSerial
      ? {
          ...
          'network.http.max-urgent-start-excessive-connections-per-host': 1,
          'network.http.speculative-parallel-limit': 0,
          'network.http.http2.enabled': false,
          'network.http.pipe': false,
        }
      : {
          'network.http.max-connections': 24,
          'network.http.max-persistent-connections-per-proxy': 6,
          'network.http.max-persistent-connections-per-server': 6,
          'network.http.max-urgent-start-excessive-connections-per-host': 2,
          'network.http.speculative-parallel-limit': 0,
        });
  }

  // Optional: disable post-quantum TLS key shares (may help iProxy Wi-Fi Split) — off by default.
  if (/^(1|true|yes|on)$/i.test(String(process.env.IPROXY_TLS_LEGACY || '').trim())) {
    Object.assign(prefs, {
