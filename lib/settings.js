...
/**
 * DataImpulse residential IPv4 — Camoufox login only (not hybrid Loki IPv6).
 * Port 10000 = sticky (~30 min same IP). `__cr.us` = United States only.
 * Docs: https://docs.dataimpulse.com/proxies/types-of-connections
 */
export const BUILT_IN_RESIDENTIAL_IPV4_PROXY_URL =
  'socks5://7c0f7e7e9347e4d38b37__cr.us:aeb91a2e66b62ff6@gw.dataimpulse.com:10000';
/** DataImpulse SOCKS rotating — new US exit per connection (port 824). */
export const BUILT_IN_RESIDENTIAL_IPV4_ROTATING_PROXY_URL =
  'socks5://7c0f7e7e9347e4d38b37__cr.us:aeb91a2e66b62ff6@gw.dataimpulse.com:824';
/** DataImpulse rotating — Middle East countries. */
export const BUILT_IN_RESIDENTIAL_IPV4_ME_PROXY_URL =
  'socks5://7c0f7e7e9347e4d38b37__cr.bh,eg,iq,il,jo,kw,lb,qa,sa,ae:aeb91a2e66b62ff6@gw.dataimpulse.com:824';
/** DataImpulse rotating — worldwide (no country filter). */
export const BUILT_IN_RESIDENTIAL_IPV4_GLOBAL_PROXY_URL =
  'socks5://7c0f7e7e9347e4d38b37:aeb91a2e66b62ff6@gw.dataimpulse.com:824';
/**
 * DataImpulse datacenter IPv4 rotating — global (port 823). Login Camoufox test only.
 * Docs: https://docs.dataimpulse.com/proxies/types-of-connections
 */
export const BUILT_IN_DATACENTER_IPV4_GLOBAL_PROXY_URL =
  'socks5://e0ae3b65061f2909a919:897acf5ad227ddd0@gw.dataimpulse.com:823';
/** Owl datacenter IPv4 US — Camoufox login test (host:port:user:pass). */
export const BUILT_IN_DATACENTER_IPV4_OWL_US_PROXY_URL =
  'socks5://172.120.42.227:64299:YNtVtMrf:NedyYDqp';
/**
 * ProxyScrape residential gateway (HTTP CONNECT).
 * Optional country: append -country-us (etc.) to the username.
 * Docs: https://docs.proxyscrape.com/
 */
export const BUILT_IN_PROXYSCRAPE_RESIDENTIAL_URL =
  'http://vz2pb8g24lahomc:wk3og91mn6eixgv@rp.scrapegw.com:6060';
...
export const BUILT_IN_PROXY_PRESETS = Object.freeze([
  ...
  Object.freeze({
    id: 'residential-ipv4',
    name: 'Residential IPv4 sticky US',
    kind: 'residential',
    url: BUILT_IN_RESIDENTIAL_IPV4_PROXY_URL,
    rotateUrl: null,
  }),
  Object.freeze({
    id: 'residential-ipv4-rotating',
    name: 'Residential IPv4 rotating US',
    kind: 'residential',
    url: BUILT_IN_RESIDENTIAL_IPV4_ROTATING_PROXY_URL,
    rotateUrl: null,
  }),
  Object.freeze({
    id: 'residential-ipv4-me',
    name: 'Residential IPv4 rotating ME',
    kind: 'residential',
    url: BUILT_IN_RESIDENTIAL_IPV4_ME_PROXY_URL,
    rotateUrl: null,
  }),
  Object.freeze({
    id: 'residential-ipv4-global',
    name: 'Residential IPv4 rotating global',
    kind: 'residential',
    url: BUILT_IN_RESIDENTIAL_IPV4_GLOBAL_PROXY_URL,
    rotateUrl: null,
  }),
  Object.freeze({
    id: 'proxyscrape-residential',
    name: 'New Residential (ProxyScrape)',
    kind: 'residential',
    url: BUILT_IN_PROXYSCRAPE_RESIDENTIAL_URL,
    rotateUrl: null,
  }),
  Object.freeze({
    id: 'datacenter-ipv4-global',
    name: 'Datacenter IPv4 rotating global',
    kind: 'residential',
    url: BUILT_IN_DATACENTER_IPV4_GLOBAL_PROXY_URL,
    rotateUrl: null,
  }),
  Object.freeze({
    id: 'datacenter-ipv4-owl-us',
    name: 'Datacenter IPv4 owl proxy US',
    kind: 'residential',
    url: BUILT_IN_DATACENTER_IPV4_OWL_US_PROXY_URL,
    rotateUrl: null,
  }),
  Object.freeze({
    id: 'aws-api',
    name: 'AWS API',
    kind: 'aws-api',
    // Identity only — Camoufox talks to a local FireProx CONNECT relay (lib/fireprox-local-proxy.js).
    url: 'http://m6m8tzyruf.execute-api.us-east-1.amazonaws.com:443:fireprox:awsapi',
    rotateUrl: null,
  }),
]);
...
export function initSettings(db) {
  ...
  // Only seed if missing, or migrate the ORIGINAL untargeted sticky :10000 (no __ params).
  // Never overwrite ME / global / rotating presets the user selected.
  {
    const enc = settings.get('proxy_login_residential_url_enc');
    const cur = enc ? decryptPassword(enc) || '' : '';
    const isLegacyUntargetedSticky =
      /gw\.dataimpulse\.com:10000/i.test(cur) && !/__/i.test(cur);
    if (!cur || isLegacyUntargetedSticky) {
      set.run('proxy_login_residential_url_enc', encryptPassword(BUILT_IN_RESIDENTIAL_IPV4_PROXY_URL));
      if (isLegacyUntargetedSticky) {
        console.warn('[settings] Migrated legacy DataImpulse sticky login → __cr.us');
      }
    }
  }
  ...
  // Login Camoufox parallel — seed once from env (needs FORCE for >1). Dashboard can change later.
  if (!settings.has('login_parallel')) {
    const raw = process.env.LOGIN_PARALLEL;
    const n = raw == null || raw === '' ? 1 : Number(raw);
    let seeded = 1;
    if (Number.isFinite(n)) {
      const requested = Math.min(LOGIN_PARALLEL_MAX, Math.max(1, Math.trunc(n)));
      if (requested <= 1 || /^(1|true|yes|on)$/i.test(String(process.env.LOGIN_PARALLEL_FORCE || '').trim())) {
        seeded = requested;
      }
    }
    set.run('login_parallel', String(seeded));
  }
...
export const LOGIN_PARALLEL_MAX = 1000;

/**
 * Camoufox login/re-login concurrency — dashboard DB only.
 * Coolify LOGIN_PARALLEL / LOGIN_PARALLEL_LOCK no longer cap runtime (user controls Parallel in UI).
 * Env LOGIN_PARALLEL still seeds DB once on first boot only.
 */
export function getLoginParallel() {
  const fromDb = Number(store?.get('login_parallel'));
  if (Number.isFinite(fromDb) && fromDb >= 1) {
    return Math.min(LOGIN_PARALLEL_MAX, Math.max(1, Math.trunc(fromDb)));
  }
  return 1;
}

export function setLoginParallel(n) {
  const next = Math.min(LOGIN_PARALLEL_MAX, Math.max(1, Math.trunc(Number(n) || 1)));
  store?.set('login_parallel', String(next));
  return next;
}

/** Always false — Parallel is dashboard-owned (LOGIN_PARALLEL_LOCK ignored). */
export function isLoginParallelLocked() {
  return false;
}
...
/** True when dashboard login preset is AWS API Gateway (FireProx). Isolated from mobile/residential. */
export function isLoginAwsApi() {
  return getProxyPresetById(getActiveProxyPresetId())?.kind === 'aws-api';
}

/** True when dashboard login preset is DataImpulse residential IPv4 (sticky or rotating). */
export function isLoginResidentialIpv4() {
  const preset = getProxyPresetById(getActiveProxyPresetId());
  return preset?.kind === 'residential';
}
...
export function getLoginProxyUrl() {
  if (isLoginAwsApi()) {
    const preset = getProxyPresetById('aws-api');
...
export function getProxyHttpUrl() {
  // AWS API uses a local FireProx relay — never iProxy HTTP CONNECT.
  if (isLoginAwsApi()) return null;
  // Never fall back to mobile HTTP CONNECT when Camoufox login is residential SOCKS.
  // HTTP residential (ProxyScrape) uses getLoginProxyUrl() via http-direct — not this helper.
  if (isLoginResidentialIpv4() || isResidentialProxy()) {
    try {
      const p = parseProxyUrl(getLoginProxyUrl());
      if (p.protocol === 'http' || p.protocol === 'https') return getLoginProxyUrl();
    } catch {
      // ignore
    }
    return null;
...
export function isMobileRelayProxy() {
  if (!isProxyEnabled() || isLoginAwsApi() || isLoginResidentialIpv4() || isResidentialProxy()) return false;
...
/** socks | http | auto — residential IPv4 login is SOCKS or HTTP depending on URL. */
export function getProxyPreferMode() {
  if (isLoginResidentialIpv4() || isResidentialProxy()) {
    try {
      const p = parseProxyUrl(getLoginProxyUrl());
      if (p.protocol === 'http' || p.protocol === 'https') return 'http';
    } catch {
      // fall through
    }
    return 'socks';
...
/** Distinguish DataImpulse presets that share host:port but differ by __cr.* username. */
function proxyIdentityKey(url) {
  if (!url) return '';
  try {
    const p = parseProxyUrl(url);
    return `${p.username}@${p.host}:${p.port}`.toLowerCase();
  } catch {
    return String(url).trim().toLowerCase();
  }
}
...
export function matchProxyPresetId({
  ...
  if (stored && getProxyPresetById(stored)?.kind === 'residential') return stored;
  if (stored && getProxyPresetById(stored)?.kind === 'aws-api') return stored;

  const loginId = proxyIdentityKey(loginResidentialUrl || '');
  if (loginId) {
    const byLoginUrl = BUILT_IN_PROXY_PRESETS.find(
      (p) => p.kind === 'residential' && proxyIdentityKey(p.url) === loginId
    );
    if (byLoginUrl) return byLoginUrl.id;
  }

  // Legacy: profile=residential used to mean full residential (now login IPv4 sticky US).
...
export function listProxyPresets() {
  return BUILT_IN_PROXY_PRESETS.map((p) => ({
    ...
    rotatesPerRequest: p.kind === 'aws-api',
    host: (() => {
      try {
...
export function selectProxyPreset(id) {
  ...
  if (preset.kind === 'aws-api') {
    // Do not touch mobile or DataImpulse slots — switch back and those proxies are unchanged.
    const cur = getLoginResidentialProxyUrl();
    if (/execute-api\.amazonaws\.com|fireprox/i.test(cur || '')) {
      setLoginResidentialProxyUrl(BUILT_IN_RESIDENTIAL_IPV4_PROXY_URL);
    }
    store?.set('proxy_profile', 'mobile');
  } else if (preset.kind === 'residential') {
  ...
  return {
    ...
    rotatesPerRequest: preset.kind === 'aws-api',
  };
}
...
export function getProxyStatus() {
  ...
  // Rotate = mobile changeip. Not used for AWS API (IP rotates per request).
  const canRotate = !!isProxyEnabled() && !awsApi && (!loginIpv4 || hybrid);
  return {
    ...
    rotatesPerRequest: loginIpv4 || activePreset?.kind === 'aws-api',
    configured: !!parsed,
