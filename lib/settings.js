...
/** Prefer HTTP CONNECT for Coolify/Firefox — more reliable than SOCKS relay for Outlook SPA. */
const DEFAULT_PROXY_URL =
  process.env.PROXY_URL ||
  'http://dlhnjqwtlv.cn.fxdx.in:16857:vibrantroot242733:QQwqwcCfg940';
...
export function initSettings(db) {
  ...
  if (process.env.PROXY_URL) {
    ...
    if (!current) {
      ...
      set.run('proxy_url_enc', encryptPassword(DEFAULT_PROXY_URL));
    } else if (/socks5h?:\/\/dlhnjqwtlv\.cn\.fxdx\.in:17539/i.test(current)) {
      // Old SOCKS endpoint hangs Camoufox→Outlook on Coolify; switch to fxdx HTTP CONNECT.
      console.warn('[settings] Migrating stored SOCKS :17539 → HTTP :16857 (override with PROXY_URL env)');
      set.run('proxy_url_enc', encryptPassword(DEFAULT_PROXY_URL));
    }
  }
...
export async function applyProxyToLaunchOptions(fromOptions, { forceNewRelay = false } = {}) {
  ...
  // Reuse relay created by beforeAccountLogin/Refresh unless forceNewRelay=true.
  const localProxy = await getLocalProxyForBrowser({ forceNew: forceNewRelay });
  ...
  fromOptions.firefoxUserPrefs = {
    ...
    // Avoid HTTP/3/QUIC over UDP which many residential proxies drop → hung navigations.
