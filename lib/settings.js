...
export function initSettings(db) {
  ...
  if (!settings.has('proxy_url_enc')) {
    set.run('proxy_url_enc', encryptPassword(DEFAULT_PROXY_URL));
  } else if (!decryptPassword(settings.get('proxy_url_enc'))) {
    console.warn(
      '[settings] proxy_url_enc could not be decrypted — re-saving from PROXY_URL env. ' +
...
export function parseProxyUrl(url) {
  ...
  throw new Error(
    'Proxy must be socks5://host:port:user:pass or http://user:pass@host:port (socks5h curl form also OK)'
  );
}
...
export async function applyProxyToLaunchOptions(fromOptions) {
  ...
  // Remote DNS prefs matter if anything falls back to direct SOCKS; local relay is HTTP.
