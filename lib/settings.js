...
/** Default SOCKS5 mobile proxy (iProxy / fxdx). Use socks5h upstream for remote DNS. */
...
export async function applyProxyToLaunchOptions(fromOptions, { forceNewRelay = false } = {}) {
  ...
  // iProxy Wi-Fi Split: phone fetches sites on mobile data, but uploads to the relay over Wi-Fi.
  // curl (1 connection) works; Firefox opens many CONNECT tunnels and stalls the Wi-Fi uplink.
  // Cap concurrency / disable multiplexed HTTP — not longer timeouts.
