...
/**
 * Camoufox login/re-login concurrency. Dashboard DB wins unless LOGIN_PARALLEL_LOCK=1.
 * Env LOGIN_PARALLEL only seeds the DB on first boot (needs LOGIN_PARALLEL_FORCE for >1).
 */
export function getLoginParallel() {
  if (/^(1|true|yes|on)$/i.test(String(process.env.LOGIN_PARALLEL_LOCK || '').trim())) {
    const raw = process.env.LOGIN_PARALLEL;
    const n = raw == null || raw === '' ? 1 : Number(raw);
    if (!Number.isFinite(n)) return 1;
    return Math.min(LOGIN_PARALLEL_MAX, Math.max(1, Math.trunc(n)));
  }
...
export function isLoginParallelLocked() {
  return /^(1|true|yes|on)$/i.test(String(process.env.LOGIN_PARALLEL_LOCK || '').trim());
}
