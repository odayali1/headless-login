...
export async function applyProxyToLaunchOptions(fromOptions, { forceNewRelay = false } = {}) {
  ...
  // SOCKS relay + Wi-Fi Split only: cap parallel CONNECT tunnels on the Wi-Fi uplink.
  if (cfg.mode === 'socks-relay' || (cfg.mode === 'http-direct' && isIproxyWifiSplitMode())) {
    Object.assign(prefs, {
      'network.http.max-connections': 12,
