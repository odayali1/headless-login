...
/** True when dashboard login preset is AWS API Gateway (FireProx). */
export function isLoginAwsApi() {
  return getProxyPresetById(getActiveProxyPresetId())?.kind === 'aws-api';
}

/** True when dashboard login preset is DataImpulse residential IPv4 (sticky or rotating) or AWS API. */
export function isLoginResidentialIpv4() {
  const preset = getProxyPresetById(getActiveProxyPresetId());
  return preset?.kind === 'residential' || preset?.kind === 'aws-api';
}
...
export function selectProxyPreset(id) {
  ...
  if (preset.kind === 'residential' || preset.kind === 'aws-api') {
    setLoginResidentialProxyUrl(preset.url);
    // Login IPv4 / FireProx only — keep hybrid ON and keep Loki on built-in IPv6.
    store?.set('proxy_profile', 'mobile');
  } else {
