...
const DEFAULT_RESIDENTIAL_PROXY_URL = String(process.env.PROXY_RESIDENTIAL_URL || '').trim();
...
export function initSettings(db) {
  ...
  if (DEFAULT_RESIDENTIAL_PROXY_URL) {
    set.run('proxy_residential_url_enc', encryptPassword(DEFAULT_RESIDENTIAL_PROXY_URL));
  } else if (!settings.has('proxy_residential_url_enc') && DEFAULT_RESIDENTIAL_PROXY_URL) {
    set.run('proxy_residential_url_enc', encryptPassword(DEFAULT_RESIDENTIAL_PROXY_URL));
  }

  // PROXY_PROFILE=mobile|residential — env wins on boot; else keep DB / default mobile.
  ...
  } else if (!settings.has('proxy_profile')) {
    set.run('proxy_profile', 'mobile');
  }
...
/**
 * Active proxy backend: mobile (iProxy sticky + changeip) or residential (rotating SOCKS).
 * Env PROXY_PROFILE wins when set; otherwise DB / dashboard toggle.
 */
export function getProxyProfile() {
  ...
  const fromDb = String(store?.get('proxy_profile') || 'mobile').trim().toLowerCase();
  return fromDb === 'residential' ? 'residential' : 'mobile';
}

export function setProxyProfile(profile) {
  const next = String(profile || '').trim().toLowerCase() === 'residential' ? 'residential' : 'mobile';
  const prev = String(store?.get('proxy_profile') || 'mobile').trim().toLowerCase();
...
export function getProxyStatus() {
  ...
  let parsed = null;
  try {
    parsed = parseProxyUrl(getProxyUrl());
  } catch {
  ...
  return {
    ...
    profile,
    residentialConfigured: !!getResidentialProxyUrl(),
    rotatesPerRequest: profile === 'residential',
    configured: !!parsed,
    host: parsed ? parsed.host : null,
    port: parsed ? parsed.port : null,
    username: parsed ? parsed.username : null,
    accountsOnCurrentIp: profile === 'residential' ? 0 : getAccountsOnIp(),
    rotateAfter: profile === 'residential' ? 0 : getRotateAfter(),
    rotateUrl: profile === 'residential' ? null : getRotateUrl(),
  };
}
