  ...
  for (const frame of frames) {
    try {
      const hit = await frame.evaluate(() => {
        ...
        const hasSkip7 = [...document.querySelectorAll('a, button, [role="button"], span, div[role="button"]')].some((el) =>
          /skip for (7|\d+) days|skip for now|days until/i.test(el.textContent || '')
        );
...
export async function clickSkipBackupEmail(page) {
  // Consumer MSA: "Skip for 7 days" / "Skip for now (X days until this is required)"
  const patterns = [
    /skip for 7 days/i,
    /skip for \d+ days/i,
    /skip for now\s*\(/i,
  ...
  const textBtn = page.locator('button, a, [role="button"], [role="link"], span.fui-Link').filter({
    hasText: /skip for (7|\d+) days|skip for now|days until/i,
  }).first();
  if (await textBtn.isVisible({ timeout: 500 }).catch(() => false)) {
    const label = ((await textBtn.innerText().catch(() => '')) || 'skip').trim();
    await textBtn.click({ noWaitAfter: true });
    await page.waitForTimeout(2500);
    await page.waitForLoadState('domcontentloaded', { timeout: 15_000 }).catch(() => {});
    return label;
  }
  ...
  for (const frame of frames) {
    try {
      const clicked = await frame.evaluate(() => {
        ...
        const nodes = [
          ...document.querySelectorAll('a, button, [role="button"], [role="link"], input[type="submit"], span.fui-Link, div[role="button"]'),
        ];
        for (const pattern of patterns) {
