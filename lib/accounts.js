...
import { listStoredAccounts, hasStoredPassword, getAccountPassword } from './db.js';
...
export async function listAccounts() {
  ...
  const accounts = [...profileMap.values()].map((acc) => {
    ...
    return {
      ...
      hasStoredPassword: row ? !!getAccountPassword(row.email, row.target) : false,
    };
  });
...
function formatAccount(data, file) {
  ...
  return {
    ...
    hasStoredPassword: !!getAccountPassword(data.email, data.target) || hasStoredPassword(data.email, data.target),
