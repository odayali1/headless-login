...
import { listStoredAccounts, hasStoredPassword } from './db.js';
...
export async function listAccounts() {
  ...
  const accounts = [...profileMap.values()].map((acc) => {
    ...
    return {
      ...
      hasStoredPassword: acc.hasStoredPassword ?? hasStoredPassword(acc.email, acc.target),
    };
  });
...
function formatAccount(data, file) {
  ...
  return {
    ...
    hasStoredPassword: hasStoredPassword(data.email, data.target),
