...
import { listStoredAccounts } from './db.js';
import {
  deriveHealth,
...
/** Cache list builds — full profile scan of 55k accounts is expensive. */
const ACCOUNTS_CACHE_TTL_MS = Number(process.env.ACCOUNTS_CACHE_TTL_MS || 120_000);
/** Serve last good list this long even when dirty (stops scan storms during bulk login). */
const ACCOUNTS_CACHE_MAX_STALE_MS = Number(process.env.ACCOUNTS_CACHE_MAX_STALE_MS || 300_000);
/** Never start another full 55k scan more often than this — overlapping scans starved cookie SSO. */
const ACCOUNTS_REBUILD_MIN_INTERVAL_MS = Number(process.env.ACCOUNTS_REBUILD_MIN_INTERVAL_MS || 90_000);
const PROFILE_READ_CONCURRENCY = Math.min(
  32,
  Math.max(4, Number(process.env.ACCOUNTS_PROFILE_READ_CONCURRENCY || 12) || 12),
);
let accountsCache = null;
let accountsCacheAt = 0;
let accountsCacheDirty = false;
...
/** Soft invalidate — keep serving stale list until rebuild (critical at 55k). */
export function invalidateAccountsCache() {
  accountsCacheDirty = true;
}

/** Update one row in the in-memory list so due/needs-refresh drops without a 55k rescan. */
export function patchCachedAccountTokens(email, tokens) {
  if (!accountsCache || !email || !tokens?.access_token) return;
  const acc = accountsCache.find((a) => a.email === email);
  if (!acc) return;
  const expiresMs = tokenExpiresInMs(tokens);
  acc.tokenExpiresAt = tokens.expires_at || acc.tokenExpiresAt;
  acc.tokenCapturedAt = tokens.captured_at || acc.tokenCapturedAt;
  acc.tokenValid = isTokenValid(tokens);
  acc.hasAccessToken = true;
  acc.hasRefreshToken = !!tokens.refresh_token || acc.hasRefreshToken;
  acc.tokenExpiresInMin = expiresMs !== null ? Math.round(expiresMs / 60_000) : acc.tokenExpiresInMin;
  acc.tokenScope = tokens.scope || acc.tokenScope;
  const stillDue =
    !acc.tokenValid ||
    !isLiveProfileCardToken(tokens) ||
    (expiresMs !== null && expiresMs <= REFRESH_BUFFER_MS);
  if (!stillDue) {
    acc.health = 'available';
    acc.healthLabel = healthLabel('available');
    acc.needsRefresh = false;
  }
}

/** Hard drop — next listAccounts must rebuild (use sparingly). */
export function invalidateAccountsCacheHard() {
  accountsCache = null;
  accountsCacheAt = 0;
  accountsCacheDirty = true;
}

export function peekAccountsCache() {
  return accountsCache;
}

export function getAccountsCacheInfo() {
  return {
    cached: !!accountsCache,
    dirty: accountsCacheDirty,
    age_ms: accountsCache ? Date.now() - accountsCacheAt : null,
...
async function buildAccountsList() {
  ...
  /** Presence of password_enc — avoid decrypting every account on each list build. */
  const emailsWithPassword = new Set();
  for (const row of stored) {
    ...
    if (row.has_password) emailsWithPassword.add(row.email);
  }

  await mapPool(files, PROFILE_READ_CONCURRENCY, async (file) => {
    try {
      const raw = await fs.readFile(path.join(PROFILES_DIR, file), 'utf8');
      const data = JSON.parse(raw);
      if (!data?.email) return;
      const acc = formatAccount(data, file, emailsWithPassword.has(data.email));
  ...
  for (const [email, row] of storedByEmail) {
    ...
    profileByEmail.set(email, {
      ...
      profileFile: null,
      lastError: null,
      hasStoredPassword: emailsWithPassword.has(email),
  ...
  const accounts = [...profileByEmail.values()].map((acc) => ({
    ...
    hasStoredPassword: emailsWithPassword.has(acc.email),
  }));
...
export async function listAccounts({ bustCache = false } = {}) {
  const now = Date.now();
  if (!bustCache && accountsCache) {
    const age = now - accountsCacheAt;
    if (!accountsCacheDirty && age < ACCOUNTS_CACHE_TTL_MS) {
      return accountsCache;
    }
    // Serve stale immediately; refresh in background so the dashboard never blocks minutes.
    if (age < ACCOUNTS_CACHE_MAX_STALE_MS) {
      if (!accountsCacheInFlight && (accountsCacheDirty || age >= ACCOUNTS_CACHE_TTL_MS)) {
        kickAccountsRebuild({ force: false });
      }
      return accountsCache;
    }
  }
  return kickAccountsRebuild({ force: !!bustCache || !accountsCache });
}

function kickAccountsRebuild({ force = false } = {}) {
  if (accountsCacheInFlight) return accountsCacheInFlight;
  const now = Date.now();
  if (
    !force &&
    accountsCache &&
    now - lastRebuildStartedAt < ACCOUNTS_REBUILD_MIN_INTERVAL_MS &&
    now - accountsCacheAt < ACCOUNTS_CACHE_MAX_STALE_MS
  ) {
...
/** Await a fresh rebuild (dashboard Refresh / stats flush). */
export async function rebuildAccountsList() {
  return kickAccountsRebuild({ force: true });
}
...
/** Most recent login job start, or last success/fail timestamp. */
function lastTryAt(acc) {
  return acc.lastLoginAttemptAt || acc.lastLoginAt || acc.updatedAt || null;
}
...
function formatAccount(data, file, hasStoredPassword = false) {
  ...
  return {
    ...
    lastLoginAttemptAt: data.lastLoginAttemptAt || null,
