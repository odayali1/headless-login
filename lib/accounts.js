...
export function invalidateAccountsCache() {
  // Account health changes after every token refresh. Keep this as a no-op hook
  // for callers, but do not cache account lists/stats.
}
import { deriveHealth, healthLabel, PRIORITY_REFRESH_WINDOW_MS, tokenExpiresInMs } from './account-health.js';
...
export function filterAccounts(accounts, { group = '', health = '', search = '' } = {}) {
  ...
  return accounts.filter((acc) => {
    ...
    if (health === 'expires_soon') {
      ...
      return ms !== null && ms > 0 && ms <= PRIORITY_REFRESH_WINDOW_MS;
    }
...
export async function listAccountsPage({ page = 1, limit = 50, group = '', health = '', search = '' } = {}) {
  ...
  const all = await listAccounts({ bustCache: true });
