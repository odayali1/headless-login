...
/** Refresh when this many ms remain before access token expiry.
 * LiveProfileCard / Loki consumer tokens are typically ~24h (expires_in ≈ 86400), not 1h. */
export const REFRESH_BUFFER_MS = Number(process.env.SMART_REFRESH_BUFFER_MS || 20 * 60 * 1000);
/** Priority window for smooth browserless refresh before tokens become old/expired. */
export const PRIORITY_REFRESH_WINDOW_MS = Number(
  process.env.SMART_REFRESH_PRIORITY_WINDOW_MS || 30 * 60 * 1000
);
export const EXPIRES_SOON_WINDOW_MS = 60 * 60 * 1000;
export const EXPIRES_20M_WINDOW_MS = 20 * 60 * 1000;
...
export function isTokenExpiringSoon(tokens, windowMs = PRIORITY_REFRESH_WINDOW_MS) {
  const ms = tokenExpiresInMs(tokens);
  return ms !== null && ms > 0 && ms <= windowMs;
}
...
export function computeAccountStats(accounts) {
  ...
  for (const acc of accounts) {
    ...
    else if (bs === 'not_prompted') stats.backup_not_prompted++;
