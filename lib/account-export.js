...
export async function listAccountsDueForRefresh() {
  ...
  for (const acc of accounts) {
    ...
    if (acc.health !== 'needs_refresh') continue;
    due.push({
      email: acc.email,
      target: CANONICAL_TARGET,
      expiresMs: Number.isFinite(acc.tokenExpiresInMin) ? acc.tokenExpiresInMin * 60_000 : 0,
    });
  }

  // Same source as dashboard, ordered by urgency.
