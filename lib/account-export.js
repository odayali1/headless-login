...
import { needsSmartRefresh, isReauthRequired, tokenExpiresInMs } from './account-health.js';
...
export async function listAccountsDueForRefresh() {
  ...
  for (const file of files) {
    try {
      ...
      if (!hasValidSession(raw)) continue;
      if (isReauthRequired(raw.lastError)) continue;
      if (raw.lastTokenRefreshAt) {
        const refreshedAgo = now - new Date(raw.lastTokenRefreshAt).getTime();
        if (refreshedAgo >= 0 && refreshedAgo < REFRESH_SUCCESS_COOLDOWN_MS) continue;
      }
      if (needsSmartRefresh(raw.tokens, true)) {
  ...
  // Most urgent first — tokens closest to expiry (or already expired) go first.
  due.sort((a, b) => a.expiresMs - b.expiresMs);
  return due;
}
