...
import { PRIORITY_REFRESH_WINDOW_MS, REFRESH_BUFFER_MS } from './account-health.js';

/** Match token-extract.js — skip HTTP lane while Microsoft rejection is remembered. */
const HTTP_REFRESH_REJECTED_SKIP_MS = Number(process.env.HTTP_REFRESH_REJECTED_SKIP_MS || 24 * 60 * 60 * 1000);

function isHttpRefreshRejectedAccount(acc) {
  if (!acc?.httpRefreshRejectedAt) return false;
  const ago = Date.now() - new Date(acc.httpRefreshRejectedAt).getTime();
...
function liveExpiresMs(acc) {
  if (acc?.tokenExpiresAt) {
    const t = new Date(acc.tokenExpiresAt).getTime();
    if (Number.isFinite(t)) return t - Date.now();
  }
  if (Number.isFinite(acc?.tokenExpiresInMin)) return acc.tokenExpiresInMin * 60_000;
  return null;
}

export async function listAccountsDueForRefresh() {
  // Use short TTL cache from accounts.js — busting every tick re-read ~1k profiles and OOMed.
  const accounts = await listAccounts();
  const due = [];

  for (const acc of accounts) {
    if (!acc?.email) continue;
    // Saved cookies/session are enough for Camoufox recovery. A dead refresh_token
    // (invalid_grant) must NOT exclude the account — that is exactly when slow lane is needed.
    if (!acc.sessionValid) continue;
    const expiresMs = liveExpiresMs(acc);
    const expiresSoon = expiresMs !== null && expiresMs > 0 && expiresMs <= PRIORITY_REFRESH_WINDOW_MS;
    const needsRefreshNow =
      acc.health === 'needs_refresh' ||
      (expiresMs !== null && expiresMs <= REFRESH_BUFFER_MS);
    if (!needsRefreshNow && !expiresSoon) continue;
    due.push({
      email: acc.email,
      target: CANONICAL_TARGET,
      expiresMs: expiresMs ?? 0,
      expiresSoon,
      httpRefreshRejected: isHttpRefreshRejectedAccount(acc),
    });
  }

  // Protect soon-expiring tokens first; old expired sessions are slower recovery work.
  // Within the same urgency band, prefer HTTP-redeemable accounts so dead RTs don't starve Loki.
  due.sort((a, b) => {
    const rank = (x) => {
    ...
    if (a.httpRefreshRejected !== b.httpRefreshRejected) return a.httpRefreshRejected ? 1 : -1;
