...
const listStmt = db.prepare(`
  SELECT a.email, a.target, a.engine, a.created_at, a.updated_at, g.group_name,
    CASE WHEN a.password_enc IS NOT NULL AND length(a.password_enc) > 0 THEN 1 ELSE 0 END AS has_password
  FROM accounts a
...
const insertWebhookStmt = db.prepare(`
  INSERT INTO sync_webhooks (id, url, events, secret, label, enabled, created_at)
  VALUES (@id, @url, @events, @secret, @label, 1, @created_at)
  ON CONFLICT(id) DO UPDATE SET
    ...
    enabled = 1
`);
const listWebhooksStmt = db.prepare('SELECT * FROM sync_webhooks WHERE enabled = 1 ORDER BY created_at ASC');
const allWebhooksStmt = db.prepare('SELECT * FROM sync_webhooks ORDER BY created_at ASC');
const getWebhookStmt = db.prepare('SELECT * FROM sync_webhooks WHERE id = ?');
const deleteWebhookStmt = db.prepare('DELETE FROM sync_webhooks WHERE id = ?');
const listGroupsChangedSinceStmt = db.prepare(`
  SELECT email, target, group_name, updated_at
  FROM account_groups
  WHERE updated_at > ?
...
export function listGroupsChangedSince(sinceIso) {
  return listGroupsChangedSinceStmt.all(sinceIso);
}
...
export function addSyncWebhook({ id, url, events, secret = null, label = null }) {
  const now = new Date().toISOString();
...
export function listSyncWebhooks({ includeDisabled = false } = {}) {
  const rows = includeDisabled ? allWebhooksStmt.all() : listWebhooksStmt.all();
  return rows.map(rowToWebhook);
}

export function getSyncWebhook(id) {
  const row = getWebhookStmt.get(id);
  return row ? rowToWebhook(row) : null;
}

export function deleteSyncWebhook(id) {
  return deleteWebhookStmt.run(id).changes > 0;
}
