...
import { getBulkSyncRecords, listDeltaSyncRecords, SYNC_BULK_MAX } from './account-sync.js';
import {
  registerWebhook,
  removeWebhook,
  listWebhooks,
  EVENT_TOKEN_UPDATED,
} from './sync-webhooks.js';
...
export function createApiV1Router(deps) {
   ...
   * Body: { emails: [...], target?: "outlook", include_refresh_token?: false }
   */
  router.post('/sync/bulk', async (req, res) => {
    try {
      const target = String(req.body?.target || 'outlook');
      if (!TARGETS[target]) {
        return res.status(400).json({ ok: false, error: `Invalid target. Use: ${Object.keys(TARGETS).join(', ')}` });
      }

      const includeRefreshToken =
        req.body?.include_refresh_token === true || req.body?.include_refresh_token === '1';

      const result = await getBulkSyncRecords({
      ...
      res.json({
        ok: true,
        max_batch: SYNC_BULK_MAX,
        ...result,
      });
    } catch (err) {
      if (err.code === 'BULK_LIMIT') return res.status(400).json({ ok: false, error: err.message });
      if (err.code === 'INVALID_BODY') return res.status(400).json({ ok: false, error: err.message });
      res.status(500).json({ ok: false, error: err.message });
    }
  });

  /**
   * GET /api/v1/sync/delta?since=<iso>&target=outlook&group=&limit=500
   */
  router.get('/sync/delta', async (req, res) => {
    try {
      ...
      const result = await listDeltaSyncRecords({
      ...
      if (err.code === 'INVALID_SINCE') return res.status(400).json({ ok: false, error: err.message });
      res.status(500).json({ ok: false, error: err.message });
    }
  });

  /** GET /api/v1/sync/webhooks */
  router.get('/sync/webhooks', (_req, res) => {
    res.json({ ok: true, webhooks: listWebhooks() });
  });

  /**
   * POST /api/v1/sync/webhooks
   * Body: { url, events?: ["account.token_updated"], secret?, label? }
   */
  router.post('/sync/webhooks', (req, res) => {
    try {
      const { url, events, secret, label } = req.body || {};
      const webhook = registerWebhook({ url, events, secret, label });
  ...
  router.delete('/sync/webhooks/:id', (req, res) => {
    const ok = removeWebhook(req.params.id);
    if (!ok) return res.status(404).json({ ok: false, error: 'Webhook not found.' });
    res.json({ ok: true });
  });

  /** GET /api/v1/sync/events — supported webhook event names */
  router.get('/sync/events', (_req, res) => {
    res.json({ ok: true, events: [EVENT_TOKEN_UPDATED] });
  });
