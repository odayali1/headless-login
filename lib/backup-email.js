...
export function createBackupTrace() {
  return { promptSeen: false, skipped: false, skipLabel: null };
}
...
export async function finalizeBackupMarking(email, trace, { reusedSession = false } = {}) {
  ...
  if (trace?.skipped) {
    await patchProfileBackupFields(email, {
      ...
      backupSkipLabel: trace.skipLabel || cur.backupSkipLabel || null,
    });
    return 'skipped';
  }
  if (trace?.promptSeen) {
...
export async function resolveBackupPrompt(page, { leafEmail, skipBackupEmail = true, log, trace, onBackupPromptSeen, onBackupSkipped } = {}) {
  const onPrompt =
    (await isAddBackupEmailSetupScreen(page)) || isInterruptUrl(page.url?.() || '');
  if (!onPrompt) return 'none';
  ...
  // Skip checkbox is ON — wait/retry; Microsoft often paints the skip link a few seconds late.
  for (let attempt = 1; attempt <= 3; attempt++) {
    const skipped = await clickSkipBackupEmail(page, { waitMs: attempt === 1 ? 10_000 : 6_000 });
    if (skipped) {
      if (trace) {
        trace.promptSeen = true;
        trace.skipped = true;
        trace.skipLabel = skipped;
      }
    ...
    if (attempt < 3) {
      log?.('prompt', `Skip button not found yet (try ${attempt}/3) — waiting…`);
      await page.waitForTimeout(2_000);
    }
  }

  if ((await isAddBackupEmailSetupScreen(page)) || isInterruptUrl(page.url?.() || '')) {
    const controls = await listVisiblePromptControls(page).catch(() => []);
    log?.(
      'backup',
      `Backup prompt visible but no skip control found (skip option was ON). url=${page.url()} controls=${(controls || []).slice(0, 12).join(' | ') || '(none)'}`
    );
    await recordBackupRequired(leafEmail);
