...
import { isAddBackupEmailSetupScreen, isInterruptUrl, clickSkipBackupEmail } from './security-prompts.js';
...
export async function resolveBackupPrompt(page, { leafEmail, skipBackupEmail = true, log, trace, onBackupPromptSeen, onBackupSkipped } = {}) {
  ...
  // Skip checkbox ON but no Skip control yet — do NOT hard-fail.
  // Caller may open English proofs/Add, or continue to Outlook if this account has no skip prompt.
  if ((await isAddBackupEmailSetupScreen(page)) || isInterruptUrl(page.url?.() || '')) {
    log?.('backup', 'Backup/interrupt on screen — Skip not found yet (will retry EN proofs or continue)');
    return 'retry';
  }
