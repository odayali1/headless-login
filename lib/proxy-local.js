...
async function resolveProxyModeUnlocked({ force = false } = {}) {
  ...
  // Residential IPv4 login (and legacy full-residential) must NEVER fall back to Coolify PROXY_HTTP_URL.
  if (isLoginResidentialIpv4() || isResidentialProxy()) {
    if (await trySocksProbe()) return resolvedMode;
    // Probe can flake — still force SOCKS relay so Camoufox uses the login residential URL.
    const main = parseProxyUrl(getProxyUrl());
