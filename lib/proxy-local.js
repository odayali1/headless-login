...
export async function resolveProxyMode({ force = false } = {}) {
  ...
  const forceHttpPath = isIproxyWifiSplitMode() || (isMobileRelayProxy() && httpUrl && prefer === 'http');
  ...
  if (forceHttpPath) {
    ...
    if (!ok) {
      if (isMobileRelayProxy() && (await trySocksProbe({ forCompare: true }))) {
        return resolvedMode;
      }
      resolvedMode = null;
      return null;
    }
    const httpMs = lastProbeLatencyMs;
    if (httpMs != null && httpMs > SLOW_PROBE_MS) {
      console.warn(`[proxy] HTTP probe slow (${httpMs}ms) — comparing SOCKS relay (Split uplink)`);
      const savedHttpMs = httpMs;
      resolvedMode = 'http-direct';
      if (await trySocksProbe({ forCompare: true })) {
        const socksMs = lastProbeLatencyMs;
        if (socksMs != null && socksMs < savedHttpMs) {
          console.log(`[proxy] SOCKS faster than HTTP (${socksMs}ms vs ${savedHttpMs}ms) — using relay for Camoufox`);
          return resolvedMode;
        }
        await closeRelayOnly();
      }
      resolvedMode = 'http-direct';
      lastProbeLatencyMs = savedHttpMs;
      console.log(`[proxy] Keeping HTTP direct (${savedHttpMs}ms probe — slow uplink, Teams-first refresh)`);
    }
    return resolvedMode;
  }
...
export async function probeProxyReachability(targetUrl = 'https://login.live.com/') {
  ...
  try {
    ...
    if (resolvedMode === 'http-direct') {
      ...
      if (!result.ok && isMobileRelayProxy() && process.platform !== 'win32') {
        const curl = await probeWithCurlHttp(parsed, targetUrl);
        if (curl.ok) {
          browserPathSplitBlocked = true;
          return {
