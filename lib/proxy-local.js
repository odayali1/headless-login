...
async function tryMobileRelayMode(httpUrl) {
  ...
  // Your tests: Split ON → HTTP Playwright FAIL, SOCKS Playwright OK, curl OK.
  // Camoufox must use SOCKS relay; curl token refresh uses HTTP :16857 separately.
  if (await trySocksProbe({ forCompare: true })) {
    ...
    if (process.platform !== 'win32') {
      ...
      if (curl.ok) {
        console.log(
          `[proxy] Mobile relay: SOCKS for Camoufox (${socksMs}ms), HTTP curl for tokens (${curl.ms}ms)`
        );
      } else {
        console.log(`[proxy] Mobile relay: SOCKS for Camoufox (${socksMs}ms)`);
      }
    } else {
      console.log(`[proxy] Mobile relay: SOCKS for Camoufox (${socksMs}ms)`);
    }
  ...
  if (process.platform !== 'win32') {
    ...
    if (curl.ok) {
      ...
      console.warn(
        `[proxy] curl OK (${curl.ms}ms) but SOCKS+HTTP browser failed — only curl token refresh possible`
      );
    }
  }
