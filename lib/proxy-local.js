...
/** Separate relay for hybrid Loki/MSAL (residential IPv6) — never shared with Camoufox mobile. */
...
/** Playwright proxy opts for hybrid Loki/MSAL (residential) or normal mobile path. */
export async function getPlaywrightProxyConfigForLane(lane = 'browser') {
  if (!isProxyEnabled()) return null;

  if (lane === 'http-refresh' && isHybridProxyEnabled()) {
  ...
  // cookie / browser / default → active profile (mobile when hybrid)
...
async function resolveProxyModeUnlocked({ force = false } = {}) {
  ...
  // Residential rotating SOCKS must NEVER fall back to Coolify PROXY_HTTP_URL (iProxy :16857).
  // That bug sent "residential" logins out the mobile phone while labeling them residential.
  if (isResidentialProxy()) {
    if (await trySocksProbe()) return resolvedMode;
    // Probe can flake — still force SOCKS relay so Camoufox uses PROXY_RESIDENTIAL_URL.
    const main = parseProxyUrl(getProxyUrl());
