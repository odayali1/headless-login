...
export async function createDedicatedLoginProxy() {
  ...
  if (isLoginAwsApi()) {
    const { getFireproxPlaywrightProxy, pinFireproxLocalProxy, unpinFireproxLocalProxy } =
      await import('./fireprox-local-proxy.js');
    const cfg = await getFireproxPlaywrightProxy();
    pinFireproxLocalProxy();
    const id = `login-aws-${++dedicatedLoginSeq}`;
    dedicatedLoginRelays.set(id, cfg.server);
    console.log(`[proxy] Dedicated AWS API (FireProx) relay ${cfg.server} (${id})`);
    return {
      mode: 'aws-api',
      shared: true,
      id,
      ...
      close: async () => {
        ...
        unpinFireproxLocalProxy();
        console.log(`[proxy] Released AWS API relay pin ${id}`);
      },
    };
  }
...
export async function getMobileSocksRelay({ forceNew = false } = {}) {
  ...
  if (!forceNew && mobileCookieRelayUrl && (await isRelayAlive(mobileCookieRelayUrl))) {
...
export async function getPlaywrightProxyConfigForLane(lane = 'browser') {
  ...
  if (lane === 'cookie' && hybridRefresh) {
    // Cookie SSO always on mobile — even when Camoufox login uses IPv4 or AWS API.
    if (isLoginResidentialIpv4() || isLoginAwsApi()) {
...
export async function getPlaywrightProxyConfig({ forceNew = false, forceResolve = false } = {}) {
  ...
  if (resolvedMode === 'aws-api' || isLoginAwsApi()) {
    const { getFireproxPlaywrightProxy } = await import('./fireprox-local-proxy.js');
    const cfg = await getFireproxPlaywrightProxy();
    return {
      mode: 'aws-api',
      server: cfg.server,
...
export async function closeLocalProxy() {
  try {
    const { stopFireproxLocalProxy } = await import('./fireprox-local-proxy.js');
    await stopFireproxLocalProxy({ force: false });
  } catch {
    // optional
  }
  if (relayPinCount > 0) {
...
export async function probeProxyReachability(targetUrl = 'https://login.live.com/') {
  ...
  try {
    ...
    if (resolvedMode === 'aws-api') {
      const cfg = await getPlaywrightProxyConfig();
      const ctx = await (await import('playwright-core')).request.newContext({
      ...
      try {
        ...
        if (result.ok && result.ms != null) lastProbeLatencyMs = result.ms;
        return { ...result, mode: 'aws-api', label: cfg.label, relay: cfg.server };
      } finally {
        await ctx.dispose().catch(() => {});
      }
    }
