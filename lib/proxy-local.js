 ...
 *
 * - SOCKS (DataImpulse): unique local anonymized relay → unique upstream tunnel for this login
 * - HTTP (ProxyScrape): direct CONNECT auth on the browser (no shared local relay)
 */
export async function createDedicatedLoginProxy() {
  ...
  if (parsed.protocol === 'http' || parsed.protocol === 'https') {
    const cfg = httpDirectConfig(parsed);
    console.log(
      `[proxy] Dedicated HTTP login proxy http://${parsed.host}:${parsed.port} (no shared relay)`
    );
    return {
      mode: 'http-direct',
      shared: false,
  ...
  console.log(
    `[proxy] Dedicated SOCKS login relay ${relayUrl} → ${parsed.protocol}://${parsed.host}:${parsed.port} (${id})`
  );
  return {
    mode: 'socks-relay',
    shared: false,
    id,
    server: relayUrl,
    label: `dedicated socks5://${parsed.host}:${parsed.port} via ${relayUrl}`,
    parsed,
    close: async () => {
