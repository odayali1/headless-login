...
const TARGETS = {
  outlook: {
    ...
    // Force English UI for Stay signed in / Skip / Sign in labels (proxy geo often serves Arabic).
    // mkt=en-US is also used on proofs/Add; GCT still returns 200 with this in production.
    loginUrl: 'https://login.live.com/?mkt=en-US',
...
async function runLoginAttempt({
  ...
  try {
    ...
    // Use reliable typing (React/Fluent) — bare fill() often submits without binding the password.
    await interact.fillPassword(page, password);
    await interact.clickSignIn(page);

    // Wait for MSA auth cookies OR a hard Microsoft block — do not rush to Outlook.
...
async function handlePostLogin(page, log, interact, context, { email, skipBackupEmail = true, backupEmailMode = 'skip', backupTrace, onBackupPromptSeen, onBackupSkipped } = {}) {
  ...
  for (let i = 0; i < 15; i++) {
    ...
    if (/password sign-in isn.?t available|sign-in isn.?t available\.?\s*try another method/i.test(body)) {
      return 'password_blocked';
    }
    if (/^too many requests$/i.test(body.trim()) || /\btoo many requests\b/i.test(body)) {
      return 'rate_limited';
    }
  ...
  if (/\btoo many requests\b/i.test(body)) return 'rate_limited';
