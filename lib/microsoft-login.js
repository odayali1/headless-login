...
import { beginLoginLiveShield, endLoginLiveShield } from './proxy.js';
import {
...
const TARGETS = {
  outlook: {
    ...
    // Prefer plain login URL (mkt query can glitch GetCredentialType on some builds).
    // Password detection is field-based, not English-title-based.
...
async function runLoginAttempt({
  ...
  try {
    ...
    if (saved?.state && hasValidSession(saved.state) && isTokenValid(saved.state.tokens)) {
    ...
    // Clear MSA cookies so a half-dead session cannot make Next a no-op.
    ...
    await submitEmailWithRetry(() => browserState.page, email, interact, log, {
      loginUrl: config.loginUrl || config.url,
    ...
    try {
      ...
      if (err.code === 'BACKUP_EMAIL_REQUIRED') {
        ...
        return {
          ...
          message: skipBackupEmail
            ? 'Microsoft backup-email screen has no Skip button for this account (skip option was already ON). Complete add-email manually, or retry later — Microsoft sometimes hides skip.'
            : 'Microsoft is asking for backup email verification. Enable "Skip backup-email prompt" on Re-login, or complete it manually.',
          email,
          target,
          engine: usedEngine,
          url: page.url(),
    ...
    await interact.fillPassword(page, password);
    await interact.clickSignIn(page);
    await page.waitForTimeout(2500);

    // Proxy geo often serves Arabic UI — if Sign in didn't take, password field stays up.
    if (await isPasswordFieldVisible(page)) {
      log('password', 'Password field still up — retrying Sign in (EN/AR)…');
      await interact.clickSignIn(page);
    ...
    if (await isPasswordFieldVisible(page)) {
      ...
      const shot = await captureScreenshot(page, jobId, 'password-stuck', usedEngine);
      return {
        status: 'failed',
        message: `Password Sign-in did not submit (still on password form; title="${title}").`,
        email,
        target,
        engine: usedEngine,
        url: page.url(),
        screenshot: shot ? `screenshots/${jobId}-password-stuck.png` : null,
      };
    }
    ...
    if (postLogin === 'backup_email_required' || postLogin === 'backup_email_required_no_skip') {
      await recordBackupRequired(email);
      const shot = await captureScreenshot(page, jobId, 'backup-email', usedEngine);
      const noSkip = postLogin === 'backup_email_required_no_skip' || skipBackupEmail;
      return {
        status: 'backup_email_required',
        message: noSkip
          ? 'Microsoft backup-email screen has no Skip button for this account (skip option was already ON). Complete add-email manually, or retry later — Microsoft sometimes hides skip.'
          : 'Microsoft is asking for backup email verification. Enable "Skip backup email for 7 days" on Re-login, or complete it manually.',
        email,
        target,
        engine: usedEngine,
        url: page.url(),
    ...
    // If still on proofs/Add — click Skip here (do not open Outlook over it).
    if (isInterruptUrl(page.url()) || (await isBackupEmailScreen(page))) {
      const r = await resolveBackupPrompt(page, {
        leafEmail: email,
        skipBackupEmail,
        backupEmailMode,
        log,
      ...
      if (r === 'backup_email_required' || r === 'backup_email_required_no_skip') {
        return backupFail(
          skipBackupEmail
            ? 'Microsoft backup-email screen — Skip was enabled but could not be clicked.'
            : 'Microsoft is asking for backup email verification. Enable "Skip backup-email prompt" on Re-login.'
        );
      }
    }

    // Open Outlook only after password left the form (old path) — then Skip if MS redirects to proofs.
    log('verify', `Opening ${config.name}…`);
    await page.goto(config.url, { waitUntil: 'domcontentloaded', timeout: navTimeoutMs() }).catch(() => {});
    await page.waitForTimeout(3000);

    for (let i = 0; i < 5; i++) {
      if (!isInterruptUrl(page.url()) && !(await isBackupEmailScreen(page))) break;
      log('prompt', `Outlook redirected to proofs/Add — clicking Skip (${i + 1}/5)…`);
      const r = await resolveBackupPrompt(page, {
        leafEmail: email,
        skipBackupEmail,
        backupEmailMode,
      ...
      if (r === 'backup_email_required' || r === 'backup_email_required_no_skip') {
        return backupFail(
          skipBackupEmail
            ? 'Outlook redirected to proofs/Add — Skip was enabled but could not be clicked.'
            : 'Microsoft is asking for backup email verification. Enable "Skip backup-email prompt" on Re-login.'
        );
      }
    ...
    if (isInterruptUrl(page.url()) || (await isBackupEmailScreen(page))) {
      return backupFail('Still on Microsoft proofs/Add after Skip attempts.');
    }
    ...
    if (!success) {
      ...
      const cookieNames = (await context.cookies())
        .map((c) => c.name)
        .filter((n) => /ESTSAUTH|WLSSC|MSPAuth|NAP|ANON/i.test(n))
        .join(',');
      return {
        status: 'failed',
        message:
          detectErrorMessage(bodyText) ||
          `Login did not complete (url=${page.url().slice(0, 80)}; cookies=${cookieNames || 'none'}).`,
        email,
        target,
        engine: usedEngine,
        url: page.url(),
...
async function buildResult({
  ...
  if (!isTokenValid(tokens)) {
    ...
    for (let sweep = 0; sweep < 4; sweep++) {
      ...
      const skipped = skipPrompts ? await clickSkipBackupEmail(page, { waitMs: 10_000 }) : null;
      if (skipped) {
...
async function isLoggedIn(page, context, config) {
  ...
  const cookies = await context.cookies();
  // Require real SSO cookies — NAP/ANON alone made us mark "Signed in to Outlook" then SSO → login_required.
  ...
  if (!/login\.(live|microsoftonline)\.com|account\.live\.com/i.test(url)) return true;

  const title = (await page.title()).toLowerCase();
  if (title.includes('sign in') || /تسجيل الدخول/.test(title)) return false;
  return false;
}

const chromiumInteract = {
  // Human-like typing + real pointer click — instant evaluate fill was soft-flagging GetCredentialType.
  async fillEmail(page, email) {
    await fillEmailReliable(page, email);
  },
  async fillPassword(page, password) {
    await fillPasswordReliable(page, password);
  },
  async clickNext(page) {
    // Click only — do not waitForPassword here (submitEmailWithRetry polls state).
    await clickPrimaryHuman(page, ['Next', 'التالي', '下一步', 'Suivant', 'Weiter']);
  ...
  async clickStaySignedIn(page, accept = true) {
    const labels = accept
      ? ['Yes', 'نعم', 'Oui', 'Ja', 'Sí', 'はい', 'Sim']
      : ['No', 'لا', 'Non', 'Nein', 'いいえ', 'Não'];
    ...
    const clicked = await page
      .evaluate((yes) => {
        const want = yes
          ? [/^\s*yes\s*$/i, /^\s*نعم\s*$/i, /^\s*oui\s*$/i]
          : [/^\s*no\s*$/i, /^\s*لا\s*$/i, /^\s*non\s*$/i];
...
const obscuraInteract = {
  ...
  clickSignIn: (page) => clickSignIn(page),
  clickStaySignedIn: async (page, accept = true) => {
    const clicked = await page.evaluate((yes) => {
      ...
      const want = yes
        ? [/^\s*yes\s*$/i, /^\s*نعم\s*$/i, /^\s*oui\s*$/i]
        : [/^\s*no\s*$/i, /^\s*لا\s*$/i, /^\s*non\s*$/i];
      const btn = buttons.find((b) => want.some((re) => re.test((b.textContent || b.value || '').trim())));
      if (btn) {
  ...
  while (Date.now() < deadline) {
    ...
    if (onLoginHost) {
      ...
      if (await isAddBackupEmailSetupScreen(page)) {
        ...
        if (r === 'backup_email_required' || r === 'backup_email_required_no_skip') {
          const err = new Error(
            r === 'backup_email_required_no_skip'
              ? 'Backup email screen has no Skip button (skip option was ON).'
              : 'Backup email verification required.'
          );
      ...
      if (dismissed === 'backup_email_required' || dismissed === 'backup_email_required_no_skip') {
        const err = new Error(
          dismissed === 'backup_email_required_no_skip'
            ? 'Backup email screen has no Skip button (skip option was ON).'
            : 'Backup email verification required.'
        );
        err.code = 'BACKUP_EMAIL_REQUIRED';
...
async function fillEmailReliable(page, email) {
  ...
  await input.click();
  await input.fill('');
  await input.pressSequentially(email, { delay: 15 });
...
async function fillPasswordReliable(page, password) {
  ...
  await input.click();
  await input.fill('');
  await input.pressSequentially(password, { delay: 15 });
  const val = await input.inputValue();
  if (val !== password) {
...
async function submitEmailWithRetry(getPage, email, interact, log, { onEmailRetry, maxAttempts = 4, loginUrl = 'https://login.live.com/' } = {}) {
  // Same-IP retries only — never rotate mid-login (changeip breaks GetCredentialType).
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    if (attempt > 1) {
      log('email', `Retrying email step (${attempt}/${maxAttempts}) on same IP…`);
      await onEmailRetry?.(attempt);
    } else {
    ...
    if (attempt > 1) {
      ...
      log('email', 'Soft reload sign-in page (same IP)…');
      await page.goto(loginUrl, { waitUntil: 'commit', timeout: navTimeoutMs() }).catch(() => {});
      await page.waitForLoadState('domcontentloaded', { timeout: domContentLoadedTimeoutMs() }).catch(() => {});
      await waitForLoginForm(page, log).catch(() => {});
    }

    // Quiet ALL browserless HTTP only for this GetCredentialType call — Loki keep-alive resumes after.
    beginLoginLiveShield();
    {
      // Dynamic import avoids microsoft-login ↔ smart-refresh cycle (account-actions → TARGETS).
      const { waitForSmartRefreshHttpQuiet } = await import('./smart-refresh.js');
      await waitForSmartRefreshHttpQuiet(log, 30_000).catch(() => {});
    }

    let gctStatus = null;
    let state;
    try {
      // Watch GetCredentialType — proves Next actually fired Microsoft lookup.
      const onResp = (res) => {
        if (/GetCredentialType/i.test(res.url())) gctStatus = res.status();
      };
      ...
      const filled = await page
        ...
        .inputValue()
        .catch(() => '');
      if (filled.trim().toLowerCase() !== email.trim().toLowerCase()) {
        log('email', `WARNING: email field value mismatch after fill ("${filled.slice(0, 40)}")`);
      }
      await interact.clickNext(page);

      state = await readSignInStep(page);
      const deadline = Date.now() + 12_000;
      while (Date.now() < deadline) {
        if (state.hasPassword || state.emailError || !state.emailVisible) break;
...

      {
        ...
        log(
          'email',
          `After Next: title="${title.slice(0, 60)}" hasPassword=${state.hasPassword} err=${state.emailError || 'none'} gct=${gctStatus ?? 'none'} url=${page.url().slice(0, 70)}`
        );
      }

      // gct=429 is the real failure mode — soft-reloading the same IP burns the budget worse.
      if (gctStatus === 429) {
        const err = new Error(
          'GetCredentialType HTTP 429 — Microsoft rate-limited this IP. Need a clean settled IP.'
        );
        err.code = 'GCT_429';
        throw err;
      }
      ...
      if (!state.emailVisible && !state.emailError) return;

      if (state.emailError) {
        log('email', `Microsoft: ${state.emailError}`);
        if (/issue looking up/i.test(state.emailError)) {
          const err = new Error(state.emailError);
          err.code = 'GCT_LOOKUP';
          throw err;
        }
        continue;
      ...
      // Next was a no-op — try Enter once on same page before full reload retry.
      if (gctStatus == null) {
        log('email', 'Next did not trigger GetCredentialType — pressing Enter…');
        await page.keyboard.press('Enter').catch(() => {});
        await page.waitForTimeout(5000);
        state = await readSignInStep(page);
        if (state.hasPassword || (!state.emailVisible && !state.emailError)) return;
        if (state.emailError) {
          log('email', `Microsoft: ${state.emailError}`);
          continue;
        }
  ...
  if (state.hasPassword) return;
  if (state.emailError) {
    throw new Error(`${state.emailError}. Try again later on this IP (do not rotate mid-login).`);
  }
...
async function clickSignIn(page) {
  // EN + AR (+ common locales) — proxy geo often serves Arabic ("تسجيل الدخول").
  await clickByEvaluate(
    page,
    ['Sign in', 'sign in', 'تسجيل الدخول', 'Se connecter', 'Anmelden', 'Iniciar sesión'],
    'button[data-testid="primaryButton"], #idSIButton9, input[type="submit"]'
  );
  // Only force-click primary if we are still on the password form (don't re-click on next page).
  if (await isPasswordFieldVisible(page)) {
    const primary = page.locator('button[data-testid="primaryButton"], #idSIButton9, input[type="submit"]').first();
...
async function handlePostLogin(page, log, interact, context, { email, skipBackupEmail = true, backupEmailMode = 'skip', backupTrace, onBackupPromptSeen, onBackupSkipped } = {}) {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    if (
      /incorrect password|wrong password|account or password is incorrect|كلمة المرور غير صحيحة|كلمة المرور خاطئة|حسابك أو كلمة المرور غير صحيحة/i.test(
        body
      )
    ) {
    ...
    // Still on password form — Sign in never took (common on AR UI). Bail early, don't spin.
    if (await isPasswordFieldVisible(page)) {
      if (i >= 2) {
        log('password', 'Still on password form after Sign in — aborting post-login wait.');
        return 'pending';
      }
      log('password', 'Password form still visible — clicking Sign in again…');
      await interact.clickSignIn(page);
      await page.keyboard.press('Enter').catch(() => {});
      await page.waitForTimeout(2500);
      continue;
    ...
    // Same as before: if proofs/Add (or interrupt URL), click Skip immediately.
    if (await isAddBackupEmailSetupScreen(page) || isInterruptUrl(url)) {
      log('prompt', `Backup/interrupt on screen — clicking Skip (url=${url.slice(0, 100)})`);
      const r = await resolveBackupPrompt(page, { leafEmail: email, skipBackupEmail, backupEmailMode, log, ...promptArgs });
      if (r === 'backup_email_required' || r === 'backup_email_required_no_skip') return r;
      ...
      if (skipPrompts) {
        const forceSkip = await clickSkipBackupEmail(page, { waitMs: 10_000 });
        if (forceSkip) {
          log('prompt', `Skipped interrupt: "${forceSkip}"`);
          skipCount++;
          await page.waitForTimeout(2500);
          continue;
        }
    ...
    if (STAY_SIGNED_IN_RE.test(body) && !handledStaySignedIn) {
    ...
    if (i === 0 || i % 3 === 0) {
      const authBits = cookies
        .filter((c) => /ESTSAUTH|WLSSC|MSPAuth|NAP|ANON/i.test(c.name))
        .map((c) => c.name)
        .slice(0, 8)
        .join(',');
      const title = (await page.title().catch(() => '')).slice(0, 50);
      log(
        'verify',
        `Post-password ${i + 1}/12 url=${url.slice(0, 90)} auth=${authBits || 'none'} title="${title}"`
      );
    }
  ...
  const cookies = await context.cookies();
  const body = (await page.locator('body').innerText().catch(() => '')).toLowerCase();
  if (STAY_SIGNED_IN_RE.test(body)) return 'pending';

  // Still on proofs/Add after the loop — do NOT return pending (that used to goto Outlook
  // and fake "success" without clicking Skip). Resolve or fail here.
  if (isInterruptUrl(page.url()) || (await isBackupEmailScreen(page))) {
    log('prompt', `Still on backup/interrupt after post-login loop — url=${page.url()}`);
    const r = await resolveBackupPrompt(page, {
      leafEmail: email,
      skipBackupEmail,
      backupEmailMode,
    ...
    if (r === 'skipped' || r === 'completed') return 'pending';
    if (r === 'backup_email_required' || r === 'backup_email_required_no_skip') return r;
    return skipPrompts ? 'backup_email_required_no_skip' : 'backup_email_required';
  }

  return hasMicrosoftSessionCookies(cookies) ? 'done' : 'pending';
}

/** Real MSA SSO cookies only — NAP/ANON show up before password completes and fake "logged in". */
function hasMicrosoftSessionCookies(cookies) {
  const names = new Set(cookies.map((c) => c.name));
  return ['ESTSAUTH', 'ESTSAUTHPERSISTENT', 'WLSSC', 'MSPAuth', '__Host-MSAAUTH'].some((n) => names.has(n));
}

/**
 * When Outlook loads but SSO says login_required, the backup interrupt was often never shown
 * on the login host. Opening proofs/Add surfaces "Skip for now (7 days…)" so we can dismiss it.
 */
async function recoverProofsSkip(page, {
  email,
  skipBackupEmail = true,
  backupEmailMode = 'skip',
  log,
  ...
  const promptArgs = { onBackupPromptSeen, onBackupSkipped, trace: backupTrace };
  log?.('prompt', `Trying proofs/Add skip recovery (url=${page.url().slice(0, 120)})…`);

  let r = await resolveBackupPrompt(page, {
    leafEmail: email,
    skipBackupEmail,
    backupEmailMode,
    log,
  ...
  if (!skipBackupEmail) return r === 'none' ? 'none' : r;

  await page
    .goto('https://account.live.com/proofs/Add', { waitUntil: 'domcontentloaded', timeout: navTimeoutMs() })
    .catch(() => {});
  await page.waitForTimeout(2500);

  r = await resolveBackupPrompt(page, {
    leafEmail: email,
    skipBackupEmail,
    backupEmailMode,
    log,
  ...
  if (skipped) {
    if (onBackupSkipped) await onBackupSkipped(skipped);
    log?.('prompt', `Skipped via proofs/Add recovery: "${skipped}"`);
    return 'skipped';
  }

  if (isInterruptUrl(page.url()) || (await isBackupEmailScreen(page))) {
    return 'backup_email_required_no_skip';
  }
  return 'none';
}
...
function detectErrorMessage(bodyText) {
  const patterns = [
    ...
    /account or password is incorrect/i,
    /كلمة المرور غير صحيحة/i,
    /حسابك أو كلمة المرور غير صحيحة/i,
