...
const TARGETS = {
  outlook: {
    ...
    // Force English UI — geoip locale (e.g. ar-JO) breaks Next/password detection.
    loginUrl: 'https://login.live.com/?mkt=en-US&ui_locales=en-US&lc=1033',
...
async function runLoginAttempt({
  ...
  try {
    ...
    {
      // Exact sequence from scripts/_diag-login-path.mjs (proven to reach password).
      const loginUrl = config.loginUrl || config.url;
      let advanced = false;
      for (let attempt = 1; attempt <= 4 && !advanced; attempt++) {
        if (attempt > 1) {
          log('email', `Retrying email step (${attempt}/4)…`);
        ...
        await page.locator('button[data-testid="primaryButton"]').first().waitFor({ state: 'visible', timeout: 10_000 });
        ...
        if (state.emailError) {
          ...
          if (/issue looking up/i.test(state.emailError)) {
            log('email', 'Waiting 4s then tapping Next once more…');
    ...
    // keep page/context refs current
    page = browserState.page;
    context = browserState.context;
