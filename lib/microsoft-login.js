  ...
  try {
    ...
    await submitEmailWithRetry(() => browserState.page, email, interact, log, {
      onEmailRetry: async (attempt) => {
        // Server may rotate IP at most once; soft-reload first (old working path).
        const rotateResult = await onEmailRetry?.(attempt, { reason: 'stuck' });
        ...
        if (attempt >= 2) {
          ...
          await p.goto(loginUrl, { waitUntil: 'commit', timeout: navTimeoutMs() }).catch(() => {});
          await p.waitForLoadState('domcontentloaded', { timeout: domContentLoadedTimeoutMs() }).catch(() => {});
...
const chromiumInteract = {
  // Old working path: setInputValue (React events) + clickByEvaluate("Next").
  ...
  async clickNext(page) {
    // Do NOT call prepareForPasswordEntry here — that runs after email succeeds.
...
/**
 * Email → Next → wait for password. Uses interact.fillEmail / clickNext
 * (setInputValue + clickByEvaluate "Next") — the path that worked before locator thrash.
 */
async function submitEmailWithRetry(getPage, email, interact, log, { onEmailRetry, maxAttempts = 4 } = {}) {
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    // Poll for password field (old path waited via prepareForPasswordEntry after success).
