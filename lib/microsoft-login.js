...
async function runLoginAttempt({
  ...
  try {
    ...
    log('navigate', `Opening ${config.name} sign-in…`);
    {
      ...
      // Persistent Firefox profile can keep stale MSA cookies that break GetCredentialType
      // ("issue looking up your account") even when Playwright storageState is skipped.
      if (forceFresh && context?.clearCookies) {
      ...
      try {
        ...
        if (isProxyEnabled()) {
          log('proxy', `Cannot reach Microsoft login (${err.message}) — retry once…`);
    ...
    // Same settle as working diag-login-path (do not poll-evaluate the form here).
    await page.waitForTimeout(2500);

    log('email', 'Entering email address…');
    {
      // GetCredentialType needs flowToken/PPFT. Rushing Next → "issue looking up your account".
      // Old working path: one ready form → one fill → one Next → password.
      const loginUrl = config.loginUrl || config.url;
      let advanced = false;

      const waitFlowReady = async (p) => {
        // Require flowToken/PPFT when present — usernameEntry alone is too early (lookup glitch).
        await p
          .waitForFunction(
            () => {
              ...
              const next = document.querySelector('button[data-testid="primaryButton"]');
              if (!field || !next || next.disabled) return false;
              const token = document.querySelector('input[name="flowToken"], input[name="PPFT"], #i0327');
              if (token && String(token.value || '').length > 8) return true;
              if (typeof window.ServerData === 'object' && window.ServerData) return true;
              return false;
            },
            { timeout: 25_000 }
          )
          .catch(() => {});
        // Even if token selector missing on new Fluent UI, give GetCredentialType time to bind.
        await p.waitForTimeout(2000);
      };

      const tryEmailOnce = async (p) => {
        await waitFlowReady(p);
        const input = p.locator('#usernameEntry, #i0116, input[name="loginfmt"], input[type="email"]').first();
        await input.waitFor({ state: 'visible', timeout: 20_000 });
        await input.click({ clickCount: 3 }).catch(() => input.click());
        await p.waitForTimeout(200);
        await input.fill(email);
        await p.waitForTimeout(700);
        await p.locator('button[data-testid="primaryButton"]').first().click({ noWaitAfter: true });

        const deadline = Date.now() + 15_000;
        let state = await readSignInStep(p);
        while (Date.now() < deadline) {
          if (state.hasPassword) return state;
          if (state.emailError) break;
          if (!state.emailVisible) return state;
          await p.waitForTimeout(1000);
          state = await readSignInStep(p);
        }
        // MS: "Tap Next to try again" — one extra Next only, then stop thrashing.
        if (state.emailError && /issue looking up/i.test(state.emailError)) {
          log('email', `Microsoft: ${state.emailError}`);
          log('email', 'Waiting 5s then tapping Next once (do not thrash)…');
          await p.waitForTimeout(5_000);
          await p.locator('button[data-testid="primaryButton"]').first().click({ noWaitAfter: true });
          const deadline2 = Date.now() + 12_000;
          while (Date.now() < deadline2) {
            state = await readSignInStep(p);
            if (state.hasPassword || (!state.emailVisible && !state.emailError)) return state;
            if (state.emailError && !/issue looking up/i.test(state.emailError)) break;
            await p.waitForTimeout(1000);
          }
        }
        return state;
      ...
      for (let attempt = 1; attempt <= 3 && !advanced; attempt++) {
        if (attempt > 1) {
          log('email', `Retrying email step (${attempt}/3)…`);
          log('email', `Soft reload sign-in page (attempt ${attempt})…`);
          await page.goto(loginUrl, { waitUntil: 'domcontentloaded', timeout: navTimeoutMs() }).catch(() => {});
          await page.waitForTimeout(3000);
          await onEmailRetry?.(attempt);
        }
        const state = await tryEmailOnce(page);
        if (state.hasPassword || (!state.emailVisible && !state.emailError)) {
          advanced = true;
          break;
        }
        if (state.emailError) log('email', `Microsoft: ${state.emailError}`);
        else log('email', 'Still on email screen after Next — retrying…');
      }
      if (!advanced) {
        const state = await readSignInStep(page);
        if (state.emailError) throw new Error(`${state.emailError}. Try again or rotate proxy IP.`);
...
async function buildResult({
  ...
  if (!isTokenValid(tokens)) {
    ...
    // Skip-backup / security prompts leave cookies settling — immediate cookie SSO → login_required.
    if (backupTrace?.promptSeen || backupTrace?.skipped) {
      log?.('token', 'Settling 4s after backup/security prompts before token capture…');
...
const chromiumInteract = {
  // Proven path (cce297d / c186726): Playwright fill() + locator Next click.
  ...
  async clickNext(page) {
    await page.locator('button[data-testid="primaryButton"]').first().click({ noWaitAfter: true });
...
async function waitForLoginFlowReady(page) {
  // Diag path: field visible + ~2s settle. Do not require ServerData (often missing on login.live.com).
  await page
...
async function fillEmailReliable(page, email) {
  ...
  // Exact path that reaches password in local diag.
  await input.fill(email);
...
async function readSignInStep(page) {
  return page.evaluate(() => {
    ...
    // Password page keeps a loginfmt display field — language-agnostic checks.
    const onPasswordPage =
      hasPassword ||
      /enter your password/i.test(title) ||
      /enter your password/i.test(lower) ||
      /passwordEntry|name="passwd"/i.test(document.documentElement?.innerHTML?.slice(0, 50000) || '');
...
async function submitEmailWithRetry(getPage, email, interact, log, { onEmailRetry, maxAttempts = 4 } = {}) {
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    // Diag waits ~5s after Next before password is reliable.
    await page.waitForTimeout(5000);

    let state = await readSignInStep(page);
    // Debug: prove whether Next advanced (password title) vs UI stuck.
    {
      const title = await page.title().catch(() => '');
      const url = page.url();
      log(
        'email',
        `After Next: title="${title}" hasPassword=${state.hasPassword} emailVisible=${state.emailVisible} err=${state.emailError || 'none'} url=${url.slice(0, 80)}`
      );
    }
    ...
    if (state.emailError) {
      ...
      if (/issue looking up/i.test(state.emailError)) {
        log('email', 'Waiting 4s then tapping Next once more (Microsoft asks to retry)…');
        await page.waitForTimeout(4_000);
        await interact.clickNext(page);
        ...
        state = await readSignInStep(page);
        {
          ...
          log(
            'email',
            `After retry Next: title="${title}" hasPassword=${state.hasPassword} err=${state.emailError || 'none'}`
          );
        }
        if (state.hasPassword) return;
        if (!state.emailVisible && !state.emailError) return;
