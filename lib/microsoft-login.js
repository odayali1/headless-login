  ...
  try {
    ...
    log('email', 'Entering email address…');
    {
      ...
      const tryEmailOnce = async (p) => {
        ...
        // fill() first (proven local path); sequential only if React rejected the value.
        await input.fill(email);
        await p.waitForTimeout(500 + Math.floor(Math.random() * 400));
        let val = await input.inputValue();
        if (val.trim().toLowerCase() !== email.trim().toLowerCase()) {
          await input.fill('');
          await p.waitForTimeout(100);
          await input.pressSequentially(email, { delay: 40 });
        ...
        // Prefer real mouse click (Camoufox humanize=true) on primary Next.
    ...
    await pwdInput.fill('');
    await pwdInput.pressSequentially(password, { delay: 25 + Math.floor(Math.random() * 35) });
