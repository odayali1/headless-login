  ...
  try {
    ...
    await submitEmailWithRetry(() => browserState.page, email, interact, log, {
      onEmailRetry: async (attempt, meta = {}) => {
        ...
        // Soft-reload like the old working path. Rotate at most once, and only for real UI rate-limit.
        const willTryRotate = !emailRotatedOnce && reason === 'throttled' && attempt >= 2;
...
async function buildResult({
  ...
  if (!isTokenValid(tokens)) {
    ...
    // TokenMan first, then cookie SSO PKCE (same path smart-refresh uses when RT is missing).
...
/**
 * Simple email step — fill → Next → wait for password UI.
 * No GetCredentialType network probe (that logging/429 thrash never existed in the working path).
 */
