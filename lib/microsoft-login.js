...
async function runLoginAttempt({
  ...
  try {
    ...
    // cce297d path: one goto → wait for form → fill → Next (no waitForFunction / no soft-reload thrash).
    ...
    await submitEmailWithRetry(() => browserState.page, email, interact, log, {
      ...
      onEmailRetry: async (attempt) => {
        // Close Camoufox BEFORE rotate — open relay pins block changeip.
        if (attempt >= 2) {
...
async function submitEmailWithRetry(getPage, email, interact, log, { onEmailRetry, maxAttempts = 4, loginUrl = 'https://login.live.com/' } = {}) {
  // Restored cce297d loop: fill → Next → wait. On lookup error, caller rotates IP + reopens browser.
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    if (attempt > 1) {
      ...
      log('email', 'Opening fresh sign-in page on new IP…');
