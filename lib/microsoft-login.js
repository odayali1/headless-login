...
async function runLoginAttempt({
  ...
  try {
    ...
    // proofs/Add often appears only after password — never jump to Outlook while it is open.
    // That was marking "success" without clicking Skip, then token SSO → login_required.
    const preAppBackup = await resolveBackupPrompt(page, {
      leafEmail: email,
      skipBackupEmail,
      backupEmailMode,
      log,
    ...
    if (preAppBackup === 'backup_email_required' || preAppBackup === 'backup_email_required_no_skip') {
      await recordBackupRequired(email);
      const shot = await captureScreenshot(page, jobId, 'backup-email', usedEngine);
      return {
        status: 'backup_email_required',
        message: skipBackupEmail
          ? 'Microsoft backup-email screen has no Skip button for this account (skip option was already ON). Complete add-email manually, or retry later.'
          : 'Microsoft is asking for backup email verification. Enable "Skip backup-email prompt" on Re-login, or complete it manually.',
        email,
        target,
        engine: usedEngine,
    ...
    if (postLogin === 'done' || postLogin === 'pending') {
      ...
      if (hasMicrosoftSessionCookies(cookies) && !isInterruptUrl(page.url()) && !(await isBackupEmailScreen(page))) {
        ...
        const afterMail = await resolveBackupPrompt(page, {
          leafEmail: email,
          skipBackupEmail,
          backupEmailMode,
          log,
        ...
        if (afterMail === 'backup_email_required' || afterMail === 'backup_email_required_no_skip') {
          await recordBackupRequired(email);
          const shot = await captureScreenshot(page, jobId, 'backup-email', usedEngine);
          return {
            status: 'backup_email_required',
    ...
    // Opening the app often redirects to account.live.com/proofs/Add — must Skip here.
    for (let i = 0; i < 4; i++) {
      if (!isInterruptUrl(page.url()) && !(await isBackupEmailScreen(page))) break;
      log('prompt', `App redirect hit backup/interrupt (try ${i + 1}/4) — clicking Skip…`);
    ...
    if (isInterruptUrl(finalUrl) || (await isBackupEmailScreen(page))) {
      await recordBackupRequired(email);
      const shot = await captureScreenshot(page, jobId, 'backup-email', usedEngine);
      return {
        status: 'backup_email_required',
        message: skipBackupEmail
          ? 'Still on Microsoft backup-email / proofs page after Skip attempts. Check screenshot — Skip link may be missing for this session.'
          : 'Microsoft is asking for backup email verification. Enable "Skip backup-email prompt" on Re-login, or complete it manually.',
        email,
        target,
        engine: usedEngine,
    ...
    if (!success && /login\.microsoftonline\.com|login\.live\.com|account\.live\.com/i.test(finalUrl)) {
      const shot = await captureScreenshot(page, jobId, 'error', usedEngine);
...
async function isLoggedIn(page, context, config) {
  ...
  if (hasAuthCookies && !/login\.(live|microsoftonline)\.com|account\.live\.com/i.test(url)) {
    return true;
  }
...
async function handlePostLogin(page, log, interact, context, { email, skipBackupEmail = true, backupEmailMode = 'skip', backupTrace, onBackupPromptSeen, onBackupSkipped } = {}) {
  ...
  for (let i = 0; i < 15; i++) {
    ...
    if (await isAddBackupEmailSetupScreen(page) || isInterruptUrl(url)) {
      ...
      if (skipPrompts) {
        const forceSkip = await clickSkipBackupEmail(page, { waitMs: 6_000 });
        if (forceSkip) {
