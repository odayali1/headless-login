...
async function runLoginAttempt({
  ...
  try {
    ...
    // Simple: click Skip for 7 days if Microsoft shows it, THEN open Outlook.
    if (skipBackupEmail) {
      await waitAndClickSkip7Days(page, log, backupCb);
    }
    ...
    if (postLogin === 'done' || postLogin === 'pending') {
      ...
      if (hasMicrosoftSessionCookies(cookies)) {
        ...
        if (skipBackupEmail) await waitAndClickSkip7Days(page, log, backupCb);
    ...
    // Outlook often redirects to proofs/Add — click Skip, then continue.
    if (skipBackupEmail) {
      await waitAndClickSkip7Days(page, log, backupCb);
    }
...
/** Wait for "Skip for now (7 days…)" and click it, then return. */
async function waitAndClickSkip7Days(page, log, backupCb = {}, { rounds = 10 } = {}) {
  for (let i = 0; i < rounds; i++) {
    const url = page.url();
    const skipVisible =
      (await page.getByText(/Skip for now|days until this is required|تخطي لمدة|تخطي الآن/i).count().catch(() => 0)) > 0;
    const onProofs = isInterruptUrl(url) || /account\.live\.com\/proofs/i.test(url);

    if (skipVisible || onProofs) {
      ...
      // One more hard click via page text
      const link = page.getByText(/Skip for now/i).first();
      if ((await link.count().catch(() => 0)) > 0) {
        await link.click({ force: true, noWaitAfter: true }).catch(() => {});
        log?.('prompt', 'Clicked Skip via getByText');
        if (backupCb.onBackupSkipped) await backupCb.onBackupSkipped('Skip for now');
        await page.waitForTimeout(2500);
        return true;
      }
    }

    // Already on Outlook / Teams with no skip UI — don't stall
    if (/outlook\.(live|office)|teams\.(microsoft|live)/i.test(url) && !skipVisible && !onProofs && i >= 1) {
      return false;
    }
