...
async function runLoginAttempt({
  ...
  try {
    ...
    await dismissSecurityPrompts(page, log, { skipBackupEmail: backupEmailMode === 'hub' ? false : skipBackupEmail, ...backupCb });
    return await buildResult({
      page, context, email, target, jobId, status: 'success', engine: usedEngine, fingerprint, log, skipBackupEmail, backupEmailMode, backupTrace, ...backupCb,
    });
  } catch (err) {
...
async function buildResult({
  ...
  const skipPrompts = backupEmailMode === 'hub' ? false : skipBackupEmail;

  let tokens = existingTokens;
  if (!isTokenValid(tokens)) {
    if (backupEmailMode === 'hub' && (await isBackupEmailScreen(page))) {
      await resolveBackupPrompt(page, { leafEmail: email, skipBackupEmail, backupEmailMode, log, ...promptArgs });
    }
    ...
    tokens = await captureOutlookTokens(page, {
      ...
      dismissPrompts: async (p) => {
        if (backupEmailMode === 'hub' && (await isBackupEmailScreen(p))) {
          await resolveBackupPrompt(p, { leafEmail: email, skipBackupEmail, backupEmailMode, log, ...promptArgs });
        }
        return dismissOutlookBlockingPrompts(p, log, { skipBackupEmail: skipPrompts, maxRounds: 3, ...promptArgs });
      },
    });
