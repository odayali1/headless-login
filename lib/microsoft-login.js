  ...
  try {
    ...
    await submitEmailWithRetry(() => browserState.page, email, interact, log, {
      onEmailRetry: async (attempt, meta = {}) => {
        // Server rotates at most once (earlier for lookup glitches). Soft-reload otherwise.
...
async function buildResult({
  ...
  if (!isTokenValid(tokens)) {
    ...
    // Right after password login, ESTSAUTH is often not ready for API-context SSO.
    // Warm Outlook briefly, then try in-browser PKCE (live cookie jar), then API cookie SSO.
    try {
      if (!/outlook\.live\.com\/mail/i.test(page.url())) {
        await page
          .goto('https://outlook.live.com/mail/', { waitUntil: 'commit', timeout: navTimeoutMs() })
          .catch(() => {});
      }
      await page.waitForTimeout(2500);
      ...
      if (isLiveProfileCardToken(browserTokens) && isTokenValid(browserTokens)) {
        tokens = browserTokens;
        log?.('token', 'LiveProfileCard token captured (in-browser SSO PKCE — no MSAL wait)');
      }

      if (!isLiveProfileCardToken(tokens) || !isTokenValid(tokens)) {
        const cookies = await context.cookies();
        const cookieTokens = await tryCookiePkceAuthCode({ cookies }, log);
        if (isLiveProfileCardToken(cookieTokens) && isTokenValid(cookieTokens)) {
          tokens = cookieTokens;
          log?.('token', 'LiveProfileCard token captured (cookie SSO PKCE — no MSAL wait)');
        }
      }
    } catch (err) {
      log?.('token', `Cookie SSO after login failed: ${err.message}`);
    }
    ...
    if (!isLiveProfileCardToken(tokens) || !isTokenValid(tokens)) {
      ...
      tokens = await captureOutlookTokens(page, {
        ...
        // Fresh login has cookies but usually no RT yet — use Teams-first session capture.
        refreshTokenKnownBad: !existingTokens?.refresh_token,
    ...
    } else if (!tokens?.source?.includes?.('cookie') && tokens?.source !== 'cookie_pkce') {
...
async function submitEmailWithRetry(getPage, email, interact, log, { onEmailRetry, maxAttempts = 4 } = {}) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    try {
      ...
      // Microsoft UI: "Tap next to try again" for lookup glitches.
      if (state.emailError === 'lookup_glitch' || lastReason === 'throttled') {
        log('email', `Microsoft: ${describeEmailError(state.emailError || 'lookup_glitch')}`);
        const pause = lastReason === 'throttled' ? 15_000 : 3_000;
        log('email', `Waiting ${Math.round(pause / 1000)}s then tapping Next again…`);
      ...
      if (state.emailError) {
        ...
        lastReason =
          state.emailError === 'not_found'
            ? 'not_found'
            : state.emailError === 'rate_limited' || lastReason === 'throttled'
              ? 'throttled'
              : 'lookup_failed';
        continue;
      }
