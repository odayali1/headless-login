...
async function runLoginAttempt({
  ...
  try {
    ...
    // Same path as before: open Outlook only after a real session + prompts cleared.
    // Old bug: pending with only NAP/ANON → unconditional goto Outlook → fake success.
    ...
    const canOpenApp = async () => {
      const cookies = await context.cookies();
      if (!hasMicrosoftSessionCookies(cookies)) return false;
      if (isInterruptUrl(page.url()) || (await isBackupEmailScreen(page))) return false;
      return true;
    };

    // Finish Stay signed in / Skip on the Microsoft host first (like handlePostLogin always did).
    if (!(await canOpenApp())) {
      log(
        'verify',
        `Sign-in submitted — finishing Stay signed in / Skip on Microsoft (not jumping to Outlook). url=${page.url().slice(0, 100)}`
      );
      const again = await handlePostLogin(page, log, interact, context, {
        email,
        skipBackupEmail,
        backupEmailMode,
        backupTrace,
      ...
      if (again === 'backup_email_required' || again === 'backup_email_required_no_skip') {
        return backupFail(
          skipBackupEmail
            ? 'Microsoft backup-email screen — Skip was enabled but could not be clicked. Check screenshot.'
            : 'Microsoft is asking for backup email verification. Enable "Skip backup-email prompt" on Re-login.'
        );
      }
      if (again === 'mfa_required') {
        const shot = await captureScreenshot(page, jobId, 'mfa', usedEngine);
        return {
          status: 'mfa_required',
          message: 'Multi-factor authentication required. Complete MFA manually or use an app password.',
      ...
      if (!(await canOpenApp())) {
        const recovered = await recoverProofsSkip(page, {
          email,
          skipBackupEmail,
          backupEmailMode,
          log,
        ...
        if (recovered === 'backup_email_required' || recovered === 'backup_email_required_no_skip') {
          return backupFail(
            'Microsoft proofs/Add interrupt blocked login. Skip was enabled — complete add-email manually if Skip is missing.'
          );
        }
      }
      if (!(await canOpenApp())) {
        ...
        return {
          status: 'failed',
          message: `Login incomplete — no ESTSAUTH yet (will not open Outlook). url=${page.url().slice(0, 80)}; cookies=${cookieNames || 'none'}`,
          email,
          target,
          engine: usedEngine,
          url: page.url(),
    ...
    log('success', `Microsoft session established — opening ${config.name}…`);
    ...
    // Same as before: dismiss Skip / security overlays after landing (natural redirect to proofs/Add).
    ...
    if (!(await isLoggedIn(page, context, config))) {
      ...
      return {
        status: 'failed',
        message: detectErrorMessage(bodyText) || 'Login did not complete — still missing ESTSAUTH after Outlook open.',
        email,
        target,
        engine: usedEngine,
        url: page.url(),
...
const chromiumInteract = {
  ...
  async clickStaySignedIn(page, accept = true) {
    const labels = accept ? ['Yes', 'نعم', 'Oui', 'Ja', 'Sí'] : ['No', 'لا', 'Non', 'Nein'];
...
async function handlePostLogin(page, log, interact, context, { email, skipBackupEmail = true, backupEmailMode = 'skip', backupTrace, onBackupPromptSeen, onBackupSkipped } = {}) {
  ...
  // Stay on login/proofs host longer — do not return pending early and jump to Outlook.
  for (let i = 0; i < 24; i++) {
    const url = page.url();
    ...
    if (i === 0 || i % 3 === 0) {
      const authBits = cookies
        ...
        .slice(0, 8)
        .join(',');
      log('verify', `Post-password ${i + 1}/15 url=${url.slice(0, 100)} auth=${authBits || 'none'}`);
    }
