...
async function runLoginAttempt({
  ...
  try {
    ...
    if (saved?.state && hasValidSession(saved.state) && isTokenValid(saved.state.tokens)) {
      ...
      browserState.session = await connectBrowser({ email, target: CANONICAL_TARGET, fingerprint, saved, forceFresh: false });
    ...
    const reopenBrowser = async (reason) => {
      ...
      // forceFresh=true → no cookies (password login). Keep launch-options.json device (do NOT regenerate).
      ...
      const navMs = navTimeoutMs();
      const domMs = domContentLoadedTimeoutMs();
      try {
        await page.goto(loginUrl, { waitUntil: 'domcontentloaded', timeout: navMs });
      } catch (err) {
        if (isProxyEnabled()) {
          const cfg = await getPlaywrightProxyConfig().catch(() => null);
          if (cfg?.mode === 'http-direct') {
            log('proxy', `Cannot reach Microsoft login (${err.message}) — retry once on mobile proxy…`);
            await page.waitForTimeout(2_000).catch(() => {});
            await page.goto(loginUrl, { waitUntil: 'domcontentloaded', timeout: navMs });
          } else {
            log('proxy', `Cannot reach Microsoft login (${err.message}) — new proxy relay + same Camoufox device…`);
            if (browserState.session) await browserState.session.close().catch(() => {});
      ...
      await waitForLoginFlowReady(page);
    };

    await reopenBrowser(`Connected via Camoufox (persistent profile)`);
    const fpReuse = session?.reusedLaunchOptions ? 'cached launch-options' : 'new launch-options written';
    log('fingerprint', `Stable device profile for ${email} (seed ${fingerprint.seed.slice(0, 8)}…, ${fpReuse})`);
    ...
    await submitEmailWithRetry(() => browserState.page, email, interact, log, {
      onEmailRetry: async (attempt, meta = {}) => {
        // Overnight logs: 231 "forcing IP rotation" but only 3 real rotates ΓÇö this job's own
        // relay pin kept rotateProxyIp stuck at camoufox_busy. Close first so pins can drop.
        if (attempt >= 2 && browserState.session) {
          log('proxy', 'Closing Camoufox before proxy rotate attempt (release relay pin)ΓÇª');
          await browserState.session.close().catch(() => {});
          browserState.session = null;
        }

        const rotateResult = await onEmailRetry?.(attempt, meta);
        const didRotate = rotateResult?.rotated === true;
        if (didRotate) {
          await reopenBrowser('Reconnected after proxy rotationΓÇª');
          return;
        }
        if (!browserState.session) {
          await reopenBrowser('Reopened after deferred rotate (same IP)ΓÇª');
        }
        if (attempt >= 2) {
          const loginUrl = config.loginUrl || config.url;
          const p = browserState.page;
          log('email', 'Soft reload sign-in page (no IP rotate)ΓÇª');
          await p.goto(loginUrl, { waitUntil: 'domcontentloaded', timeout: navTimeoutMs() }).catch(() => {});
          await waitForLoginForm(p, log).catch(() => {});
        }
      },
    });
    page = browserState.page;
    ...
    if (postLogin === 'done' || postLogin === 'pending') {
      ...
      // Cookies sometimes land a beat after post-login redirects (esp. LOGIN_PARALLEL=2).
...
async function buildResult({
  ...
  if (!isTokenValid(tokens)) {
    ...
    // TokenMan path inside captureOutlookTokens: mint any MSAL RT → Loki redeem (no post-login PKCE).
    if (target === 'teams' && !/outlook\.live\.com/i.test(page.url())) {
      ...
      await page
        .goto('https://outlook.live.com/mail/', { waitUntil: 'domcontentloaded', timeout: navTimeoutMs() })
        .catch(() => {});
...
function detectPasswordBlocker(bodyText) {
  ...
  if (/issue looking up your account/i.test(body)) {
    return 'Microsoft lookup glitch on this IP (email may still be valid) — retry or rotate IP.';
  }
  if (/couldn.?t find.*account|account doesn.?t exist|we couldn.?t find an account/i.test(body)) {
    return 'Microsoft says this account was not found — verify the email address.';
  }
...
/** Wait until login page JS is ready (flowToken / ServerData) before GetCredentialType. */
async function waitForLoginFlowReady(page) {
  await page
    .waitForFunction(
      () => {
        const hasField = !!document.querySelector(
          '#usernameEntry, #i0116, input[name="loginfmt"], input[type="email"]'
        );
        if (!hasField) return false;
        // login.live.com / microsoftonline expose config used by GetCredentialType.
        const ready =
          typeof window.ServerData === 'object' ||
          typeof window.$Config === 'object' ||
          !!document.querySelector('input[name="flowToken"], input[name="PPFT"], #i0327');
        return ready || document.readyState === 'complete';
      },
      { timeout: 15_000 }
    )
    .catch(() => {});
  // Brief human settle ΓÇö rushing Next right after paint triggers lookup glitches.
  await page.waitForTimeout(600 + Math.floor(Math.random() * 900));
}

function attachGetCredentialTypeProbe(page) {
  const state = {
    seen: false,
    httpStatus: 0,
    throttleStatus: null,
  ...
  const onResponse = async (response) => {
    ...
    if (!/GetCredentialType/i.test(url)) return;
    state.seen = true;
    state.httpStatus = response.status();
    if (response.status() === 429) {
      state.tooManyRequests = true;
      state.networkError = 'Too Many Requests';
      return;
    }
    try {
      ...
      state.throttleStatus = json?.ThrottleStatus ?? null;
      state.ifExistsResult = json?.IfExistsResult ?? null;
    } catch (err) {
      const msg = String(err.message || err);
      if (/too many requests/i.test(msg)) state.tooManyRequests = true;
      state.networkError = msg;
    }
  };
  const onRequestFailed = (request) => {
    if (!/GetCredentialType/i.test(request.url())) return;
    state.seen = true;
    state.networkError = request.failure()?.errorText || 'request failed';
  };
...
async function fillEmailReliable(page, email) {
  ...
  // Type like a human ΓÇö instant fill() is a common GetCredentialType bot trigger.
  const delay = 45 + Math.floor(Math.random() * 55);
  await input.pressSequentially(email, { delay });
  ...
  if (val.trim().toLowerCase() !== email.trim().toLowerCase()) {
    throw new Error(`Email field did not accept address (got "${val.slice(0, 40)}")`);
  }
}
...
async function readSignInStep(page) {
  return page.evaluate(() => {
    ...
    let emailError = null;
    // "Issue looking up" = GetCredentialType/network glitch ΓÇö NOT "account missing".
    if (/issue looking up your account/i.test(body)) emailError = 'lookup_glitch';
    else if (/couldn.?t find.*account|account doesn.?t exist|we couldn.?t find an account/i.test(lower)) {
      emailError = 'not_found';
    } else if (/enter a valid email/i.test(lower)) emailError = 'invalid';
    else if (/too many|try again later/i.test(lower)) emailError = 'rate_limited';
    return { hasPassword, emailVisible, emailError, bodySnippet: body.replace(/\s+/g, ' ').trim().slice(0, 160) };
  });
}

function describeEmailError(code) {
  if (code === 'lookup_glitch') {
    return 'There was an issue looking up your account (Microsoft/network ΓÇö email may still be valid)';
  }
  if (code === 'not_found') return 'Microsoft says account not found';
  if (code === 'invalid') return 'Invalid email format';
  if (code === 'rate_limited') return 'Too many requests ΓÇö try again later';
  return code || 'unknown email error';
}

async function waitAfterEmailNext(page, { timeoutMs = 12_000 } = {}) {
  const deadline = Date.now() + timeoutMs;
  let state = await readSignInStep(page);
  while (Date.now() < deadline) {
    if (state.hasPassword || state.emailError || !state.emailVisible) return state;
...
async function submitEmailWithRetry(getPage, email, interact, log, { onEmailRetry, maxAttempts = 4 } = {}) {
  let lastReason = 'stuck';

  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    if (attempt > 1) {
      log('email', `Retrying email step (${attempt}/${maxAttempts})ΓÇª`);
      await onEmailRetry?.(attempt, { reason: lastReason });
      // After IP throttle / lookup glitch, give Microsoft a cooldown before next GetCredentialType.
      if (lastReason === 'lookup_failed' || lastReason === 'throttled') {
        const cool = 12_000 + attempt * 4_000;
        log('email', `Cooling down ${Math.round(cool / 1000)}s before next account lookupΓÇª`);
        const pageCool = typeof getPage === 'function' ? await getPage() : getPage;
        await pageCool.waitForTimeout(cool);
      }
    } else {
      log('email', 'Entering email addressΓÇª');
    }
    ...
    const probe = attachGetCredentialTypeProbe(page);
    try {
      await interact.fillEmail(page, email);
      // Pause after typing before Next ΓÇö matches real user behavior on login.live.com.
      await page.waitForTimeout(400 + Math.floor(Math.random() * 700));
      await interact.clickNext(page);

      const gct = await probe.wait(14_000);
      if (gct.seen) {
        if (gct.tooManyRequests || /too many requests/i.test(gct.networkError || '')) {
          log('email', 'GetCredentialType: Microsoft 429 Too Many Requests (IP rate-limited)');
          lastReason = 'throttled';
        } else if (gct.networkError) {
          log('email', `GetCredentialType network fail: ${gct.networkError}`);
          lastReason = 'lookup_failed';
        } else if (gct.throttleStatus != null && Number(gct.throttleStatus) !== 0) {
          log('email', `GetCredentialType throttled (ThrottleStatus=${gct.throttleStatus})`);
          lastReason = 'throttled';
        } else if (gct.httpStatus && gct.httpStatus >= 400) {
          log('email', `GetCredentialType HTTP ${gct.httpStatus}`);
          lastReason = 'lookup_failed';
        } else {
          log(
            'email',
            `GetCredentialType OK (IfExists=${gct.ifExistsResult ?? '?'}, throttle=${gct.throttleStatus ?? 0})`
          );
        }
      } else {
        log('email', 'GetCredentialType response not seen ΓÇö page/proxy may be stuck');
        lastReason = 'lookup_failed';
      }

      let state = await waitAfterEmailNext(page);
      if (state.hasPassword) return;
      if (!state.emailVisible && !state.emailError) return;

      // 429: do NOT tap Next again ΓÇö that deepens the rate-limit. Rotate IP instead.
      if (lastReason === 'throttled' || gct.tooManyRequests) {
        log('email', 'IP rate-limited ΓÇö will rotate proxy (not retrying Next on same IP)');
        continue;
      }

      // Soft lookup glitch (not 429): Microsoft says "Tap next to try again".
      if (state.emailError === 'lookup_glitch') {
        log('email', `Microsoft: ${describeEmailError(state.emailError)}`);
        log('email', 'Waiting 3s then tapping Next againΓÇª');
        await page.waitForTimeout(3_000);
        await interact.clickNext(page);
        await probe.wait(12_000);
        state = await waitAfterEmailNext(page, { timeoutMs: 10_000 });
        if (state.hasPassword) return;
        if (!state.emailVisible && !state.emailError) return;
      }
      ...
      if (state.emailError) {
        log('email', `Microsoft: ${describeEmailError(state.emailError)}`);
        lastReason =
          state.emailError === 'not_found'
            ? 'not_found'
            : state.emailError === 'rate_limited'
              ? 'throttled'
              : 'lookup_failed';
        continue;
      }
      ...
      log('email', 'Still on email screen after Next ΓÇö retryingΓÇª');
      if (lastReason !== 'throttled' && lastReason !== 'lookup_failed') lastReason = 'stuck';
    } finally {
      probe.stop();
    }
  }
  ...
  if (state.emailError) {
    throw new Error(`${describeEmailError(state.emailError)}. Try again or rotate proxy IP.`);
  }
  if (!state.hasPassword && state.emailVisible) {
    throw new Error('Could not advance past email screen ΓÇö Microsoft may be blocking this IP.');
  }
}
