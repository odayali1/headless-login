...
async function runLoginAttempt({
  ...
  try {
    ...
    // Restored Jul 14 (cce297d) login interaction — locator fill + primary Sign-in click.
    ...
    // Keep post-password cookie/block wait (anti fake-success) — not in Jul 14 code.
...
const chromiumInteract = {
  // Jul 14 (cce297d) interaction path — last known batch login that worked broadly.
...
function softBlockCodeFromText(bodyText = '', blockMsg = null) {
  ...
  if (
    /password sign-in isn.?t available|sign-in isn.?t available\.?\s*try another method/i.test(body) ||
    /Password sign-in isn.?t available/i.test(msg)
  ) {
    return 'PASSWORD_BLOCKED';
  }
  if (/\btoo many requests\b/i.test(body) || /Too Many Requests/i.test(msg)) {
    return 'RATE_LIMITED';
  }
