  ...
  try {
    ...
    await submitEmailWithRetry(() => browserState.page, email, interact, log, {
      onEmailRetry: async (attempt) => {
        // Soft-reload first (7338181). e08847b force-rotated every retry and broke lookup.
        // Close Camoufox before rotate so relay pins don't deadlock (keep that part of e08847b).
        if (attempt === 2) {
          const loginUrl = config.loginUrl || config.url;
        ...
        if (attempt >= 3) {
          if (browserState.session) {
            log('proxy', 'Closing Camoufox before proxy rotate (release relay pin)…');
          ...
          return rotateResult || { rotated: false };
        }
        return { rotated: false };
      },
    });
...
const chromiumInteract = {
  // Working path (7338181): setInputValue + clickByEvaluate("Next").
  // e08847b restored fillEmailReliable + locator primaryButton and broke email→password.
...
async function submitEmailWithRetry(getPage, email, interact, log, { onEmailRetry, maxAttempts = 4 } = {}) {
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    // Poll longer — local password screen can take >3s after Next on mobile proxy.
    ...
    // Microsoft "issue looking up" — tap Next again on same page (do NOT rotate yet).
