...
async function buildResult({
  ...
  if (!isTokenValid(tokens)) {
    ...
    // Same fast mid-lane as smart-refresh: fresh ESTSAUTH cookies → PKCE auth-code (~10–20s).
    // Avoids the slow Outlook MSAL wait that often times out right after login.
    try {
      const cookies = await context.cookies();
