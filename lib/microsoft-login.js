...
async function runLoginAttempt({
  ...
  try {
    ...
    // Always leave proofs/Add before token capture — MSAL only loads on Outlook mail.
    if (!/outlook\.live\.com\/mail/i.test(page.url())) {
      log('token', 'Landing Outlook mail after login/skip…');
      await page
...
async function buildResult({
  ...
  if (!isTokenValid(tokens)) {
    ...
    // Token capture must start on Outlook mail — not proofs/Add or account.live.com.
    if (!/outlook\.live\.com\/mail/i.test(page.url())) {
      log?.('token', 'Opening Outlook mail before token capture…');
      await page
        .goto('https://outlook.live.com/mail/', { waitUntil: 'domcontentloaded', timeout: navTimeoutMs() })
        .catch(() => {});
      await page.waitForTimeout(2500);
