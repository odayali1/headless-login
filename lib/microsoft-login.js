...
import { closeLocalProxy } from './proxy-local.js';
import {
...
async function runLoginAttempt({
  ...
  try {
    ...
    const reopenBrowser = async (reason) => {
      ...
      try {
        ...
        if (isProxyEnabled()) {
          ...
          // forceFresh relay via closed singleton; persistent Firefox profile still reused
    ...
    if (!forceFresh && saved?.state && hasValidSession(saved.state)) {
      ...
      if (!needsToken) {
        await page.goto(config.url, { waitUntil: 'domcontentloaded', timeout: 60_000 }).catch(() => {});
...
async function buildResult({
  ...
  if (!isTokenValid(tokens)) {
    ...
    if (target === 'teams' && !/outlook\.live\.com/i.test(page.url())) {
      ...
      await page.goto('https://outlook.live.com/mail/', { waitUntil: 'domcontentloaded', timeout: 60_000 }).catch(() => {});
