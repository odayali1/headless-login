...
async function runLoginAttempt({
  ...
  try {
    ...
    if (!afterPwd.ok) {
      ...
      const blockMsg =
        afterPwd.block ||
        detectPostPasswordBlockFromText(bodyText) ||
        `Password submitted but no MSA auth cookies (url=${page.url().slice(0, 90)} body="${String(bodyText)
          .slice(0, 160)
          .replace(/\s+/g, ' ')}")`;
      const softCode = afterPwd.code || softBlockCodeFromText(bodyText, blockMsg) || 'NO_AUTH_COOKIES';
      log('error', blockMsg);
...
async function clickStaySignedInYes(page, accept = true) {
  if (!accept) {
    ...
    for (const label of noLabels) {
      ...
      if (await btn.isVisible({ timeout: 600 }).catch(() => false)) {
        await btn.click({ noWaitAfter: true, delay: 30 }).catch(() => {});
        await page.waitForTimeout(2000);
        return true;
      }
    }
  ...
  // DOM click first — avoids Page.dispatchMouseEvent crash when Camoufox closes mid-click.
  ...
  if (await classicYes.isVisible({ timeout: 800 }).catch(() => false)) {
    await classicYes.click({ noWaitAfter: true, delay: 30 }).catch(() => {});
    await page.waitForTimeout(2500);
    return true;
  }
  ...
  if (await primary.isVisible({ timeout: 800 }).catch(() => false)) {
    ...
    if (STAY_SIGNED_IN_RE.test(body)) {
      await primary.click({ noWaitAfter: true, delay: 30 }).catch(() => {});
      await page.waitForTimeout(2500);
      return true;
    }
  }
...
const chromiumInteract = {
  // Prefer DOM click (evaluate) — Playwright Mouse.click → Page.dispatchMouseEvent crashes Node when Camoufox closes mid-click.
  ...
  async clickNext(page) {
    await clickByEvaluate(
      page,
      ['Next', 'Siguiente', 'Avançar', 'Weiter', 'Suivant', 'التالي'],
  ...
  async clickSignIn(page) {
    await clickByEvaluate(
      page,
      ['Sign in', 'Next', 'Siguiente', 'Avançar', 'Anmelden', 'Se connecter', 'التالي', 'تسجيل الدخول'],
      'button[data-testid="primaryButton"], #idSIButton9, input[type="submit"]'
    );
    await page.waitForTimeout(2500);
  },
...
async function fillEmailReliable(page, email) {
  ...
  // Fluent email field can hang on a normal click ("performing click action") under slow
  // residential SOCKS — focus/force avoids a 30s hard fail when the input is already usable.
  try {
    await input.click({ timeout: 8_000, delay: 20 });
  } catch {
    await input.focus({ timeout: 5_000 }).catch(() => {});
    await input.click({ force: true, timeout: 5_000 }).catch(() => {});
  }
  await input.fill('');
...
/** Prefer DOM click — Mouse CDP (Page.dispatchMouseEvent) kills the process when the page closes. */
...
/** EN + AR + ES + PT — proven soft-blocks from production logs. */
const PASSWORD_SIGNIN_BLOCKED_RE =
  /password sign-in isn.?t available|sign-in isn.?t available\.?\s*try another method|el inicio de sesi[oó]n con contrase[nñ]a no est[aá] disponible|pruebe con otro m[eé]todo|entrada com senha n[aã]o est[aá] dispon[ií]vel|experimente outro m[eé]todo|تسجيل الدخول باستخدام كلمة المرور غير متوف|كلمة المرور غير متوف|جر.?ب طريقة أخرى/i;
...
function detectPostPasswordBlockFromText(bodyText = '') {
  ...
  if (
    /^too many requests$/i.test(body.trim()) ||
    /\btoo many requests\b/i.test(body) ||
    /intentaste iniciar sesi[oó]n demasiadas veces/i.test(body) ||
    /too many.*times.*(incorrect|wrong|password|account)/i.test(body) ||
    /طلبات كثيرة جدا|عدد الطلبات كبير/i.test(body)
  ) {
