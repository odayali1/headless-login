  ...
  try {
    ...
    await submitEmailWithRetry(() => browserState.page, email, interact, log, {
      onEmailRetry: async (attempt, meta = {}) => {
        ...
        // Only rotate when the UI itself shows rate-limit — never on probe HTTP status alone.
        const willTryRotate = reason === 'throttled' && !emailRotatedOnce && attempt >= 2;
        ...
        const rotateResult = willTryRotate ? await onEmailRetry?.(attempt, meta) : null;
        const didRotate = rotateResult?.rotated === true;
        if (didRotate) {
          emailRotatedOnce = true;
          await reopenBrowser('Reconnected after proxy rotation…');
          return rotateResult;
        }
        if (!browserState.session) {
          await reopenBrowser('Reopened after soft reload…');
        }
        if (attempt >= 2) {
          const loginUrl = config.loginUrl || config.url;
          const p = browserState.page;
          log('email', 'Soft reload sign-in page…');
...
function attachGetCredentialTypeProbe(page) {
  ...
  // Status-only — do NOT response.text()/json(). Consuming the body races the page's own
  // GetCredentialType handler and can make login.live.com retry / look like a 429 storm.
  const onResponse = (response) => {
    const url = response.url();
    if (!/GetCredentialType/i.test(url)) return;
    state.seen = true;
    state.httpStatus = response.status();
    if (response.status() === 429) state.tooManyRequests = true;
  };
...
async function fillEmailReliable(page, email) {
  ...
  // fill() once — pressSequentially fires input events that can trigger extra GetCredentialType calls.
...
/**
 * Email step — UI-driven (password field / errors), not GetCredentialType-probe-driven.
 * The probe used to call response.text() and abort on HTTP 429, which closed Camoufox,
 * rotated IP, and re-submitted 4x — that IS the request storm that looks like "429".
 */
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    if (attempt > 1) {
      ...
      if (lastReason === 'lookup_failed' || lastReason === 'throttled') {
        const cool = 4_000 + attempt * 2_000;
    ...
    try {
      ...
      // Same as the old working path — give the page time before judging the UI.
      await page.waitForTimeout(2_500);

      if (probe.state.seen) {
        if (probe.state.tooManyRequests) {
          log('email', `GetCredentialType HTTP ${probe.state.httpStatus} (logged only — waiting for password UI)`);
        } else {
          log('email', `GetCredentialType HTTP ${probe.state.httpStatus || '?'}`);
        }
      }
      ...
      // HTTP 429 on GetCredentialType + still on email → one IP rotate (not 4 soft-reloads on hot IP).
      if (probe.state.tooManyRequests) {
        log('email', 'GetCredentialType 429 and still on email — will rotate proxy once');
        lastReason = 'throttled';
        continue;
      }

      // Soft lookup glitch: Microsoft says "Tap next to try again" — one more Next, no rotate.
