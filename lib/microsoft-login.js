...
import { loadProfile, saveProfile, hasValidSession } from './profile.js';
...
export async function loginMicrosoft({
  ...
  if (needsFreshLogin && password) {
    log('engine', 'Camoufox — stable Firefox profile + stealth fingerprint (same device every time).');
    return runLoginAttempt({
      email, password, target, config, engine: useEngine, saved, jobId, log, forceFresh, skipBackupEmail, onEmailRetry,
    });
  }
...
async function runLoginAttempt({
  email, password, target, config, engine, saved, jobId, log, forceFresh = false, skipBackupEmail = true, onEmailRetry,
}) {
  let session;
  let context;
  let page;
  ...
  try {
    ...
    const reopenBrowser = async (reason) => {
      ...
      browserState.session = await connectBrowser({ email, target, fingerprint, saved, forceFresh: true });
    ...
    if (!forceFresh && saved?.state && hasValidSession(saved.state)) {
      ...
      const needsToken =
        target === 'outlook' && !isTokenValid(saved.state.tokens);
      if (!needsToken) {
    ...
    if (!/login\.microsoftonline\.com|login\.live\.com|login\.microsoft\.com/i.test(currentUrl)) {
      if (await isLoggedIn(page, context, config)) {
        ...
        return await buildResult({
          page, context, email, target, jobId, status: 'success', engine: usedEngine, fingerprint, log, skipBackupEmail,
        });
      }
    }
    ...
    try {
      await prepareForPasswordEntry(page, log, { skipBackupEmail, jobId, engine: usedEngine });
    } catch (err) {
    ...
    const postLogin = await handlePostLogin(page, log, interact, context, { skipBackupEmail });
    ...
    if (postLogin === 'done' || postLogin === 'pending') {
      ...
      if (hasMicrosoftSessionCookies(cookies)) {
        ...
        log('token', 'Opening Outlook to capture LiveProfileCard token…');
        await page.goto(config.url, { waitUntil: 'domcontentloaded', timeout: 45_000 }).catch(() => {});
        await page.waitForTimeout(2500);
        await dismissSecurityPrompts(page, log, { skipBackupEmail });
        return await buildResult({
          page, context, email, target, jobId, status: 'success', engine: usedEngine, fingerprint, log, skipBackupEmail,
        });
      }
...
async function buildResult({
  ...
  if (target === 'outlook' && !isTokenValid(tokens)) {
    await dismissSecurityPrompts(page, log, { skipBackupEmail });
    log?.('token', 'Requesting LiveProfileCard.Access token…');
    tokens = await captureOutlookTokens(page, { log, context, engine, existingTokens: existingTokens || tokens });
  ...
  const profileFile = await saveProfile(context, email, target, {
    engine,
    staySignedIn: true,
    jobId,
    lastStatus: status,
    fingerprint,
    tokens: isTokenValid(tokens) || isLiveProfileCardToken(tokens) ? tokens : existingTokens,
  });
...
async function prepareForPasswordEntry(page, log, { skipBackupEmail = true, timeoutMs = 50_000, jobId, engine } = {}) {
  ...
  while (Date.now() < deadline) {
    ...
    if (onLoginHost) {
      if (await isBackupEmailScreen(page)) {
        ...
        if (skipped) {
          log?.('prompt', `Skipped backup-email prompt before password: "${skipped}"`);
          await page.waitForTimeout(3000);
          continue;
        }
      }

      const dismissed = await dismissSecurityPrompts(page, log, { skipBackupEmail });
...
async function handlePostLogin(page, log, interact, context, { skipBackupEmail = true } = {}) {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    if (/stay signed in/i.test(body)) {
      // Don't treat as done while the KMSI prompt is still visible
    } else if (cookies.length >= 12 || hasMicrosoftSessionCookies(cookies)) {
      return 'done';
    }
...
async function isBackupEmailScreen(page) {
  return page.evaluate(() => {
    const body = (document.body?.innerText || '').toLowerCase();
...
async function clickSkipBackupEmail(page) {
  const patterns = [
    /skip for 7 days/i,
    /skip for now/i,
    /remind me later/i,
...
async function dismissSecurityPrompts(page, log, { skipBackupEmail = true } = {}) {
  if (await isBackupEmailScreen(page)) {
    if (!skipBackupEmail) {
  ...
  const generic = await page.evaluate(() => {
    const patterns = [/skip for now/i, /not now/i, /remind me later/i];
    const nodes = [...document.querySelectorAll('a, button, [role="button"], [role="link"]')];
    for (const pattern of patterns) {
      for (const el of nodes) {
        const text = (el.textContent || '').replace(/\s+/g, ' ').trim();
