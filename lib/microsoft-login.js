...
async function runLoginAttempt({
  ...
  try {
    ...
    const reopenBrowser = async (reason) => {
      ...
      try {
        await page.goto(loginUrl, { waitUntil: 'commit', timeout: navMs });
        await page.waitForLoadState('domcontentloaded', { timeout: domMs }).catch(() => {});
      } catch (err) {
        if (isProxyEnabled()) {
          ...
          if (cfg?.mode === 'http-direct') {
            ...
            log('proxy', `Cannot reach Microsoft login (${err.message}) — new proxy relay + fresh Camoufox…`);
            if (browserState.session) await browserState.session.close().catch(() => {});
...
async function submitEmailWithRetry(getPage, email, interact, log, { onEmailRetry, maxAttempts = 4 } = {}) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    // Microsoft UI often says "Tap next to try again" for lookup glitches.
    if (state.emailError === 'lookup_glitch') {
      log('email', `Microsoft: ${describeEmailError(state.emailError)}`);
      log('email', 'Tapping Next again (lookup glitch — not “email missing”)…');
      await interact.clickNext(page);
    ...
    if (state.emailError) {
      log('email', `Microsoft: ${describeEmailError(state.emailError)}`);
      lastReason = state.emailError === 'lookup_glitch' || state.emailError === 'rate_limited'
        ? 'lookup_failed'
