...
export async function loginMicrosoft({
  ...
  if (needsFreshLogin && password) {
    ...
    return runLoginAttempt({
      email, password, target, config, engine: useEngine, saved, jobId, log, forceFresh, regenerateFingerprint, skipBackupEmail, backupEmailMode, onEmailRetry,
    });
  }
...
async function runLoginAttempt({
  email, password, target, config, engine, saved, jobId, log, forceFresh = false, regenerateFingerprint = false, skipBackupEmail = true, backupEmailMode = 'skip', onEmailRetry,
}) {
  let session;
  let context;
  let page;
