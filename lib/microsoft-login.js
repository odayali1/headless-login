...
async function runLoginAttempt({
  ...
  try {
    ...
    // Force English proofs/Add → click Skip → then open Outlook.
    if (skipBackupEmail) {
...
/**
 * Open English proofs/Add (same screen as laptop), click "Skip for now (7 days…)", done.
 * Login URL stays plain — mkt only on this proofs step.
 */
async function openProofsAndClickSkip(page, log, backupCb = {}) {
  // Already on the skip screen — just click.
  if (
    /account\.live\.com\/proofs/i.test(page.url()) ||
...
async function clickSkipOnCurrentPage(page, log, backupCb = {}) {
  ...
  const looksLikeSkip =
    /account\.live\.com\/proofs/i.test(url) ||
    /skip for now|days until this is required|let'?s protect your account|#iShowSkip/i.test(body) ||
    (await page.locator('#iShowSkip').count().catch(() => 0)) > 0 ||
    (await page.getByText(/Skip for now/i).count().catch(() => 0)) > 0;

  if (!looksLikeSkip) {
    log?.('prompt', `No Skip screen (url=${url.slice(0, 100)}) — continuing`);
    return false;
  }
  ...
  log?.('prompt', 'proofs/Add visible but Skip click failed');
  return false;
}
