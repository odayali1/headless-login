  ...
  try {
    ...
    await submitEmailWithRetry(() => browserState.page, email, interact, log, {
      onEmailRetry: async (attempt, meta = {}) => {
        // At most one rotate (server-side). Soft-reload between attempts — do NOT close Camoufox
        // on every retry (that thrash was added for speed/429 and broke first-attempt login).
...
async function submitEmailWithRetry(getPage, email, interact, log, { onEmailRetry, maxAttempts = 4 } = {}) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    // Simple path (pre speed-up / 429-force-rotate): fill → Next → watch UI.
    // No GetCredentialType probe driving close/rotate loops — that path broke first-attempt login.
