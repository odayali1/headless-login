...
async function runLoginAttempt({
  ...
  try {
    ...
    // Pre-Jul14: do NOT clearCookies here — wiping MSA jar right before login looked more bot-like
    // and was not part of the working bulk path. forceFresh already skips injecting saved cookies.
    await page.goto(config.loginUrl || config.url, { waitUntil: 'commit', timeout: navTimeoutMs() });
    await page.waitForLoadState('domcontentloaded', { timeout: domContentLoadedTimeoutMs() }).catch(() => {});
    // Brief human pause on the form before typing (Camoufox humanize moves cursor on clicks).
...
const chromiumInteract = {
  // Jul 16 (a9cff9f) human typing — pressSequentially beat instant fill() for MSA Fluent.
  ...
  async clickNext(page) {
    ...
    await btn.click({ delay: 40 + Math.floor(Math.random() * 80), noWaitAfter: true });
    await page.waitForTimeout(2500);
  },
  async clickSignIn(page) {
    ...
    await btn.click({ delay: 50 + Math.floor(Math.random() * 100), noWaitAfter: true });
    await page.waitForTimeout(2500);
  },
