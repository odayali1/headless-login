...
async function runLoginAttempt({
  ...
  try {
    ...
    await submitEmailWithRetry(() => browserState.page, email, interact, log, {
      ...
      onGct429: async () => {
        // Proven signal from logs: gct=429. Soft-reload on same IP is useless.
        // Close → force rotate → settle → reopen once.
        log('email', 'GetCredentialType 429 — closing browser, rotating IP once, then retry…');
        if (browserState.session) {
          await browserState.session.close().catch(() => {});
          browserState.session = null;
        }
        await onEmailRetry?.('gct429');
        await reopenBrowser('Reconnected after GetCredentialType 429 rotate…');
      },
    });
    page = browserState.page;
    context = browserState.context;
...
const chromiumInteract = {
  // Native-value fill + evaluate click — Playwright locator Next often no-ops on Fluent UI.
...
async function submitEmailWithRetry(getPage, email, interact, log, { onGct429, maxAttempts = 4, loginUrl = 'https://login.live.com/?mkt=en-US' } = {}) {
  /**
   * Root cause from production logs: GetCredentialType returns HTTP 429.
   * Soft-reloading 4x on the same IP only burns the quota more (Tadors all gct=429).
   * Orbury recovered after cooldown on same IP (gct=200) — so cool first, rotate only if still 429.
   */
  const coolMs = Number(process.env.LOGIN_GCT_429_COOLDOWN_MS || 45_000);
  let rotatedFor429 = false;
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    if (attempt > 1) {
      ...
      log('email', 'Opening sign-in page…');
    ...
    while (Date.now() < deadline) {
      ...
      if (gctStatus != null && gctStatus !== 200) break;
    ...
    // --- GetCredentialType 429: stop thrashing ---
    if (gctStatus === 429) {
      log('email', 'Microsoft GetCredentialType HTTP 429 (Too Many Requests) on this IP');

      // 1) Cool without more lookups — Orbury recovered this way.
      if (coolMs > 0) {
        log('email', `Cooling ${Math.round(coolMs / 1000)}s (no more GetCredentialType)…`);
        await page.waitForTimeout(coolMs);
      }

      // One quiet retry on same page (fill + Next only — no soft-reload storm).
      gctStatus = null;
      const onResp2 = (res) => {
        if (/GetCredentialType/i.test(res.url())) gctStatus = res.status();
      };
      ...
      state = await readSignInStep(page);
      for (let i = 0; i < 10; i++) {
        if (state.hasPassword || state.emailError || (gctStatus != null && gctStatus !== 200)) break;
        await page.waitForTimeout(1000);
        state = await readSignInStep(page);
      }
      ...
      log(
        'email',
        `After 429 cooldown: hasPassword=${state.hasPassword} gct=${gctStatus ?? 'none'}`
      );
      if (state.hasPassword) return;
      if (!state.emailVisible && !state.emailError) return;

      // 2) Still 429 → one forced IP rotate (only once per login).
      if (gctStatus === 429 && !rotatedFor429 && onGct429) {
        rotatedFor429 = true;
        await onGct429();
        continue;
      }
      ...
      throw new Error(
        'GetCredentialType HTTP 429 — Microsoft rate-limited this IP. Wait or rotate the mobile IP, then retry.'
      );
    }
  ...
  if (state.hasPassword) return;
  if (state.emailError) {
    throw new Error(`${state.emailError}. Microsoft blocked account lookup on this IP.`);
  }
  if (state.emailVisible) {
    throw new Error('Could not advance past email screen — GetCredentialType did not succeed.');
  }
}
