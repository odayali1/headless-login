...
async function runLoginAttempt({
  ...
  try {
    ...
    let emailRotateBudget = { throttle: 0, soft: false };
    await submitEmailWithRetry(() => browserState.page, email, interact, log, {
      onEmailRetry: async (attempt, meta = {}) => {
        ...
        const maxThrottle = Number(process.env.LOGIN_MAX_THROTTLE_ROTATES || 2);
        const willTryRotate =
          reason === 'throttled'
            ? attempt >= 2 && emailRotateBudget.throttle < maxThrottle
            : reason === 'lookup_failed'
              ? attempt >= 2 && !emailRotateBudget.soft
              : reason === 'stuck'
                ? attempt >= 3 && !emailRotateBudget.soft
                : false;

        if (willTryRotate && browserState.session) {
          log('proxy', 'Closing Camoufox before proxy rotate attempt (release relay pin)…');
        ...
        if (didRotate) {
          if (reason === 'throttled') emailRotateBudget.throttle += 1;
          else emailRotateBudget.soft = true;
          await reopenBrowser('Reconnected after proxy rotation…');
          return rotateResult;
        }
        if (!browserState.session) {
          await reopenBrowser('Reopened after deferred rotate (same IP)…');
        }
        if (attempt >= 2 && !didRotate) {
          const loginUrl = config.loginUrl || config.url;
          const p = browserState.page;
          log('email', 'Soft reload sign-in page (no IP rotate)…');
...
async function waitForLoginFlowReady(page) {
  await page
    ...
    .catch(() => {});
  // Brief human settle … rushing Next right after paint triggers lookup glitches.
...
async function fillEmailReliable(page, email) {
  ...
  // Type like a human … instant fill() is a common GetCredentialType bot trigger.
...
async function readSignInStep(page) {
  return page.evaluate(() => {
    ...
    let emailError = null;
    // "Issue looking up" = GetCredentialType/network glitch … NOT "account missing".
...
function describeEmailError(code) {
  if (code === 'lookup_glitch') {
    return 'There was an issue looking up your account (Microsoft/network … email may still be valid)';
  }
...
async function submitEmailWithRetry(getPage, email, interact, log, { onEmailRetry, maxAttempts = 4 } = {}) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    if (attempt > 1) {
      ...
      const rotateResult = await onEmailRetry?.(attempt, { reason: lastReason });
      ...
      if (lastReason === 'lookup_failed' || lastReason === 'throttled') {
        const cool = rotateResult?.rotated ? 45_000 : 15_000 + attempt * 5_000;
        log('email', `Cooling down ${Math.round(cool / 1000)}s before next account lookup…`);
    ...
    try {
      ...
      // Pause after typing before Next … matches real user behavior on login.live.com.
      ...
      if (gct.seen) {
        ...
        log('email', 'GetCredentialType response not seen … page/proxy may be stuck');
        lastReason = 'lookup_failed';
      }
      ...
      // 429: do NOT tap Next again … that deepens the rate-limit. Rotate IP instead.
      if (lastReason === 'throttled' || gct.tooManyRequests) {
        log('email', 'IP rate-limited … will rotate proxy (not retrying Next on same IP)');
        continue;
      }
      ...
      if (state.emailError === 'lookup_glitch') {
        ...
        log('email', 'Waiting 3s then tapping Next again…');
