  ...
  try {
    ...
    await submitEmailWithRetry(() => browserState.page, email, interact, log, {
      onEmailRetry: async (attempt, meta = {}) => {
        // Overnight logs: 231 "forcing IP rotation" but only 3 real rotates — this job's own
        ...
        if (!browserState.session) {
          const reason =
            rotateResult?.reason === 'same_ip'
              ? 'Reopened after rotate that kept same exit IP…'
              : 'Reopened after deferred rotate (same IP)…';
          await reopenBrowser(reason);
        }
        if (attempt >= 2) {
          const loginUrl = config.loginUrl || config.url;
...
async function fillEmailReliable(page, email) {
  ...
  await page.waitForTimeout(200);
  // Instant fill — pressSequentially can fire multiple GetCredentialType lookups while typing.
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    if (attempt > 1) {
      ...
      if (lastReason === 'lookup_failed' || lastReason === 'throttled') {
        const cool = rotateResult?.rotated
          ? 20_000
          : rotateResult?.reason === 'same_ip'
            ? 60_000
            : rotateResult?.reason === 'recent_rotate'
              ? 35_000
              : lastReason === 'throttled'
                ? 25_000 + attempt * 8_000
                : 12_000 + attempt * 4_000;
