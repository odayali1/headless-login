...
export async function getFireproxUrlForOrigin(originLike) {
  ...
  if (!loadFireproxAwsCredentials()) {
    throw new Error(
      `No FireProx API for ${origin}. Create one on the EC2: python fire.py --command create --url ${origin}`
    );
  }
...
export function hasFireproxForOrigin(originLike) {
  try {
    return cache.has(normalizeOrigin(originLike));
  } catch {
    return false;
  }
}
