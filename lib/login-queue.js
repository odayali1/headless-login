...
/**
 * Gap between starting each Camoufox.
 * Env LOGIN_PARALLEL_STAGGER_MS wins (0 = start as fast as the pump can).
 * Default scales with parallel: fixed 8s made Parallel=100 stuck at ~7–8 active
 * (steady-state ≈ job_duration / stagger).
 */
function resolveStaggerMs(parallelMax) {
  const raw = process.env.LOGIN_PARALLEL_STAGGER_MS;
  if (raw != null && String(raw).trim() !== '') {
    const n = Number(raw);
    return Number.isFinite(n) ? Math.max(0, n) : 8_000;
  }
  const n = Math.max(1, Number(parallelMax) || 1);
  if (n >= 100) return 200;
  if (n >= 50) return 400;
  if (n >= 20) return 1_000;
  if (n >= 5) return 3_000;
  return 8_000;
}
  ...
  async function pump() {
    ...
    try {
      while (running.size < max && waiters.length) {
        ...
        const staggerMs = resolveStaggerMs(max);
        if (max > 1 && running.size >= 1 && staggerMs > 0) {
          const lastStarted = Math.max(0, ...[...running.values()].map((r) => r.startedAt || 0));
          const since = Date.now() - lastStarted;
          if (since < staggerMs) {
            const wait = staggerMs - since;
            console.log(
