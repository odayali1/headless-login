...
/** When LOGIN_PARALLEL=2, delay 2nd browser so both don't hit GetCredentialType at once. */
...
function envParallel() {
  ...
  const requested = Math.min(2, Math.max(1, Math.trunc(n)));
  // Two Camoufox logins on one mobile IP → GetCredentialType 429. Set LOGIN_PARALLEL_FORCE=1 to allow 2.
...
export function createLoginQueue({
  ...
  async function pump() {
    while (running.size < max && waiters.length) {
      ...
      if (max > 1 && running.size === 1) {
        const [first] = [...running.values()];
        const since = Date.now() - (first?.startedAt || 0);
        if (since < PARALLEL_STAGGER_MS) {
          const wait = PARALLEL_STAGGER_MS - since;
          console.log(`[queue] Staggering parallel login ${Math.round(wait / 1000)}s (avoid GetCredentialType 429)`);
          await sleep(wait);
        }
      }
