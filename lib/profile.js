...
/** Linux is case-sensitive; Windows is not. Dashboard emails are lowercased; profile files often keep original case. */
let outlookPathIndex = null;
let outlookPathIndexAt = 0;
const OUTLOOK_INDEX_TTL_MS = 120_000;

async function getOutlookProfileIndex() {
  if (outlookPathIndex && Date.now() - outlookPathIndexAt < OUTLOOK_INDEX_TTL_MS) {
    return outlookPathIndex;
  }
  await fs.mkdir(PROFILES_DIR, { recursive: true });
  const files = await fs.readdir(PROFILES_DIR);
  const map = new Map();
  const suffix = `-${CANONICAL_TARGET}.json`;
  for (const f of files) {
    if (!f.endsWith(suffix)) continue;
    const emailPart = f.slice(0, -suffix.length);
    map.set(emailPart.toLowerCase(), path.join(PROFILES_DIR, f));
  }
  outlookPathIndex = map;
  outlookPathIndexAt = Date.now();
  return map;
}

export function invalidateOutlookProfileIndex() {
  outlookPathIndex = null;
  outlookPathIndexAt = 0;
}

export async function resolveOutlookProfilePath(email) {
  const want = String(email || '').trim();
  if (!want) return null;
  const candidates = [profilePath(want), profilePath(want.toLowerCase())];
  const seen = new Set();
  ...
  const idx = await getOutlookProfileIndex();
  return idx.get(want.toLowerCase()) || idx.get(sanitize(want).toLowerCase()) || null;
}
...
export async function saveProfile(context, email, extra = {}) {
  ...
  const payload = {
    ...
    lastLoginAttemptAt: extra.lastLoginAttemptAt || now,
