...
export async function refreshAccountToken(
  ...
  try {
    ...
    } else if (wifiSplit && !skipBrowserless) {
      log('token', 'Wi-Fi Split ON — skip browserless HTTP (use Camoufox session capture)');
    } else {
      refreshTokenKnownBad = true;
    }
    ...
    if (isProxyEnabled()) {
      ...
      if (probe.ok) {
        ...
        throw new ProxyUnreachableError(
          probe.error ||
            'Proxy unreachable — with Wi-Fi Split ON use PROXY_HTTP_URL and restart the iProxy app on the phone.'
        );
      }
    }
    ...
    const tokens = await captureOutlookTokens(page, {
      ...
      skipPreflightRefresh: wifiSplit || false,
