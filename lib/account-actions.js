...
import { captureOutlookTokens, isTokenValid, tryLightweightTokenRefresh } from './token-extract.js';
...
export async function refreshAccountToken(email, target, { engine = 'auto', onProgress, jobId = 'refresh' } = {}) {
  ...
  try {
    ...
    if (refreshToken) {
      log('token', 'Trying lightweight token refresh (no Outlook load)…');
      const lightweight = await tryLightweightTokenRefresh(context, refreshToken, log);
      if (lightweight?.error === 'invalid_grant') {
        throw new Error(
          `Refresh token expired/revoked by Microsoft (${lightweight.error_description || 'invalid_grant'}). Re-login is required.`
        );
      }
      if (lightweight?.access_token && isTokenValid(lightweight)) {
        ...
        await saveProfile(context, email, {
          ...
          tokens: {
            ...lightweight,
            refresh_token: lightweight.refresh_token || refreshToken,
          },
        });
        log('success', 'Token refreshed (lightweight — no Outlook page load)');
        notifyAccountTokenUpdated(email, target, { reason: 'token_refresh' }).catch(() => {});
        return {
          status: 'success',
          email,
          target,
          accessToken: lightweight.access_token,
          tokenScope: lightweight.scope,
          tokenExpiresAt: lightweight.expires_at,
          hasToken: true,
        };
      }
      log('token', 'Lightweight refresh insufficient — loading Outlook…');
    }
