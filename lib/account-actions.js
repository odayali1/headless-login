...
export async function refreshAccountToken(
  ...
  // Hybrid: Loki uses residential — continue; only Camoufox/cookie wait for login.
  ...
  try {
    if (!skipBrowserless) {
      if (
        ...
        // During login + hybrid: Loki only (cookie hits login.live on mobile).
        const browserless = await tryAllBrowserlessRefresh(saved.state, log, {
          forceRefresh: forceBrowserlessRefresh,
          lokiOnly: loginOwnsMobile && isHybridProxyEnabled(),
        });
        if (browserless.refreshTokenKnownBad) {
          await markHttpRefreshRejected(email).catch(() => {});
        }
