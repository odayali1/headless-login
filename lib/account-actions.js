...
import { isProxyEnabled, isHybridProxyEnabled } from './settings.js';
import { invalidateAccountsCache, patchCachedAccountTokens } from './accounts.js';
...
async function finishRefresh(email, target, useEngine, jobId, fingerprint, saved, tokens, refreshToken, log, { via = 'session' } = {}) {
  ...
  await saveProfileTokens(email, merged, {
    ...
    // Loki/browserless/cache success clears a prior invalid_grant skip so HTTP keep-alive resumes.
    // Cookie SSO keeps the flag if set (dead RT) but does not set it — cookie often mints a fresh RT.
    clearHttpRefreshRejected: via === 'loki' || via === 'browserless' || via === 'cache',
  });
  patchCachedAccountTokens(email, merged);
...
export async function tryBrowserlessAccountRefresh(
  ...
  // Non-hybrid login pause. Hybrid Loki uses residential — isBrowserlessHttpBlocked() is false.
  ...
  return {
    ...
    cookieSsoCongested: !!browserless.cookieSsoCongested,
    saved,
  };
}
...
export async function refreshAccountToken(
  ...
  // Non-hybrid: Loki shares mobile IP with login → must wait.
  // Hybrid: Loki on residential + cookie SSO on mobile — continue; only Camoufox waits.
  if (!isHybridProxyEnabled() && (isLoginProxyExclusive() || isBrowserlessHttpBlocked())) {
  ...
  const refreshToken = saved.state.tokens?.refresh_token;
  const forceBrowserlessRefresh = isTokenExpiringSoon(saved.state.tokens);
  const loginOwnsMobile = isLoginProxyExclusive();
  ...
  try {
    if (!skipBrowserless) {
      if (
        ...
        // Hybrid: Loki (residential) + cookie SSO (mobile) keep running during login.
        const browserless = await tryAllBrowserlessRefresh(saved.state, log, {
          forceRefresh: forceBrowserlessRefresh,
          lokiOnly: false,
        });
        ...
        if (browserless.skippedHttp || browserless.deferredCookie) {
          const err = new Error(
            browserless.deferredCookie
              ? 'Cookie SSO deferred — mobile IP rotating'
              : 'Login in progress — HTTP refresh deferred'
          );
          err.code = 'REFRESH_DEFERRED';
          throw err;
        }
    ...
    // Never open Camoufox refresh beside a login on the mobile IP.
    if (loginOwnsMobile || isBrowserlessHttpBlocked()) {
