...
import { closeLocalProxy, probeProxyReachability } from './proxy-local.js';
...
export async function refreshAccountToken(
  ...
  try {
    ...
    if (isProxyEnabled()) {
      ...
      if (probe.ok) {
        log('proxy', `Proxy probe OK (${probe.status || 200} in ${probe.ms}ms) before Camoufox`);
      } else {
        log(
          'proxy',
          `Proxy probe failed: ${probe.error || 'unknown'} — if iProxy Wi-Fi Split is on, enable "Mobile data always on" + "Disable Wi-Fi if no internet for 5 min"`
        );
        await closeLocalProxy().catch(() => {});
      }
    }
