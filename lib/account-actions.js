...
import { closeLocalProxy, probeProxyReachability, ProxyUnreachableError, isSlowProxyProbe, isBrowserPathSplitBlocked, resolveProxyMode } from './proxy-local.js';
import { isProxyEnabled, useSessionOnlyTokenPath } from './settings.js';
...
export async function refreshAccountToken(
  ...
  let session;
  const sessionOnlyPath = useSessionOnlyTokenPath();
  ...
  try {
    ...
    let slowProxyPath = false;
    let splitBrowserBlocked = false;
    if (isProxyEnabled()) {
      await resolveProxyMode({ force: true });
      const probe = await probeProxyReachability('https://login.live.com/');
      splitBrowserBlocked = probe.splitBlocked || isBrowserPathSplitBlocked();
      if (probe.ok) {
        slowProxyPath = isSlowProxyProbe(probe.ms);
        log(
          'proxy',
          `Proxy probe OK (${probe.mode}, ${probe.status || 200} in ${probe.ms}ms) before Camoufox` +
            (slowProxyPath ? ' — slow uplink' : '')
        );
      } else if (splitBrowserBlocked) {
        log(
          'proxy',
          'Wi‑Fi Split: curl OK, browser blocked — curl refresh already tried; opening Camoufox (serial mode)…'
        );
      } else {
        throw new ProxyUnreachableError(
          probe.error ||
            'Proxy unreachable — set PROXY_HTTP_URL for mobile relay and restart the iProxy app on the phone.'
        );
      }
    }

    if (isProxyEnabled() && isMobileRelayProxy() && isBrowserPathSplitBlocked()) {
      throw new ProxyUnreachableError(
        'Wi-Fi Split: curl token refresh failed and browser path blocked. Restart iProxy on the phone or turn traffic split OFF briefly for Re-login.'
      );
    }
    ...
    const tokens = await captureOutlookTokens(page, {
      ...
      skipPreflightRefresh: sessionOnlyPath || false,
      refreshTokenKnownBad,
      slowProxyPath: slowProxyPath || splitBrowserBlocked,
