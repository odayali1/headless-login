...
/** Block every browserless HTTP call (Loki included) — login exclusive + GCT shield + rotate. */
...
export function isProxyIpSaturated() {
  // Hybrid Loki traffic uses residential — do not pause keep-alive for mobile HTTP budget.
...
async function connectProxy(log, { label, forceNewRelay = false } = {}) {
  ...
  // Residential / forceNewRelay: forceResolve so we never keep a stale mobile http-direct mode.
  const cfg = await getPlaywrightProxyConfig({
    forceNew: forceNewRelay,
    forceResolve: forceNewRelay || isResidentialProxy(),
  });
  ...
  if (cfg?.mode === 'socks-relay') {
    ...
    if (isResidentialProxy()) {
      throw new Error(
        'Residential profile resolved to HTTP CONNECT — refuse mobile PROXY_HTTP_URL fallback. Check PROXY_RESIDENTIAL_URL SOCKS.'
      );
    }
...
export function trackBrowserlessHttpOp(log, { success = false, via = '' } = {}) {
  ...
  // Hybrid: Loki/MSAL leave via residential — only cookie SSO (mobile) counts toward phone rotate.
  if (isHybridProxyEnabled()) {
    const v = String(via || '').toLowerCase();
    if (v && v !== 'cookie-pkce' && v !== 'cookie') return Promise.resolve();
  }
  const run = async () => {
...
export async function beforeAccountLogin(log) {
  ...
  if (isResidentialProxy()) {
    // Drop any cached http-direct mode left from a previous mobile session.
    const { resetProxyMode, closeLocalProxy } = await import('./proxy-local.js');
    resetProxyMode();
    ...
    if (!exitIp) {
      log?.(
        'proxy',
        'WARNING: residential SOCKS could not resolve exit IP — switch dashboard to Mobile iProxy, or fix PROXY_RESIDENTIAL_URL'
      );
      throw new Error(
        'Residential SOCKS unreachable. Switch proxy profile to Mobile iProxy in the dashboard (or set proxy_profile=mobile).'
      );
    }
    log?.(
      'proxy',
      `Using residential rotating SOCKS — exit ${exitIp} (per-request IP, no mobile changeip)`
    );
    await connectProxy(log, { label: `residential ${exitIp}`, forceNewRelay: true });
    return;
  }
...
export async function afterAccountBrowserSession(log) {
  if (!isProxyEnabled()) return;
  // Residential rotates every request — no sticky-IP batch counter / changeip.
  if (isResidentialProxy()) return;
  const rotateAfter = Number(process.env.PROXY_ROTATE_EVERY) || getRotateAfter();
...
export async function rotateProxyIp(log, { force = false, allowDuringLogin = false } = {}) {
  const run = async () => {
    if (isResidentialProxy()) {
      log?.('proxy', 'Residential proxy — skip changeip (IP rotates per connection)');
