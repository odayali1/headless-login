...
export async function beforeAccountLogin(log) {
  ...
  // Fresh mobile IP needs a short settle — rushing GetCredentialType right after changeip
  // causes "issue looking up your account" (local proof: 15s settle → first-try password).
  if (msSinceLastProxyRotate() < 25_000) {
    const settle = Number(process.env.PROXY_POST_ROTATE_SETTLE_MS || 8_000);
    if (settle > 0) {
      log?.('proxy', `Settling ${Math.round(settle / 1000)}s after IP rotate before Camoufox login…`);
      await sleep(settle);
    }
  }
