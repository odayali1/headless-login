...
/**
 * HTTP/Loki/cookie-SSO refreshes share the mobile IP but must NOT share the login
 * session counter — that made login see "IP full (8/6)" and changeip right before email.
 */
...
/** True when HTTP refresh already filled this IP — stop more HTTP (does not force login rotate). */
export function isProxyIpSaturated() {
  if (!isProxyEnabled()) return false;
  return httpOpsOnIp >= HTTP_ROTATE_EVERY;
}

/** How many browserless HTTP ops ran on the current IP (0 after rotate). */
export function getHttpOpsOnIp() {
  return httpOpsOnIp;
}
...
/**
 * After each browserless refresh account — bump HTTP-only counter and rotate when full.
 * Never bumps the login session counter. Frozen while a login holds the IP.
 */
export function trackBrowserlessHttpOp(log) {
  if (!isProxyEnabled()) return Promise.resolve();
  const run = async () => {
    if (loginProxyExclusive) {
      log?.('proxy', 'HTTP refresh skipped — login holds the IP (no changeip)');
      return;
    }
    if (httpOpsOnIp >= HTTP_ROTATE_EVERY) {
      ...
      if (!result?.rotated) {
        log?.(
          'proxy',
          `HTTP IP saturated (${httpOpsOnIp}/${HTTP_ROTATE_EVERY}) — fast lane must pause until rotate`
        );
      }
      return;
    }
    httpOpsOnIp += 1;
    if (httpOpsOnIp < HTTP_ROTATE_EVERY) return;
    log?.(
      'proxy',
      `HTTP refresh load on IP (${httpOpsOnIp}/${HTTP_ROTATE_EVERY}) — rotating mobile IP…`
    );
    const result = await rotateProxyIp(log);
    if (!result?.rotated) {
      log?.(
        'proxy',
        `HTTP rotate deferred (${result?.reason || 'busy'}) — HTTP counter stays at ${httpOpsOnIp}`
      );
    }
  };
...
/**
 * Login — NEVER changeip here unless IP was already burned by HTTP (GetCredentialType 429).
 * Rotate only AFTER a successful login batch, or after a proven GCT 429 mid-login.
 */
export async function beforeAccountLogin(log) {
  ...
  // Prior smart-refresh HTTP on this IP is what leaves GetCredentialType returning 429.
  // Cool the IP before the first Next — do not soft-reload thrash into more 429s.
  if (httpOpsOnIp > 0) {
    const cool = Number(process.env.LOGIN_PRE_GCT_COOLDOWN_MS || 30_000);
    if (cool > 0) {
      log?.(
        'proxy',
        `${httpOpsOnIp} HTTP refresh op(s) already used this IP — cooling ${Math.round(cool / 1000)}s before login GetCredentialType…`
      );
      await sleep(cool);
    }
  }
  ...
  log?.(
    'proxy',
    `Keeping current IP for login (${onIp}/${rotateAfter}, httpOps=${httpOpsOnIp}) — no changeip before email`
  );
  await connectProxy(log, { label: `account ${Math.min(onIp, Math.max(rotateAfter - 1, 0)) + 1} on this IP` });
}
