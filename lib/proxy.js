...
export async function afterLoginIpSoftBlock(log, { reason = 'ip_soft_block' } = {}) {
  ...
  if (isResidentialProxy() || isLoginResidentialIpv4() || isLoginAwsApi()) {
    // New SOCKS/FireProx connection is enough — no sticky changeip (and don't burn the mobile phone IP).
    log?.(
      'proxy',
      isLoginAwsApi()
        ? `Login soft-block (${reason}) on AWS API — next login uses a new FireProx exit`
...
export async function beforeAccountLogin(log) {
  ...
  // AWS API Gateway (FireProx) — IP rotates every request; no SOCKS / changeip.
  if (isLoginAwsApi()) {
    ...
    if (!exitIp) {
      log?.(
        'proxy',
        'WARNING: AWS API (FireProx) could not resolve exit IP — gateway may be down'
      );
      throw new Error('AWS API (FireProx) unreachable. Check FireProx gateways and retry.');
    }
    log?.('proxy', `Using AWS API (FireProx) — probe exit ${exitIp} (new AWS IP per request)`);
    return;
  }
  ...
  if (isResidentialProxy() || isLoginResidentialIpv4()) {
    // Residential Camoufox uses a dedicated per-login relay (created at launch).
    // Do not warm/close the shared mobile relay here — that races parallel logins.
    const { probeExitIp: probeIp } = await import('./proxy-exit-ip.js');
    const exitIp = await probeIp().catch(() => null);
    if (!exitIp) {
      log?.(
    ...
    log?.(
      'proxy',
      `Using residential for login — probe exit ${exitIp} (each Camoufox gets its own dedicated relay; refresh stays hybrid mobile+IPv6 when hybrid ON)`
    );
    return;
  }
...
export async function afterAccountBrowserSession(log) {
  ...
  if (isResidentialProxy() || isLoginResidentialIpv4() || isLoginAwsApi()) {
    ...
    log?.(
      'proxy',
      isLoginAwsApi()
        ? 'AWS API login — skip mobile changeip/settle (exit rotates per FireProx request)'
...
export async function rotateProxyIp(log, { force = false, allowDuringLogin = false } = {}) {
  const run = async () => {
    if (isLoginAwsApi()) {
      log?.('proxy', 'AWS API — skip changeip (IP rotates per FireProx request)');
      return { rotated: true, skipped: false, reason: 'aws_api', exitIp: null };
    }
    if (isResidentialProxy() || isLoginResidentialIpv4()) {
