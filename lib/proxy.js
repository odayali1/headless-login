...
/**
 * Browserless (Loki / MSAL / cookie SSO) must NEVER call changeip — it airplane-modes
 * the mobile modem and kills in-flight SSO. Env PROXY_HTTP_ROTATE_EVERY is ignored.
 * Camoufox login still uses PROXY_ROTATE_EVERY / GCT_429 path (with force min-gap).
 */
...
/**
 * Even force/GCT_429 rotates must not spam iProxy — double airplane = modem thrash.
 * Default 5 minutes between ANY successful changeip.
 */
const FORCE_ROTATE_MIN_GAP_MS = envNonNegInt('PROXY_FORCE_ROTATE_MIN_GAP_MS', 300_000);
...
export async function rotateProxyIp(log, { force = false, allowDuringLogin = false } = {}) {
  const run = async () => {
    ...
    // Hard floor for ALL rotates (including force/GCT_429) — stops airplane-mode thrash.
    if (lastRotateAt && ago < FORCE_ROTATE_MIN_GAP_MS) {
      const left = Math.ceil((FORCE_ROTATE_MIN_GAP_MS - ago) / 1000);
      log?.(
        'proxy',
        `Skipping rotation — modem changed IP ${Math.round(ago / 1000)}s ago (min gap ${left}s left — no re-airplane)`
      );
      return { rotated: false, skipped: true, reason: 'force_cooldown', recent: true };
    }
    ...
    try {
      ...
      // Mobile changeip already triggered airplane — do NOT call changeip again.
      // Early probe often still sees the old IP while the modem is reconnecting; a second
      // changeip = airplane → online → airplane (what users see as "stuck").
      let afterIp = await probeExitIp();
      if (beforeIp && afterIp && beforeIp === afterIp) {
        const extra = Math.max(20_000, waitMs);
        log?.(
          'proxy',
          `Exit IP still ${afterIp} — waiting ${Math.round(extra / 1000)}s for modem (no second changeip)`
        );
        await sleep(extra);
        await closeLocalProxy();
      ...
      if (beforeIp && afterIp && beforeIp === afterIp) {
        log?.(
          'proxy',
          `Rotate inconclusive — still seeing ${afterIp} (modem may still be switching). Not re-hitting changeip.`
        );
        // Treat as done enough to avoid thrash — mark rotate time so force gap applies.
        lastRotateAt = Date.now();
        resetIpCounters();
        return { rotated: true, skipped: false, reason: 'assumed_ok', exitIp: afterIp };
      }
