...
async function connectProxy(log, { label, forceNewRelay = false } = {}) {
  ...
  if (cfg?.mode === 'socks-relay') {
    ...
    if (forceIpv4) {
      throw new Error(
        'Residential IPv4 login resolved to HTTP CONNECT — refuse mobile PROXY_HTTP_URL fallback.'
      );
    }
...
export async function beforeAccountLogin(log) {
  ...
  if (isResidentialProxy() || isLoginResidentialIpv4()) {
    ...
    if (!exitIp) {
      log?.(
        'proxy',
        'WARNING: residential SOCKS could not resolve exit IP — switch dashboard to a mobile proxy, or fix residential IPv4'
      );
      throw new Error(
        'Residential IPv4 SOCKS unreachable. Switch login proxy to Huawei old / New Samsung in the dashboard.'
      );
    }
    log?.(
      'proxy',
      `Using residential IPv4 for login — exit ${exitIp} (refresh stays hybrid mobile+IPv6 when hybrid ON)`
    );
