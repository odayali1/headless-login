...
export async function afterLoginIpSoftBlock(log, { reason = 'ip_soft_block' } = {}) {
  ...
  if (isResidentialProxy()) {
    // New SOCKS connection is enough — no sticky changeip.
    log?.(
      'proxy',
      `Login soft-block (${reason}) on residential — next login will reconnect for a new exit`
    );
    setAccountsOnIp(0);
  ...
  if (result?.rotated && result?.exitIp) {
    ...
    await sleep(settle);
    return { ...result, beforeIp, settledMs: settle };
  }
