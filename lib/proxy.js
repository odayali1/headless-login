...
export async function beforeAccountLogin(log) {
  ...
  log?.('proxy', `Using ${proxy.protocol || 'socks5'} proxy ${proxy.server} (account ${onIp + 1} on this IP).`);
  log?.('proxy', `Local relay ${relay} → Firefox (auth handled in-process).`);
}
...
export async function beforeAccountRefresh(log) {
  ...
  const proxy = getPlaywrightProxy();
  const relay = await getLocalProxyForBrowser({ forceNew: true });
  log?.('proxy', `Using ${proxy.protocol || 'socks5'} proxy ${proxy.server} (token refresh).`);
  log?.('proxy', `Local relay ${relay} → Firefox (auth handled in-process).`);
}
