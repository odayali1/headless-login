...
/** Public exit IP via upstream proxy (curl). Used to verify changeip / feed Camoufox geoip. */
...
export async function beforeAccountLogin(log) {
  ...
  if (isResidentialProxy()) {
    ...
    log?.('proxy', 'Using residential rotating SOCKS (per-request IP) — no mobile changeip');
    await connectProxy(log, { label: 'residential rotating', forceNewRelay: true });
    return;
  }
