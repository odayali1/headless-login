...
export async function rotateProxyIp(log, { force = false } = {}) {
  const run = async () => {
    ...
    try {
      ...
      while (getLocalProxyRelayPinCount() > 0 && Date.now() - pinWaitStart < ROTATE_DRAIN_MS) {
        await sleep(400);
      }
      ...
      if (!quiet || pins > 0) {
        log?.(
          'proxy',
          `Camoufox still busy after ${Math.round(ROTATE_DRAIN_MS / 1000)}s (pool quiet=${quiet}, relay pins=${pins}) — delaying rotate`
        );
