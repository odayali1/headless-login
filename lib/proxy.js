...
/**
 * Login — keep the current IP. Only rotate between batches after PROXY_ROTATE_EVERY
 * successful logins. Never changeip mid-login / on email retry / because of HTTP load.
 */
export async function beforeAccountLogin(log) {
  ...
  // Optional escape hatch only (default off).
  if (LOGIN_CLEAN_IP_AFTER_HTTP > 0 && httpOpsOnIp >= LOGIN_CLEAN_IP_AFTER_HTTP) {
    log?.(
      'proxy',
      `${httpOpsOnIp} HTTP op(s) on IP — rotating before login (PROXY_LOGIN_CLEAN_IP_AFTER_HTTP=${LOGIN_CLEAN_IP_AFTER_HTTP})…`
    );
    await rotateProxyIp(log, { force: true, allowDuringLogin: true });
  }

  const rotateAfter = Number(process.env.PROXY_ROTATE_EVERY) || getRotateAfter();
  const before = getAccountsOnIp();
  const onIp = await maybeRotateBeforeBrowserSession(log, { allowDuringLogin: true });
  if (onIp === 0 && before >= rotateAfter && msSinceLastProxyRotate() < 25_000) {
    const settle = Number(process.env.PROXY_POST_ROTATE_SETTLE_MS || 8_000);
    if (settle > 0) {
      log?.('proxy', `Settling ${Math.round(settle / 1000)}s after batch IP rotate…`);
      await sleep(settle);
    }
  } else {
    log?.(
