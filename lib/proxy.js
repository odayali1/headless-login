...
/** Rotate before Camoufox login if this many ops already ran on the current IP (default 1 = any use). */
const LOGIN_CLEAN_IP_AFTER_HTTP = Number(process.env.PROXY_LOGIN_CLEAN_IP_AFTER_HTTP || 1);
...
export function trackBrowserlessHttpOp(log) {
  ...
  const run = async () => {
    // Already at limit and last rotate attempt was skipped — do not keep climbing 8→21 on a hot IP.
    if (isProxyIpSaturated()) {
      const result = await rotateProxyIp(log);
      if (!result?.rotated) {
        log?.(
          'proxy',
          `HTTP IP saturated (${getAccountsOnIp()}/${HTTP_ROTATE_EVERY}) — fast lane must pause until rotate`
        );
      }
      return;
    }
...
/** Login — always take a clean IP if this one was used for HTTP refresh or prior sessions. */
export async function beforeAccountLogin(log) {
  ...
  // Logs: "account 4 on this IP" after smart-refresh → immediate GetCredentialType 429.
  // Default threshold 1: any prior use of this IP → rotate before login.
  if (httpOnIp >= LOGIN_CLEAN_IP_AFTER_HTTP) {
    log?.(
      'proxy',
      `${httpOnIp} op(s) on current IP — rotating before login (GetCredentialType needs clean IP)…`
    );
