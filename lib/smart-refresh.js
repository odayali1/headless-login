...
const BROWSERLESS_PARALLEL = Math.min(
  5,
  Math.max(1, Number(process.env.SMART_REFRESH_BROWSERLESS_PARALLEL || (isProxyEnabled() ? 2 : 3)))
);
...
async function runCamoufoxLane(accounts) {
  ...
  if (isLoginQueueBusy()) {
    logFn(
      `[smart-refresh] Camoufox deferred — login queue busy (${accounts.length} account(s) need slow lane later)`
    );
    return;
  }
