...
async function tick() {
  ...
  try {
    ...
    // Login GetCredentialType + 5× parallel token HTTP on one mobile IP → 429. Yield while user logins run.
    const loginBusy = isLoginQueueBusy();
    const proxyRotating = isProxyRotating();
    let browserlessResults = [];
    if (loginBusy || proxyRotating) {
      if (loginBusy) {
        logFn('[smart-refresh] Fast lane paused — login queue busy (GetCredentialType needs clean IP)');
      }
      if (proxyRotating) {
        logFn('[smart-refresh] Fast lane paused — mobile IP rotating');
      }
    } else {
      // Cookie PKCE SSO works even when stored RT is dead — include Camoufox-only in browserless batch.
      const browserlessBatch = [...httpLane, ...camoufoxOnly].slice(0, TICK_BATCH_SIZE);
      if (browserlessBatch.length) {
        browserlessResults = await runBrowserlessLane(browserlessBatch);
      }
    }
