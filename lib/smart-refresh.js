...
/**
 * HTTP/Loki + cookie PKCE share the mobile IP with Camoufox login.
...
/** Unused while login runs — fast lane hard-pauses (skipLane). Kept for status/API. */
const BROWSERLESS_PARALLEL_BUSY = envInt('SMART_REFRESH_BROWSERLESS_PARALLEL_BUSY', 1, { min: 1, max: 8 });
...
const TICK_BATCH_SIZE_BUSY = envInt('SMART_REFRESH_TICK_BATCH_SIZE_BUSY', 1, {
  min: 1,
  max: 8,
});
...
/**
 * Drain in-flight HTTP before Camoufox login. Proven: Loki/cookie SSO on the same
 * mobile IP causes GetCredentialType 429 ("issue looking up your account").
 */
export async function waitForSmartRefreshHttpQuiet(log, timeoutMs = 90_000) {
  if (inFlight.size === 0) return;
  log?.(
    'proxy',
    `Waiting for ${inFlight.size} in-flight HTTP refresh(es) to finish before login (protects GetCredentialType)…`
  );
...
/**
 * Fast lane MUST hard-pause during login / post-login cooldown.
 * User proof: same accounts login when smart-refresh is off; gct=429 when HTTP runs beside login.
 */
export function resolveFastLaneLimits() {
  ...
  if (loginExclusive || loginRunning || loginQueued) {
    return {
      ...
      batch: 0,
      throttled: true,
      reason: loginExclusive || loginRunning ? 'login in progress (HTTP paused — protects gct)' : 'login queue waiting',
      skipLane: true,
    };
  }
  if (proxyRotating) {
...
async function tick() {
  ...
  try {
    ...
    const browserlessBatch = limitsAfterRotate.skipLane
      ? []
      : [...httpLane, ...camoufoxOnly].slice(0, limitsAfterRotate.batch);
