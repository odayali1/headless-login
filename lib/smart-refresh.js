...
async function runCamoufoxLane(accounts) {
  ...
  if (isLoginQueueBusy()) {
    ...
    logFn(
      `[smart-refresh] Camoufox deferred — login queue busy (${accounts.length} account(s) need slow lane; ${priorityBlocked} priority). Pause or Cancel queued under Login queue to free Camoufox.`
    );
    return;
  }
