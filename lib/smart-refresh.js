...
/** Fast lane limits — full speed when idle, throttled during login/IP rotate. */
export function resolveFastLaneLimits() {
  const loginBusy = isLoginQueueBusy();
  const proxyRotating = isProxyRotating();
  const throttled = loginBusy || proxyRotating;
  let reason = null;
  if (loginBusy) reason = 'login queue busy';
  else if (proxyRotating) reason = 'IP rotating';
  return {
    parallel: throttled ? BROWSERLESS_PARALLEL_BUSY : BROWSERLESS_PARALLEL,
    batch: throttled ? TICK_BATCH_SIZE_BUSY : TICK_BATCH_SIZE,
    throttled,
    reason,
  };
}
