...
/**
 * Fast lane during login: Loki RT redeem only (keep-alive for bulk day-long queues).
 * Cookie SSO (login.live.com) + Camoufox slow lane stay deferred — those burn GCT.
 * Full skip only for live-shield (email Next) / rotate / IP saturated.
 */
export function resolveFastLaneLimits() {
  ...
  if (liveShield || proxyRotating) {
    return {
      parallel: 0,
      batch: 0,
      throttled: true,
      reason: liveShield ? 'GetCredentialType shield (brief)' : 'IP rotating',
      skipLane: true,
      lokiOnly: false,
    };
  }
  ...
  if (loginBusy) {
    return {
      ...
      throttled: true,
      reason: 'login active — Loki-only keep-alive (cookie SSO paused)',
      skipLane: false,
      lokiOnly: true,
    };
  }
