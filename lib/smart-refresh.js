...
import { beforeAccountRefresh, isProxyRotating, isProxyIpSaturated, isLoginProxyExclusive, trackBrowserlessHttpOp } from './proxy.js';
...
/** Block until in-flight HTTP refreshes finish (login exclusive already set). */
export async function waitForSmartRefreshHttpQuiet(log, timeoutMs = 90_000) {
  if (inFlight.size === 0) return;
  log?.(
    'proxy',
    `Waiting for ${inFlight.size} in-flight HTTP refresh(es) to finish before login…`
  );
  ...
  if (inFlight.size > 0) {
    log?.(
      'proxy',
      `WARNING: ${inFlight.size} HTTP refresh(es) still running — login continuing on shared IP`
    );
  }
}
...
export function resolveFastLaneLimits() {
  ...
  if (loginExclusive || loginRunning || loginQueued) {
    return {
      parallel: 0,
      batch: 0,
      throttled: true,
      reason: loginExclusive || loginRunning ? 'login in progress' : 'login queue waiting',
      skipLane: true,
    };
  }
  if (proxyRotating) {
...
async function runCamoufoxRefresh(acc) {
  const key = accountKey(acc);
  // Login holds the mobile IP + SOCKS pins — never open a second Camoufox beside it.
  ...
  try {
    await withCamoufoxSlot(async () => {
      if (isLoginProxyExclusive() || isLoginQueueBusy()) {
        throw new Error('Login in progress — Camoufox refresh deferred');
      }
...
async function runCamoufoxLane(accounts) {
  ...
  // CRITICAL: do NOT run Camoufox beside login — logs showed pin/rotate deadlock + lookup failures.
  if (isLoginProxyExclusive() || isLoginQueueBusy()) {
    camoufoxWaiting.length = 0;
    camoufoxWaitingKeys.clear();
    if (accounts.length) {
      logFn(
        `[smart-refresh] Slow lane deferred (login in progress) — ${accounts.length} dead-RT waiting`
      );
    }
    return;
  }
...
async function tick() {
  ...
  try {
    ...
    // When HTTP filled the IP but cooldown blocked rotate, retry rotate ONLY if no login/queue.
    if (
      limits.reason === 'HTTP IP saturated (awaiting rotate)' &&
      isProxyIpSaturated() &&
      !isProxyRotating() &&
      ...
      if ((q.running || 0) > 0 || ((q.waiting || 0) > 0 && !q.paused)) {
        // Login about to use this IP — do not changeip.
      } else {
        await trackBrowserlessHttpOp((step, message) => {
          if (message) logFn(`[smart-refresh] [${step}] ${message}`);
        }).catch(() => {});
    ...
    if (needsCamoufox.length) {
      if (limits.skipLane || isLoginProxyExclusive() || isLoginQueueBusy()) {
        logFn(
          `[smart-refresh] Slow lane deferred (${limits.reason || 'login'}) — ${needsCamoufox.length} dead-RT waiting`
        );
      } else {
