...
/** Shared login/Camoufox queue — keeps smart-refresh from overlapping manual refresh/login. */
let enqueueLoginJob = null;
...
export function initSmartRefresh({
  ...
  enqueueLogin: enqueueFn,
} = {}) {
  if (log) logFn = log;
  onRefreshed = cb || null;
  isLoginQueueBusy = queueBusyFn || (() => false);
  enqueueLoginJob = enqueueFn || null;
  stopSmartRefresh();
...
async function runBrowserlessLane(accounts) {
  ...
  const results = await runParallel(accounts, BROWSERLESS_PARALLEL, async (acc) => {
    ...
    try {
      ...
      if (result.refreshTokenKnownBad || result.skippedHttp) skippedRejected++;
  ...
  if (skippedRejected) {
    logFn(
      `[smart-refresh] Fast lane: ${skippedRejected} need Camoufox (refresh token previously rejected)`
    );
  }
  if (skippedValid) {
...
async function runCamoufoxRefresh(acc) {
  ...
  if (enqueueLoginJob) {
    try {
      return await enqueueLoginJob(work, {
        jobId: `smart-${Date.now()}`,
...
async function runCamoufoxLane(accounts) {
  ...
  if (isLoginQueueBusy()) {
    ...
    logFn(
      `[smart-refresh] Camoufox deferred — login queue busy (${accounts.length} need slow lane; ${priorityBlocked} priority). Pause or Cancel queued to free Camoufox.`
    );
    return;
  }
  ...
  logFn(
    `[smart-refresh] Slow lane: queueing ${batch.length} Camoufox job(s) (shared login queue, max ${CAMOUFOX_PARALLEL})`
  );
...
async function tick() {
  ...
  try {
    ...
    if (priorityCount) {
      logFn(`[smart-refresh] Priority: ${priorityCount} account(s) expire within ${Math.round(PRIORITY_REFRESH_WINDOW_MS / 60_000)}m`);
    }
    ...
    const needsCamoufox = browserlessResults.filter((r) => r.needsCamoufox).map((r) => r.acc);
