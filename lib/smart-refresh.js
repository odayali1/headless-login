...
import { beforeAccountRefresh, isProxyRotating, trackBrowserlessHttpOp } from './proxy.js';
...
export function resolveFastLaneLimits() {
  ...
  if (proxyRotating) {
    // HTTP refresh during changeip wait burns the NEW exit IP before GetCredentialType retry.
    return {
      parallel: 0,
      batch: 0,
      throttled: true,
  ...
  if (loginRunning) {
    // Cookie SSO + GetCredentialType share Microsoft IP limits — defer HTTP until login finishes.
    return {
      parallel: 0,
      batch: 0,
      throttled: true,
