...
async function tick() {
  ...
  try {
    ...
    if (needsCamoufox.length) {
      if (limitsAfterRotate.skipLane || isLoginProxyExclusive() || isLoginQueueBusy()) {
        logFn(
          `[smart-refresh] Slow lane deferred (${limitsAfterRotate.reason || 'login'}) — ${needsCamoufox.length} dead-RT waiting`
        );
      } else {
