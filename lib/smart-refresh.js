...
/** During Camoufox login — keep Loki/cookie SSO running, just quieter. */
...
/**
 * Brief settle before login — HTTP keeps running during login (throttled), so do not wait 90s.
 */
export async function waitForSmartRefreshHttpQuiet(log, timeoutMs = 8_000) {
  if (inFlight.size === 0) return;
  const waitMs = Math.min(timeoutMs, 8_000);
  log?.(
    'proxy',
    `Settling ${waitMs / 1000}s — ${inFlight.size} HTTP refresh(es) in flight (login + smart-refresh share IP)`
  );
...
/**
 * Fast lane (Loki / MSAL / cookie SSO) keeps running during login — throttled only.
 * Camoufox slow lane stays deferred while login holds the browser/IP.
 */
export function resolveFastLaneLimits() {
  ...
  // Only hard-stop HTTP while changeip is mid-flight (kills cookie SSO sockets).
  if (proxyRotating) {
    return {
      parallel: 0,
      batch: 0,
  ...
  if (loginBusy) {
    return {
      ...
      throttled: true,
      reason: 'login active — HTTP throttled (Camoufox paused)',
      skipLane: false,
    };
  }
  return {
...
async function tick() {
  ...
  try {
    ...
    // Browserless never changeip (kills mobile modem). Cookie PKCE stays on current IP.
