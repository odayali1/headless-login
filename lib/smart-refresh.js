...
/**
 * Idle: Loki + cookie PKCE. Hybrid login: Loki residential + cookie mobile (both keep running).
 * Non-hybrid login: pause HTTP. Cookie SSO is not deferred for login jobs.
 * Cap 24: env 32–64 floods login.live.com, 35s timeouts, and the due pile grows.
 */
const BROWSERLESS_PARALLEL_MAX = 24;
const requestedBrowserlessParallel = Number(process.env.SMART_REFRESH_BROWSERLESS_PARALLEL);
const BROWSERLESS_PARALLEL = envInt(
  'SMART_REFRESH_BROWSERLESS_PARALLEL',
  isProxyEnabled() ? 8 : 12,
  { min: 1, max: BROWSERLESS_PARALLEL_MAX }
);
if (Number.isFinite(requestedBrowserlessParallel) && requestedBrowserlessParallel > BROWSERLESS_PARALLEL) {
  console.log(
    `[smart-refresh] Clamping SMART_REFRESH_BROWSERLESS_PARALLEL ${requestedBrowserlessParallel} → ${BROWSERLESS_PARALLEL} (wide bursts timeout and grow needs-refresh)`
  );
}
/** During Camoufox login — keep-alive via Loki RT only (no hard pause for day-long bulk). */
const BROWSERLESS_PARALLEL_BUSY = envInt('SMART_REFRESH_BROWSERLESS_PARALLEL_BUSY', 4, { min: 1, max: 16 });
...
const TICK_BATCH_SIZE = envInt('SMART_REFRESH_TICK_BATCH_SIZE', Math.max(8, BROWSERLESS_PARALLEL * 2), {
  min: BROWSERLESS_PARALLEL,
  max: 48,
});
const TICK_BATCH_SIZE_BUSY = envInt('SMART_REFRESH_TICK_BATCH_SIZE_BUSY', Math.max(4, BROWSERLESS_PARALLEL_BUSY * 2), {
  min: 1,
  max: 64,
});
/** Fail one Fast-lane account if Loki+cookie SSO hang (dead socket). Does not tear down relays. */
const BROWSERLESS_ACCOUNT_TIMEOUT_MS = envInt('SMART_REFRESH_ACCOUNT_TIMEOUT_MS', 120_000, {
  min: 30_000,
  max: 300_000,
});
const TICK_WATCHDOG_MS = envInt('SMART_REFRESH_TICK_WATCHDOG_MS', 180_000, {
  min: 60_000,
  max: 900_000,
});
...
export function getSmartRefreshStatus() {
  ...
  return {
    ...
    tickRunning,
    tickAgeSec: tickRunning && tickStartedAt ? Math.round((Date.now() - tickStartedAt) / 1000) : 0,
 ...
 * Hybrid ON: no-op — Loki is on residential and must not stall mobile login.
 * Hybrid OFF: brief quiet so Loki on the same mobile IP does not cause gct=429.
 */
export async function waitForSmartRefreshHttpQuiet(log, timeoutMs = 45_000) {
  if (isHybridProxyEnabled()) return;
 ...
 * - Hybrid ON: Loki on residential + cookie SSO on mobile — keep both running (no pause)
...
export function resolveFastLaneLimits() {
  ...
  // Hybrid: keep Loki (residential) + cookie SSO (mobile) during login / GCT.
  // Mid-changeip: skip the lane briefly (cookie SSO also blocked while rotating).
  if (hybrid && proxyRotating) {
    return {
      parallel: 0,
      batch: 0,
      throttled: true,
  ...
  if (hybrid && (loginBusy || liveShield)) {
    return {
      parallel: BROWSERLESS_PARALLEL,
      batch: TICK_BATCH_SIZE,
      throttled: true,
      reason: loginBusy
        ? 'login — hybrid Loki residential + cookie mobile'
        : 'GCT shield — hybrid Loki residential + cookie mobile',
      skipLane: false,
      lokiOnly: false,
    };
  }
...
/** Cookie SSO congestion: retry Fast lane soon instead of 10min fail-backoff + Camoufox. */
function markFastLaneRetrySoon(email, retryInMs = 60_000) {
  lastFailAt.set(email, Date.now() - FAIL_BACKOFF_MS + Math.max(15_000, retryInMs));
}

function withAccountTimeout(promise, ms) {
  let timer;
  return new Promise((resolve, reject) => {
    timer = setTimeout(
      () => reject(new Error(`browserless refresh timed out after ${Math.round(ms / 1000)}s`)),
      ms
    );
    promise.then(
      (v) => {
  ...
  const results = await runParallel(accounts, parallel, async (acc) => {
    ...
    try {
      const result = await withAccountTimeout(
        tryBrowserlessAccountRefresh(acc.email, acc.target, {
        ...
        BROWSERLESS_ACCOUNT_TIMEOUT_MS
      );
      if (result.fromCache) {
        skippedValid++;
      ...
      // Timeouts / hang-ups are congestion, not dead cookies. Camoufox would grow a 100+ queue.
      if (result.cookieSsoCongested) {
        markFastLaneRetrySoon(acc.email);
        logFn(`[smart-refresh] Defer ${acc.email} (cookie SSO congested) — retry Fast lane, skip Camoufox`);
        return { acc, ok: false, deferred: true };
      }
      return { acc, ok: false, needsCamoufox: true };
    } catch (err) {
      logFn(`[smart-refresh] FAIL ${acc.email} (browserless): ${err.message}`);
      if (/timed out|socket hang up|ECONNRESET|ECONNABORTED/i.test(err.message || '')) {
        markFastLaneRetrySoon(acc.email);
        return { acc, ok: false, deferred: true };
      }
      return { acc, ok: false, needsCamoufox: true };
...
function drainCamoufoxPool() {
  ...
  const pool = getCamoufoxPoolStatus();
  const slots = Math.max(0, pool.max - pool.running - pool.waiting);
  const started = [];
  let launched = 0;
  while (camoufoxWaiting.length && launched < slots) {
  ...
  if (started.length) {
    ...
    logFn(
      `[smart-refresh] Slow lane: queued ${started.length} Camoufox (${after.running}/${after.max} running, ${after.waiting} in pool, ${camoufoxWaiting.length} waiting)`
    );
  }
}
...
async function tick() {
  ...
  if (tickRunning) {
    if (tickStartedAt && Date.now() - tickStartedAt > TICK_WATCHDOG_MS) {
      logFn(
        `[smart-refresh] Watchdog: tick stuck ${Math.round((Date.now() - tickStartedAt) / 1000)}s — unlocking so refresh can continue`
      );
      tickRunning = false;
    } else {
      return;
