...
import { beforeAccountRefresh, isProxyRotating } from './proxy.js';
...
/** HTTP/Loki + cookie PKCE. Cap 5 — 10 parallel SSO contexts + Camoufox OOMed the host. */
const BROWSERLESS_PARALLEL = envInt('SMART_REFRESH_BROWSERLESS_PARALLEL', 5, { min: 1, max: 8 });
...
/** During active Camoufox login — GetCredentialType is fragile; keep HTTP minimal not zero. */
const BROWSERLESS_PARALLEL_LOGIN = envInt('SMART_REFRESH_BROWSERLESS_PARALLEL_LOGIN', 1, { min: 1, max: 4 });
...
const TICK_BATCH_SIZE_LOGIN = envInt('SMART_REFRESH_TICK_BATCH_SIZE_LOGIN', 2, {
  min: 1,
  max: 10,
});
...
export function getSmartRefreshStatus() {
  ...
  return {
    ...
    browserlessParallelLogin: BROWSERLESS_PARALLEL_LOGIN,
...
/** Fast lane limits — full speed when idle; quieter when login queued; minimal during active login. */
export function resolveFastLaneLimits() {
  ...
  if (proxyRotating) {
    return {
      parallel: BROWSERLESS_PARALLEL_BUSY,
      batch: TICK_BATCH_SIZE_BUSY,
      throttled: true,
      reason: 'IP rotating',
    };
  }
  if (loginRunning) {
    return {
      parallel: BROWSERLESS_PARALLEL_LOGIN,
      batch: TICK_BATCH_SIZE_LOGIN,
      throttled: true,
...
async function tick() {
  ...
  try {
    ...
    const browserlessBatch = [...httpLane, ...camoufoxOnly].slice(0, limits.batch);
