...
const FAIL_BACKOFF_MS = Number(process.env.SMART_REFRESH_FAIL_BACKOFF_MS || 15 * 60 * 1000);
...
const BROWSERLESS_PARALLEL = Math.min(
  5,
  Math.max(1, Number(process.env.SMART_REFRESH_BROWSERLESS_PARALLEL || 4))
);
const CAMOUFOX_PARALLEL = Math.min(
  4,
  Math.max(1, Number(process.env.SMART_REFRESH_CAMOUFOX_PARALLEL || 2))
);
/** Max Camoufox refresh jobs started per tick (capped by CAMOUFOX_PARALLEL and free slots). */
...
async function tick() {
  ...
  try {
    ...
    const browserlessBatch = eligible.slice(0, BROWSERLESS_PARALLEL);
