...
/**
 * Idle: cookie PKCE first (Loki misses at this scale). Hybrid login: Loki residential + cookie mobile.
 * Cap 48: 64-wide mixed Loki+SSO timed out; cookie-first at 32–48 SSO succeeds in ~1s.
 */
const BROWSERLESS_PARALLEL_MAX = 48;
...
if (Number.isFinite(requestedBrowserlessParallel) && requestedBrowserlessParallel > BROWSERLESS_PARALLEL) {
  console.log(
    `[smart-refresh] Clamping SMART_REFRESH_BROWSERLESS_PARALLEL ${requestedBrowserlessParallel} → ${BROWSERLESS_PARALLEL}`
  );
}
...
const TICK_BATCH_SIZE = envInt('SMART_REFRESH_TICK_BATCH_SIZE', Math.max(16, BROWSERLESS_PARALLEL * 2), {
