 ...
 *  If Coolify sets SMART_REFRESH_CAMOUFOX_PARALLEL=1, recovery stays too slow — prefer unset or 2. */
...
async function runCamoufoxLane(accounts) {
  ...
  // IMPORTANT: do NOT freeze Camoufox when the user has a long bulk queue.
  // That caused 12h of "Camoufox deferred — login queue busy" with ~0 recovery while
  // Needs refresh climbed (dead-RT accounts only clear via Camoufox).
  if (isLoginQueueBusy()) {
    logFn(
      `[smart-refresh] User login queue busy — still running Camoufox recovery pool (${accounts.length} dead-RT; max ${CAMOUFOX_PARALLEL} parallel)`
    );
  }
  ...
  if (!batch.length) {
    if (accounts.length > 0) {
      logFn(
        `[smart-refresh] Camoufox pool full (${camoufoxRunning.size}/${CAMOUFOX_PARALLEL}) — ${accounts.length} waiting`
      );
    }
    return;
  }
