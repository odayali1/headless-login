...
async function loadOrCreateLaunchOptions(email, target, fingerprint, savedState, forceFresh) {
  ...
  if (!forceFresh) {
    try {
      const cached = stripProxyFromOptions(JSON.parse(await fs.readFile(optsFile, 'utf8')));
  ...
  const fromOptions = stripProxyFromOptions(await buildCamoufoxLaunchOptions(config));
...
/** Remove broken or stale proxy entries from saved launch-options.json files. */
export async function repairAllLaunchOptions() {
  ...
  if (fixed) console.log(`[proxy] Stripped stale proxy from ${fixed} launch-options.json file(s)`);
}
