...
export async function launchCamoufoxSession({
  ...
  const { isLoginResidentialIpv4 } = await import('./settings.js');
  let dedicatedProxy = null;
  let useSharedPin = false;
  if (isProxyEnabled() && isLoginResidentialIpv4()) {
    const { createDedicatedLoginProxy } = await import('./proxy-local.js');
    dedicatedProxy = await createDedicatedLoginProxy();
    if (dedicatedProxy?.mode === 'http-direct') {
      launchOpts.proxy = {
        server: dedicatedProxy.server,
        username: dedicatedProxy.username,
        password: dedicatedProxy.password,
      };
    } else if (dedicatedProxy?.server) {
