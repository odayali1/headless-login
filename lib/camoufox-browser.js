...
function baseCamoufoxConfig(email, fingerprint, savedState, { forGeoip = false } = {}) {
  ...
  // Phone mimic: mobile-sized window + touch points. Camoufox has no android/ios OS — keep windows.
  // See https://camoufox.com/python/usage/ and daijro/camoufox#50 (mobile fingerprints not fully supported).
  if (fp.mimicPhone) {
    config.config = {
      'navigator.maxTouchPoints': Number(fp.maxTouchPoints) > 0 ? Number(fp.maxTouchPoints) : 5,
    };
    // Intentional navigator override for touch — silence Camoufox leak warning.
