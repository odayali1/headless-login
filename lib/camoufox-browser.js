...
/**
 * Camoufox docs: os = "windows" | "macos" | "linux" (or a list). Default is all three.
 * Sticky per email so the same mailbox keeps one desktop OS.
 * Prefer windows/macos only — real Firefox desktop share; skip linux (Coolify host is linux,
 * and mass linux fingerprints cluster). Mobile window sizes stay opt-in via mimicPhone
 * (Camoufox has no android/ios — docs warn mobile CSS on desktop OS is inconsistent).
 */
function pickAccountOs(email = '', preferred = null) {
  if (preferred === 'macos' || preferred === 'windows') return preferred;
  const hash = crypto.createHash('sha256').update(String(email || '').toLowerCase().trim()).digest();
  return hash[0] % 5 === 0 ? 'macos' : 'windows';
}

/** Bump when Camoufox device policy changes (forces one-time BrowserForge rebuild). */
const DEVICE_POLICY_VERSION = 6;
...
function baseCamoufoxConfig(email, fingerprint, savedState, { forGeoip = false, os } = {}) {
  ...
  const config = {
    headless: true,
    // Camoufox docs: true or max cursor-move seconds. Pre-Jul14 used true; longer helps MSA clicks.
    humanize: Number(process.env.CAMOUFOX_HUMANIZE || 1.5) || 1.5,
    os: accountOs,
    // Pre-Jul14 working bulk path: sticky per-account window from email fingerprint.
    // Not one global size — each mailbox keeps its own WxH (BrowserForge still fills the rest).
