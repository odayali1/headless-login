...
function platformOs(savedState) {
  ...
  // Camoufox os is desktop-only (windows/macos/linux); phone mode uses window + touch, not android.
...
async function writeCachedDeviceMode(email, target, mimicPhone) {
  ...
  await fs.writeFile(file, JSON.stringify({ mimicPhone: !!mimicPhone }, null, 2));
}
...
function baseCamoufoxConfig(email, fingerprint, savedState, { forGeoip = false } = {}) {
  ...
  const config = {
    ...
    // Camoufox applies window size onto a unique BrowserForge fingerprint (per launch-options cache).
    window: [fp.viewport.width, fp.viewport.height],
    // Coolify host IP must never leak via WebRTC while traffic exits mobile proxy — MS soft-flags that.
    block_webrtc: true,
    enable_cache: true,
  };
  // Phone mimic: mobile-sized window only. Camoufox has no android/ios OS — keep windows.
  // Do NOT force navigator.maxTouchPoints (inconsistent with desktop Firefox → more MS GCT 429s).
  // See https://camoufox.com/python/usage/ and daijro/camoufox#50.
...
async function loadOrCreateLaunchOptions(email, target, fingerprint, savedState, regenerateFingerprint) {
  ...
  // Phone/desktop mode change must rebuild launch-options (cached device would keep old window/touch).
  const cachedMode = await readCachedDeviceMode(email, target);
  let mustRebuild = regenerateFingerprint === true;
  if (cachedMode === null) {
    // Pre-feature caches have no sidecar — rebuild when phone mode is requested.
    if (wantPhone) mustRebuild = true;
  } else if (cachedMode !== wantPhone) {
    mustRebuild = true;
  }
  ...
  if (!mustRebuild) {
    try {
      ...
      if (!(await isLaunchOptionsStale(cached))) {
        ...
        if (cachedMode === null) await writeCachedDeviceMode(email, target, wantPhone);
 ...
 * @param {boolean} [opts.mimicPhone] - mobile-sized Camoufox window + touch; omit to keep profile setting
 */
export async function launchCamoufoxSession({
  ...
  // Always enforce WebRTC block even on cached launch-options from older builds.
  if (launchOpts.firefoxUserPrefs && typeof launchOpts.firefoxUserPrefs === 'object') {
    launchOpts.firefoxUserPrefs['media.peerconnection.enabled'] = false;
  }
  ...
  if (resolvedFp.mimicPhone) {
    console.log(
      `[camoufox] phone mimic for ${email}: ${resolvedFp.viewport.width}x${resolvedFp.viewport.height} touch=${resolvedFp.maxTouchPoints || 5}`
    );
  }
