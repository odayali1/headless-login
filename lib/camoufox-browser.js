...
/**
 * Last resort when geoip cannot spoof WebRTC to the proxy exit:
 * block WebRTC entirely so Coolify/server public IP never appears in ICE.
 * Never apply a fake US timezone here — that mismatches MENA mobile exits and clusters accounts.
 */
function applyNoLeakWebRtcFallback(launchOpts, reason) {
  launchOpts.firefoxUserPrefs = {
    ...(launchOpts.firefoxUserPrefs || {}),
    'media.peerconnection.enabled': false,
  };
  console.warn(`[camoufox] ${reason} — WebRTC blocked (no server-IP leak). Do not use US locale fallback.`);
}
...
async function applyGeoipToLaunchOptions(launchOpts, email, fingerprint, savedState) {
  if (!isProxyEnabled() || !launchOpts.proxy?.server) {
    return false;
  }
  if (!isCamoufoxGeoipEnabled()) {
    applyNoLeakWebRtcFallback(launchOpts, 'CAMOUFOX_GEOIP disabled');
    return false;
  }
  ...
  try {
    ...
    if (!exitIp) {
      applyNoLeakWebRtcFallback(launchOpts, 'could not probe proxy exit IP');
      return false;
    }
    ...
    // WebRTC stays on so Camoufox can spoof ICE to the exit IP (pro path).
    if (launchOpts.firefoxUserPrefs) {
      delete launchOpts.firefoxUserPrefs['media.peerconnection.enabled'];
    }
    console.log(`[camoufox] geoip matched exit IP ${exitIp} (timezone/locale/WebRTC spoof — pro path)`);
    return true;
  } catch (err) {
    applyNoLeakWebRtcFallback(launchOpts, `geoip failed (${err.message})`);
    return false;
  }
}
...
export async function launchCamoufoxSession({
  ...
  const contextOpts = {};
  // CRITICAL: never paint US/UK Playwright locale/TZ onto a MENA mobile exit when geoip failed.
  // That mismatch + any WebRTC leak would make 4k–5k accounts look like one farm.
  // When geoipApplied, Camoufox CAMOU_CONFIG already owns locale/TZ/WebRTC for the exit IP.
  if (!geoipApplied && !isProxyEnabled()) {
