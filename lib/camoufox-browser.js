...
async function applyGeoipToLaunchOptions(launchOpts, email, fingerprint, savedState, accountOs) {
  ...
  try {
    ...
    // Probe through THIS session's proxy (dedicated relay), not a random shared upstream hop.
...
export async function launchCamoufoxSession({
  ...
  // Residential login: dedicated proxy per Camoufox (never share relay across parallel logins).
  // Mobile sticky: shared local relay + pin (same phone IP by design).
  const { isLoginAwsApi, isLoginResidentialIpv4 } = await import('./settings.js');
  let dedicatedProxy = null;
  let useSharedPin = false;
  if (isProxyEnabled() && (isLoginAwsApi() || isLoginResidentialIpv4())) {
    const { createDedicatedLoginProxy } = await import('./proxy-local.js');
    dedicatedProxy = await createDedicatedLoginProxy();
    // Local anonymized relay — auth stays in proxy-chain; Camoufox only gets 127.0.0.1.
    if (dedicatedProxy?.server) {
      launchOpts.proxy = { server: dedicatedProxy.server };
    }
    launchOpts.firefoxUserPrefs = {
      ...(launchOpts.firefoxUserPrefs || {}),
    ...
    useSharedPin = isProxyEnabled() && !!launchOpts.proxy?.server;
  }

  // geoip spoofs WebRTC to THIS session's exit IP (probed through dedicated relay when used).
  await applyGeoipToLaunchOptions(
    launchOpts,
    email,
    resolvedFp,
  ...
  const contextOpts = {};
  if (dedicatedProxy?.ignoreHTTPSErrors) {
  ...
  if (useSharedPin) pinLocalProxyRelay();
  let cleaned = false;
  const cleanupProxy = async () => {
    if (cleaned) return;
    cleaned = true;
    if (useSharedPin) unpinLocalProxyRelay();
    if (dedicatedProxy?.close) await dedicatedProxy.close().catch(() => {});
  };
