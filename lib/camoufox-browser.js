...
/**
 * Sticky OS per account (windows | macos). Never host linux.
 * - Same email → same OS forever (Microsoft device continuity).
 * - Different emails → mix (~80% windows / ~20% macos) so 10k accounts are not one OS farm.
 * - Never "linux": Coolify host is Linux; linux fingerprints + host fonts are a leak risk at scale.
 * Docs allow os list; sticky-per-account is the mass-login-safe form of rotation.
 * @see https://camoufox.com/python/usage/ — Device Rotation / os
 */
function pickAccountOs(email, savedOs) {
  if (savedOs === 'windows' || savedOs === 'macos') return savedOs;
  const n = crypto.createHash('sha256').update(String(email || '').toLowerCase().trim()).digest()[0];
  return n % 5 === 0 ? 'macos' : 'windows';
}

/** Bump when Camoufox device policy changes (forces one-time BrowserForge rebuild). */
const DEVICE_POLICY_VERSION = 3;
...
function baseCamoufoxConfig(email, fingerprint, savedState, { forGeoip = false, os } = {}) {
  ...
  const accountOs = pickAccountOs(email, os || savedState?.camoufoxOs);
...
async function applyGeoipToLaunchOptions(launchOpts, email, fingerprint, savedState, accountOs) {
  if (!isProxyEnabled() || !isCamoufoxGeoipEnabled() || !launchOpts.proxy?.server) {
    // No geoip spoof possible — never expose Coolify host IP via WebRTC.
    hardenWebRtcNoLeak(launchOpts);
    return false;
  }

  const { config } = baseCamoufoxConfig(email, fingerprint, savedState, {
    forGeoip: true,
    os: accountOs,
  });
...
async function loadOrCreateLaunchOptions(email, target, fingerprint, savedState, regenerateFingerprint) {
  ...
  const accountOs = pickAccountOs(email, cachedMode?.os || savedState?.camoufoxOs);
