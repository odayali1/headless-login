...
export async function captureOutlookTokens(page, { log, context, engine, existingTokens, forceRenew = false, dismissPrompts, skipPreflightRefresh = false, refreshTokenKnownBad = false } = {}) {
  ...
  const work = async () => {
    ...
    if (freshLogin) {
      ...
      if (!acceptCapturedToken(tokens) && context) {
...
/**
 * Fresh password login — pro TokenMan path:
 * 1) Trigger MSAL on Outlook (no full inbox wait)
 * 2) As soon as ANY refresh_token appears → Loki redeem → done
 * 3) Short Teams fallback for RT
 * Skips cookie PKCE (always login_required right after password).
 */
async function captureFreshLoginTokenMan(page, listener, log, dismissPrompts) {
  ...
  // Soft reload to provoke oauth2/token posts — don't wait for mail UI.
  await page.reload({ waitUntil: NAV_WAIT, timeout: navTimeout }).catch(() => {});
  await runDismissPrompts(page, dismissPrompts);
  await pokeOutlookMsal(page);

  let tokens = await listenAndRedeem(page, listener, log, 32_000, triedRts, 1_500);
  if (acceptCapturedToken(tokens)) {
    log?.('token', 'TokenMan success via Outlook MSAL RT → Loki');
    return tokens;
  }

  // Cached LiveProfile AT (rare but free).
  ...
  // Brief return to Outlook — one more RT poll.
  await ensureOutlookMailPage(page, log, dismissPrompts, navTimeout);
  await pokeOutlookMsal(page);
  tokens = await listenAndRedeem(page, listener, log, 18_000, triedRts, 1_500);
  if (acceptCapturedToken(tokens)) {
