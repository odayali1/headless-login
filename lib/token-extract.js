...
export function attachTokenListener(page, context) {
  ...
  const accept = (tokens) => {
    if (captured || !tokens?.access_token) return;
  ...
  const onResponse = async (response) => {
    ...
    try {
      ...
      // Prefer LiveProfileCard / Loki; also keep refresh_token even if scope is MSA-only.
      if (isLiveProfileCardToken(json) || String(json.scope || '').toLowerCase().includes('loki')) {
        accept(normalizeTokenPayload(json, 'network'));
      }
    } catch {
      // ignore non-json
    }
