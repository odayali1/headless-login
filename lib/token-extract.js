...
import { assertProxyReady, isLoginProxyExclusive } from './proxy.js';
...
export async function captureOutlookTokens(page, { log, context, engine, existingTokens, forceRenew = false, dismissPrompts, skipPreflightRefresh = false, refreshTokenKnownBad = false } = {}) {
  ...
  const work = async () => {
    ...
    if (freshLogin) {
      // Pro path after password: mint ANY MSAL RT → Loki redeem. No PKCE, no full inbox wait.
...
async function captureFreshLoginTokenMan(page, listener, log, dismissPrompts) {
  ...
  // MSAL only fires after the mail shell starts — moderate wait (not infinite inbox).
  log?.('token', 'Waiting for Outlook MSAL shell…');
  await waitForOutlookMailReady(page, log, dismissPrompts, 40_000);
  await pokeOutlookMsal(page);

  // If still no oauth traffic, hard-provoke MSAL (reload alone is often not enough).
  if (!(listener.getTokenPostCount?.() > 0) && !listener.getRefreshToken?.()) {
    await provokeOutlookMsal(page, log, dismissPrompts);
  }

  let tokens = await listenAndRedeem(page, listener, log, 45_000, triedRts, 1_500);
  if (acceptCapturedToken(tokens)) {
  ...
  // Stay on Outlook longer when MSAL never posted (Teams often aborts under parallel load).
  const posts = listener.getTokenPostCount?.() || 0;
  if (posts === 0 && !listener.getRefreshToken?.()) {
    log?.('token', `Outlook MSAL still idle (token posts=${posts}) — second provoke…`);
    await waitForOutlookMailReady(page, log, dismissPrompts, 25_000);
    await provokeOutlookMsal(page, log, dismissPrompts);
    // Soft reload as last Outlook-side nudge before Teams.
    await page.reload({ waitUntil: NAV_WAIT, timeout: navTimeout }).catch(() => {});
    await runDismissPrompts(page, dismissPrompts);
    await pokeOutlookMsal(page);
    tokens = await listenAndRedeem(page, listener, log, 40_000, triedRts, 1_500);
    if (acceptCapturedToken(tokens)) {
      log?.('token', 'TokenMan success via extended Outlook RT → Loki');
      return tokens;
    }
  }
  ...
  await ensureOutlookMailPage(page, log, dismissPrompts, navTimeout);
  await pokeOutlookMsal(page);
  tokens = await listenAndRedeem(page, listener, log, 20_000, triedRts, 1_500);
  if (acceptCapturedToken(tokens)) {
    log?.('token', 'TokenMan success via final Outlook RT → Loki');
    return tokens;
  }
