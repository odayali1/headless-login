...
async function captureViaSavedSession(
  ...
  // Laptop-like flow: wake Outlook briefly, then Teams (fires Loki + token POSTs with refresh_token).
  if (!/outlook\.live\.com\/mail/i.test(page.url())) {
    await ensureOutlookMailPage(page, log, dismissPrompts, navTimeout);
  }
  await waitForMsalReady(page, log, dismissPrompts, 20_000);
  ...
  log?.('token', 'Opening Teams to capture access + refresh tokens…');
  await ensureTeamsPage(page, log, dismissPrompts, navTimeout);
  await page.waitForTimeout(4000);

  let tokens = await listener.wait(25_000);
  ...
  if (!acceptCapturedToken(tokens, { fresh: true }) || !networkRt) {
    // Same as the fast probe apps: one Teams retry without clearing the session.
