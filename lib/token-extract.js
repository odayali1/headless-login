...
export async function captureOutlookTokens(page, { log, context, engine, existingTokens, forceRenew = false, dismissPrompts } = {}) {
  ...
  const work = async () => {
    ...
    let refreshToken = refreshFromMsal || refreshFromProfile;
...
/** Exchange refresh_token using saved cookies only — no Outlook page load (~5-50 KB vs 3-8 MB). */
