...
async function captureFreshLoginTokenMan(page, listener, log, dismissPrompts) {
  ...
  // If still no oauth traffic, one reload then keep polling.
  if (!(listener.getTokenPostCount?.() > 0) && !listener.getRefreshToken?.()) {
    log?.('token', 'Reloading Outlook to trigger oauth2/token…');
  ...
  if (posts === 0 && !listener.getRefreshToken?.()) {
    log?.('token', `Outlook MSAL still idle (token posts=${posts}) — extended Outlook wait…`);
    await waitForOutlookMailReady(page, log, dismissPrompts, 35_000);
    await pokeOutlookMsal(page);
