...
export async function captureOutlookTokens(page, { log, context, engine, existingTokens, forceRenew = false, dismissPrompts, skipPreflightRefresh = false, refreshTokenKnownBad = false } = {}) {
  ...
  const work = async () => {
    ...
    if (refreshToken) {
      ...
      // No RT yet (typical right after password login) — skip long Outlook MSAL waits;
      // Teams-first session capture is what actually yields refresh_token in production.
      log?.('token', 'No refresh_token yet — Teams-first session capture…');
      return captureViaSavedSession(page, listener, log, dismissPrompts, {
        sawInvalidGrant,
        invalidGrantMessage,
        context,
        refreshFromProfile: null,
      });
    }
...
export async function tryCookiePkceAuthCode(savedState, log) {
  ...
  // Prefer the resolved Camoufox path (SOCKS relay). Plain HTTP CONNECT often returns 407
  // for authorize redirects even when Loki POSTs work.
  let proxy;
  try {
    const cfg = await getPlaywrightProxyConfig();
    if (cfg?.mode === 'http-direct') {
      proxy = { server: cfg.server, username: cfg.username, password: cfg.password };
    } else if (cfg?.server) {
  ...
  for (const profile of profiles) {
    ...
    try {
      ...
      if (json?.error) {
        log?.('token', `Cookie SSO token redeem: ${json.error}`);
      }
    } catch (err) {
