...
export async function captureOutlookTokens(page, { log, context, engine, existingTokens, forceRenew = false, dismissPrompts, skipPreflightRefresh = false, refreshTokenKnownBad = false } = {}) {
  ...
  const CAPTURE_TIMEOUT_MS = wifiSplit ? 150_000 : refreshTokenKnownBad ? 90_000 : proxySlow ? 90_000 : 150_000;
...
async function gotoWithProxyRetry(page, url, log, { waitUntil = NAV_WAIT, timeout = NAV_TIMEOUT_MS } = {}) {
  ...
  try {
    ...
    log?.('proxy', `Navigation failed (${err.message}) — refreshing proxy relay`);
    // Same Camoufox process cannot switch relay ports. Soft wait + one commit retry only.
