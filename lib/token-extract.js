...
export async function captureOutlookTokens(page, { log, context, engine, existingTokens, forceRenew = false, dismissPrompts, skipPreflightRefresh = false, refreshTokenKnownBad = false } = {}) {
  ...
  const work = async () => {
    ...
    // roadtx-style via the LIVE Camoufox context (same SOCKS + cookie jar).
    // Browserless HTTP CONNECT PKCE often returns interaction_required right after password
    // and can race-resolve the shared proxy while this browser is still open.
