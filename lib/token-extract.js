...
/** Cookie SSO on mobile — keep running during login, but don't starve Camoufox. */
const COOKIE_SSO_PARALLEL = Number(process.env.COOKIE_SSO_PARALLEL || 4);
const COOKIE_SSO_PARALLEL_LOGIN = Number(process.env.COOKIE_SSO_PARALLEL_LOGIN || 1);
...
function cookieSsoMaxParallel() {
  const idle = Number.isFinite(COOKIE_SSO_PARALLEL) && COOKIE_SSO_PARALLEL > 0 ? COOKIE_SSO_PARALLEL : 4;
  const duringLogin =
...
export async function captureOutlookTokens(page, { log, context, engine, existingTokens, forceRenew = false, dismissPrompts, skipPreflightRefresh = false, refreshTokenKnownBad = false } = {}) {
  ...
  const work = async () => {
    ...
    if (likelyFresh && context && !isCancelled()) {
      ...
      let earlyCookie = await tryContextRequestPkceAuthCode(context, log);
      if (acceptCapturedToken(earlyCookie)) {
        listener.stop?.();
        return earlyCookie;
      }
      // login_required often means proofs/Add was never skipped — surface + click Skip, then retry once.
      log?.('token', `Context cookie SSO not ready (url=${page.url().slice(0, 100)}) — proofs/Skip recovery…`);
      await runDismissPrompts(page, dismissPrompts);
      if ((await isBackupEmailScreen(page)) || isInterruptUrl(page.url())) {
        const skipped = await clickSkipBackupEmail(page, { waitMs: 12_000 });
        if (skipped) log?.('token', `Skipped backup interrupt before SSO retry: "${skipped}"`);
      } else {
        await page
          .goto('https://account.live.com/proofs/Add', { waitUntil: 'domcontentloaded', timeout: 30_000 })
          .catch(() => {});
        await page.waitForTimeout(2000);
        const skipped = await clickSkipBackupEmail(page, { waitMs: 12_000 });
        if (skipped) {
          log?.('token', `Skipped via proofs/Add before SSO retry: "${skipped}"`);
          await runDismissPrompts(page, dismissPrompts);
        }
      }
      earlyCookie = await tryContextRequestPkceAuthCode(context, log);
      if (acceptCapturedToken(earlyCookie)) {
        listener.stop?.();
        return earlyCookie;
      }
      log?.('token', 'Context cookie SSO still not ready — landing Outlook, then retry / TokenMan fallback…');
    }
 ...
 * forCookie=true → always mobile (login.live.com). Hybrid Loki/MSAL → residential IPv6.
 */
async function playwrightProxyOpts({ forCookie = false } = {}) {
  if (!isProxyEnabled()) return undefined;

  try {
    const { isHybridProxyEnabled } = await import('./settings.js');
    if (isHybridProxyEnabled()) {
      const { getPlaywrightProxyConfigForLane } = await import('./proxy-local.js');
      // Cookie SSO stays on mobile; Loki/MSAL use residential.
      const lane = forCookie ? 'browser' : 'http-refresh';
