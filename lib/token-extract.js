...
/** Cookie SSO PKCE on mobile — browserless, no Camoufox. Cap keeps MSA softban risk bounded. */
...
export async function tryCookiePkceAuthCode(savedState, log) {
  ...
  return withCookieSsoSlot(async () => {
    const { pinCookieSsoRelay, unpinCookieSsoRelay } = await import('./proxy-local.js');
    pinCookieSsoRelay();
    // Hard cap so a dead 127.0.0.1 relay cannot stall the Fast lane forever.
    const hardDeadline = Date.now() + Math.max(BROWSERLESS_HTTP_TIMEOUT_MS * 2, 75_000);
    try {
      ...
      for (const profile of profiles) {
        if (Date.now() >= hardDeadline) {
          log?.('token', 'Cookie SSO aborted — hard deadline (avoid Fast lane hang)');
          break;
        }
        const { verifier, challenge } = makePkce();
