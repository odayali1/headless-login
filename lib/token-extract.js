...
async function gotoWithProxyRetry(page, url, log, { waitUntil = NAV_WAIT, timeout = 60_000 } = {}) {
  ...
  try {
    ...
    // Do NOT closeLocalProxy() here — this Firefox session still points at the old relay port.
    // Closing it makes retry hang forever. Login/reopenBrowser recreates Camoufox with a fresh relay.
    if (!isProxyEnabled()) throw err;
    log?.('proxy', `Navigation failed (${err.message}) — one more attempt on same session…`);
