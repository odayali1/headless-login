...
export async function captureOutlookTokens(page, { log, context, engine, existingTokens, forceRenew = false, dismissPrompts, skipPreflightRefresh = false, refreshTokenKnownBad = false } = {}) {
  // Fresh login: one quick cookie SSO shot, then TokenMan RT→Loki (same order that worked before),
  // then cookie SSO again if TokenMan misses — smart-refresh path unchanged.
  ...
  const work = async () => {
    ...
    // Fast win when ESTSAUTH is already good (e.g. after Skip) — one shot only.
    if (likelyFresh && context && !isCancelled()) {
      log?.('token', 'Fresh login — quick cookie SSO shot…');
      const earlyCookie = await tryContextRequestPkceAuthCode(context, log);
      if (acceptCapturedToken(earlyCookie)) {
        listener.stop?.();
        return earlyCookie;
      }
      log?.('token', 'Cookie SSO not ready — TokenMan path (mint RT → Loki), then cookie again…');
    }
    ...
    if (freshLogin) {
      // ee51e8e path: TokenMan Outlook first, then cookie SSO BEFORE Teams so we don't
      // burn the 180s timeout on a dead Teams pass (that was "token not captured").
      tokens = await captureFreshLoginTokenMan(page, listener, log, dismissPrompts, {
        skipTeams: true,
      });

      if (!acceptCapturedToken(tokens) && context && !isCancelled()) {
        log?.('token', 'TokenMan Outlook miss — cookie SSO PKCE (same as smart-refresh)…');
      ...
      if (!acceptCapturedToken(tokens) && !isCancelled()) {
        log?.('token', 'Cookie SSO miss — in-browser PKCE…');
        const inBrowser = await tryInBrowserPkceAuthCode(page, context, log);
        if (acceptCapturedToken(inBrowser)) tokens = inBrowser;
      }
      if (!acceptCapturedToken(tokens) && !isCancelled() && !page.isClosed?.()) {
        log?.('token', 'Still no token — short Teams pass…');
        tokens = await captureViaTeamsQuick(page, listener, log, dismissPrompts, new Set(), {
          listenMs: 12_000,
        });
      }
      if (!acceptCapturedToken(tokens) && context && !isCancelled()) {
        log?.('token', 'Teams miss — last cookie SSO retry…');
 ...
 */
async function captureFreshLoginTokenMan(page, listener, log, dismissPrompts, { skipTeams = false } = {}) {
  log?.('token', 'Fresh login — TokenMan path (mint RT → Loki redeem)…');
  const triedRts = new Set();
  const navTimeout = Math.min(navTimeoutMs(), 45_000);

  // Never start TokenMan from proofs/Add — shell never loads there.
  if (/account\.live\.com/i.test(page.url())) {
    log?.('token', 'Left proofs/account page — opening Outlook for MSAL…');
  }
  await ensureOutlookMailPage(page, log, dismissPrompts, navTimeout);
  await runDismissPrompts(page, dismissPrompts);

  log?.('token', 'Waiting for Outlook MSAL shell…');
  await waitForOutlookMailReady(page, log, dismissPrompts, 25_000);
  await pokeOutlookMsal(page);
  ...
  log?.('token', `TokenMan Outlook miss — token posts=${listener.getTokenPostCount?.() || 0}, hasRt=${!!listener.getRefreshToken?.()}`);
...
async function captureViaTeamsQuick(page, listener, log, dismissPrompts, triedRts = new Set(), { listenMs = 22_000 } = {}) {
  if (page.isClosed?.()) return null;
  const navTimeout = Math.min(navTimeoutMs(), 40_000);
  const firstListen = Math.max(8_000, Number(listenMs) || 22_000);
  await ensureTeamsPage(page, log, dismissPrompts, navTimeout);
  if (page.isClosed?.()) return null;
  ...
  let tokens = await listenAndRedeem(page, listener, log, firstListen, triedRts, 1_500);
  if (acceptCapturedToken(tokens)) return tokens;
  if (page.isClosed?.()) return null;

  // Second reload only when first listen had a reasonable budget (mail looked alive).
  if (firstListen < 18_000) {
    return acceptCapturedToken(tokens);
  }
