...
async function captureFreshLoginTokenMan(page, listener, log, dismissPrompts) {
  ...
  const mailReady = await waitForOutlookMailReady(page, log, dismissPrompts, 20_000);
  await pokeOutlookMsal(page);
  ...
  let tokens = await listenAndRedeem(page, listener, log, 20_000, triedRts, 1_500);
  if (acceptCapturedToken(tokens)) {
  ...
  // Outlook never painted mail chrome → Teams rarely helps; keep Teams short.
  ...
  if (!page.isClosed?.()) {
    tokens = await captureViaTeamsQuick(page, listener, log, dismissPrompts, triedRts, {
      listenMs: mailReady && listener.getTokenPostCount?.() > 0 ? 22_000 : 12_000,
    });
    if (acceptCapturedToken(tokens)) {
