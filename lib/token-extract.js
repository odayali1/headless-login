...
export async function captureOutlookTokens(page, { log, context, engine, existingTokens, forceRenew = false } = {}) {
  ...
  const work = async () => {
    ...
    const refreshFromProfile = forceRenew ? null : existingTokens?.refresh_token;
    ...
    let tokens = await triggerOutlookTokenCapture(page, listener, log);

    if (!isLiveProfileCardToken(tokens)) {
      log?.('token', 'Second Outlook reload to trigger MSAL…');
      tokens = await triggerOutlookTokenCapture(page, listener, log);
    }
...
async function triggerOutlookTokenCapture(page, listener, log) {
...
async function waitForOutlookMailReady(page, log, timeoutMs = 75_000) {
