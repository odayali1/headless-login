...
import { getPlaywrightProxyConfig, isBrowserPathSplitBlocked } from './proxy-local.js';
import { isBackupEmailScreen } from './security-prompts.js';
import {
  navTimeoutMs,
  msalReadyTimeoutMs,
  captureTimeoutMs,
  skipPlaywrightHttpRefresh,
} from './proxy-timing.js';
...
export async function captureOutlookTokens(page, { log, context, engine, existingTokens, forceRenew = false, dismissPrompts, skipPreflightRefresh = false, refreshTokenKnownBad = false, slowProxyPath = false } = {}) {
  const sessionOnlyPath = skipPlaywrightHttpRefresh();
  const CAPTURE_TIMEOUT_MS = captureTimeoutMs({ refreshTokenKnownBad, slowProxyPath });
  ...
  const work = async () => {
    ...
    // Mobile relay: Playwright HTTP is slow; use Camoufox session (curl probe can still be fast).
    if (sessionOnlyPath && isProxyEnabled()) {
      log?.('token', 'Mobile proxy — session capture only (skip slow Playwright HTTP refresh)');
      return captureViaSavedSession(page, listener, log, dismissPrompts, {
        sawInvalidGrant: refreshTokenKnownBad,
...
async function captureViaSavedSession(
  ...
  log,
  dismissPrompts,
  { sawInvalidGrant, invalidGrantMessage, context, refreshFromProfile, slowProxyPath = false }
) {
  log?.('token', 'Using saved session for MSAL token capture…');
  const navTimeout = navTimeoutMs();

  if (slowProxyPath) {
    log?.('token', 'Slow proxy uplink — Teams-first (skip Outlook SPA on Wi‑Fi Split)');
    await ensureTeamsPage(page, log, dismissPrompts);
  } else if (!/outlook\.live\.com\/mail/i.test(page.url())) {
    await ensureOutlookMailPage(page, log, dismissPrompts, navTimeout);
  }
  if (!slowProxyPath && isMobileRelayProxy() && !/outlook\.live\.com|teams\.live\.com/i.test(page.url())) {
    log?.('token', 'Outlook did not open on mobile proxy — trying Teams…');
    await gotoWithProxyRetry(page, TEAMS_LIVEPROFILE.appUrl, log, { timeout: navTimeout }).catch(() => {});
  }
  await waitForMsalReady(page, log, dismissPrompts, msalReadyTimeoutMs());
  ...
  const refreshToken = refreshFromMsal || (sawInvalidGrant ? null : refreshFromProfile);
  if (refreshToken && context) {
    log?.('token', 'Trying in-browser refresh_token exchange (session Origin + cookies)…');
    const exchanged = await exchangeLiveProfileToken(page, context, refreshToken, log);
    if (acceptCapturedToken(exchanged)) {
      listener.wait(1).catch(() => {});
      return exchanged;
    }
    if (isInvalidGrant(exchanged)) {
      ...
      log?.('token', 'In-browser refresh rejected — waiting for MSAL silent token…');
    } else if (exchanged?.error) {
      log?.('token', `In-browser exchange: ${formatTokenExchangeError(exchanged)}`);
    }
  }

  const cached = await readMsalLiveProfileToken(page);
  if (acceptCapturedToken(cached, { fresh: true })) {
    listener.wait(1).catch(() => {});
    return cached;
  }
  ...
  if (!acceptCapturedToken(tokens, { fresh: true })) {
    const cachedAfterWait = await readMsalLiveProfileToken(page);
    if (acceptCapturedToken(cachedAfterWait, { fresh: true })) tokens = cachedAfterWait;
  }

  if (!acceptCapturedToken(tokens, { fresh: true })) {
    log?.('token', 'Reloading Outlook to trigger MSAL token request…');
    await page.reload({ waitUntil: NAV_WAIT, timeout: navTimeoutMs() }).catch(() => {});
    await waitForMsalReady(page, log, dismissPrompts, 30_000);
    await pokeOutlookMsal(page);
    tokens = await listener.wait(35_000);
    if (!acceptCapturedToken(tokens, { fresh: true })) {
      const cachedAfterReload = await readMsalLiveProfileToken(page);
      if (acceptCapturedToken(cachedAfterReload, { fresh: true })) tokens = cachedAfterReload;
    }
  }

  if (!acceptCapturedToken(tokens, { fresh: true })) {
    if (slowProxyPath) {
      log?.('token', 'Teams-first did not yield a fresh token — trying Outlook…');
      await ensureOutlookMailPage(page, log, dismissPrompts, navTimeout).catch(() => {});
      await waitForMsalReady(page, log, dismissPrompts, msalReadyTimeoutMs());
      const exchanged = refreshToken && context
        ? await exchangeLiveProfileToken(page, context, refreshToken, log)
        : null;
      if (acceptCapturedToken(exchanged)) {
        listener.wait(1).catch(() => {});
        return exchanged;
...
async function tryCurlTokenRefreshProfile(refreshToken, profile) {
  const { curlPostForm } = await import('./curl-proxy-http.js');
  const body = buildRefreshTokenBody(refreshToken, profile);
  try {
    return await curlPostForm(profile.tokenUrl, body, {
      Origin: profile.origin,
      Referer: profile.referer,
      'content-type': 'application/x-www-form-urlencoded;charset=utf-8',
    });
  } catch (err) {
    return { error: 'curl_refresh_failed', error_description: err.message };
  }
}

/** refresh_token exchange via curl — works on Wi‑Fi Split when Playwright HTTP fails. */
export async function tryCurlTokenRefresh(refreshToken, log) {
  if (!refreshToken || !isProxyEnabled()) return null;

  let lastError = null;
  for (const profile of TOKEN_PROFILES) {
    log?.('token', `Curl refresh (${profile.label} MSAL, Wi‑Fi Split safe)…`);
    const result = await tryCurlTokenRefreshProfile(refreshToken, profile);
    if (result?.access_token && isLiveProfileCardToken(result)) {
      const normalized = normalizeTokenPayload(result, `curl_refresh_${profile.label.toLowerCase()}`);
...
async function tryAllCurlRefreshCandidates(savedState, log) {
  const candidates = [];
  ...
  if (!candidates.length) return { tokens: null, refreshTokenKnownBad: false };

  let sawInvalidGrant = false;
  for (const rt of candidates) {
    const result = await tryCurlTokenRefresh(rt, log);
    if (result?.access_token && isTokenValid(result)) {
      return { tokens: result, refreshTokenKnownBad: false };
    }
    if (isInvalidGrant(result)) {
      sawInvalidGrant = true;
      log?.('token', 'Refresh token rejected by Microsoft (curl)');
    }
  }
  return { tokens: null, refreshTokenKnownBad: sawInvalidGrant };
}

async function tryBrowserlessTokenRefreshProfile(savedState, refreshToken, profile) {
  const cfg = isProxyEnabled() ? await getPlaywrightProxyConfig() : null;
  const proxy =
...
async function gotoWithProxyRetry(page, url, log, { waitUntil = NAV_WAIT, timeout = navTimeoutMs() } = {}) {
  ...
  try {
    ...
    if (isMobileRelayProxy()) {
      log?.('proxy', `Navigation failed (${err.message})`);
      throw err;
    }
...
export async function tryAllBrowserlessRefresh(savedState, log) {
  ...
  // On mobile relay / Wi‑Fi Split, curl works when Playwright HTTP does not.
  if (isProxyEnabled() && isMobileRelayProxy()) {
    const curlResult = await tryAllCurlRefreshCandidates(savedState, log);
    if (curlResult.tokens?.access_token && isTokenValid(curlResult.tokens)) {
      return { tokens: curlResult.tokens, refreshTokenKnownBad: false, fromCache: false, via: 'curl' };
    }
    if (curlResult.refreshTokenKnownBad) {
      return { tokens: null, refreshTokenKnownBad: true, fromCache: false };
    }
    if (isBrowserPathSplitBlocked()) {
