...
export async function captureOutlookTokens(page, { log, context, engine, existingTokens, forceRenew = false, dismissPrompts, skipPreflightRefresh = false, refreshTokenKnownBad = false } = {}) {
  // Fresh login uses TokenMan RT→Loki (faster than waiting on full Outlook SPA).
  const likelyFresh = !existingTokens?.refresh_token && !refreshTokenKnownBad;
  // Session recovery (bad RT) needs Teams+Outlook room — 90s caused orphan work after close.
  ...
  const work = async () => {
    ...
    if (freshLogin) {
      // Pro path after password: mint ANY MSAL RT → Loki redeem.
      tokens = await captureFreshLoginTokenMan(page, listener, log, dismissPrompts);

      // Same path smart-refresh uses when RT is missing — do this BEFORE 180s timeout waste.
...
/**
 * Fresh password login — TokenMan path:
 * Wait until Outlook MSAL is actually posting tokens, then RT → Loki immediately.
 * Production logs showed soft-reload-without-ready never minted an RT.
 */
