...
/** Cookie SSO PKCE on mobile — browserless, no Camoufox. Cap 16: env 32–64 floods login.live.com. */
const COOKIE_SSO_PARALLEL_MAX = 16;
...
const COOKIE_SSO_PARALLEL = Math.min(
  ...
  Math.max(1, Number(process.env.COOKIE_SSO_PARALLEL || 16) || 16)
);
if (Number.isFinite(requestedCookieSsoParallel) && requestedCookieSsoParallel > COOKIE_SSO_PARALLEL) {
  console.log(
    `[token] Clamping COOKIE_SSO_PARALLEL ${requestedCookieSsoParallel} → ${COOKIE_SSO_PARALLEL} (wide PKCE bursts time out and grow needs-refresh)`
  );
}
