...
import { isProxyEnabled, isIproxyWifiSplitMode } from './settings.js';
...
function navTimeoutMs() {
  return isIproxyWifiSplitMode() ? Number(process.env.PROXY_WIFI_SPLIT_NAV_TIMEOUT_MS || 120_000) : NAV_TIMEOUT_MS;
}
...
export async function captureOutlookTokens(page, { log, context, engine, existingTokens, forceRenew = false, dismissPrompts, skipPreflightRefresh = false, refreshTokenKnownBad = false } = {}) {
  ...
  const CAPTURE_TIMEOUT_MS = wifiSplit ? 180_000 : refreshTokenKnownBad ? 90_000 : proxySlow ? 90_000 : 150_000;
  ...
  const work = async () => {
    ...
    // Wi-Fi Split: Playwright HTTP takes ~9–37s per call; skip slow paths, use Camoufox session (like manual curl).
    if (wifiSplit && isProxyEnabled()) {
      log?.('token', 'Wi-Fi Split — session capture only (skip slow Playwright HTTP refresh)');
...
async function captureViaSavedSession(
  ...
  if (isIproxyWifiSplitMode() && !/outlook\.live\.com|teams\.live\.com/i.test(page.url())) {
    log?.('token', 'Outlook did not open on Wi-Fi Split — trying Teams…');
    await gotoWithProxyRetry(page, TEAMS_LIVEPROFILE.appUrl, log, { timeout: navTimeout }).catch(() => {});
  }
  await waitForMsalReady(page, log, dismissPrompts, isIproxyWifiSplitMode() ? 45_000 : 35_000);
