...
import {
  ...
  isLoginCookieSsoBlocked,
  isLoginProxyExclusive,
} from './proxy.js';
import { getPlaywrightProxyConfig } from './proxy-local.js';
import { isBackupEmailScreen, isInterruptUrl, clickSkipBackupEmail } from './security-prompts.js';

const OUTLOOK_MAIL = 'https://outlook.live.com/mail/';
/** Outlook/Teams/MSA token endpoints (v2, v1, and Live oauth20). */
const TOKEN_URL_RE = /oauth2\/v2\.0\/token|\/oauth2\/token(?:\?|$)|oauth20_token\.srf/i;
...
/** Fail fast on HTTP token refresh — long timeouts made every dead-RT burn ~25s before cookie SSO. */
const BROWSERLESS_HTTP_TIMEOUT_MS = Number(process.env.BROWSERLESS_HTTP_TIMEOUT_MS || 8_000);
/** Cookie SSO PKCE on mobile. Cap 32 — cookie-first succeeds in ~1s; 64-wide mixed Loki+SSO timed out. */
const COOKIE_SSO_PARALLEL_MAX = 32;
const requestedCookieSsoParallel = Number(process.env.COOKIE_SSO_PARALLEL);
const COOKIE_SSO_PARALLEL = Math.min(
  COOKIE_SSO_PARALLEL_MAX,
  Math.max(1, Number(process.env.COOKIE_SSO_PARALLEL || 24) || 24)
);
if (Number.isFinite(requestedCookieSsoParallel) && requestedCookieSsoParallel > COOKIE_SSO_PARALLEL) {
  console.log(
    `[token] Clamping COOKIE_SSO_PARALLEL ${requestedCookieSsoParallel} → ${COOKIE_SSO_PARALLEL}`
  );
}
const COOKIE_SSO_PARALLEL_LOGIN = Math.min(
  16,
  Math.max(1, Number(process.env.COOKIE_SSO_PARALLEL_LOGIN || 4) || 4)
);

let cookieSsoInFlight = 0;
/** @type {Array<() => void>} */
const cookieSsoWaiters = [];

function cookieSsoMaxParallel() {
  const idle = Number.isFinite(COOKIE_SSO_PARALLEL) && COOKIE_SSO_PARALLEL > 0 ? COOKIE_SSO_PARALLEL : 16;
  const duringLogin =
    Number.isFinite(COOKIE_SSO_PARALLEL_LOGIN) && COOKIE_SSO_PARALLEL_LOGIN > 0
      ? COOKIE_SSO_PARALLEL_LOGIN
      : 4;
  return isLoginProxyExclusive() ? Math.min(idle, duringLogin) : idle;
}

async function withCookieSsoSlot(fn) {
  const max = Math.max(1, cookieSsoMaxParallel());
  while (cookieSsoInFlight >= max) {
    await new Promise((resolve) => cookieSsoWaiters.push(resolve));
  }
  cookieSsoInFlight += 1;
  try {
    return await fn();
  } finally {
    cookieSsoInFlight = Math.max(0, cookieSsoInFlight - 1);
    const next = cookieSsoWaiters.shift();
    if (next) next();
  }
}

/** Drain in-flight cookie SSO before tearing down the mobile SOCKS relay (changeip). */
export async function waitForCookieSsoQuiet(timeoutMs = 45_000) {
  const deadline = Date.now() + Math.max(0, Number(timeoutMs) || 0);
  while (cookieSsoInFlight > 0 && Date.now() < deadline) {
    await new Promise((r) => setTimeout(r, 200));
  }
  return cookieSsoInFlight === 0;
}
...
export function attachTokenListener(page, context) {
  ...
  const noteTokenUrl = (url, postData) => {
    if (!TOKEN_URL_RE.test(url || '')) return;
    tokenPostCount += 1;
    const post = postData || '';
    if (!post) return;
  ...
  const onRequest = (request) => {
    try {
      ...
      noteTokenUrl(url, request.postData() || '');
      if (!/loki\.delve\.office\.com/i.test(url)) return;
  ...
  const onResponse = async (response) => {
    ...
    try {
      ...
      // Non-LiveProfile AT: RT already stored above for Loki redeem.
    } catch {
      // ignore non-json
    }
  };
  ...
  return {
    ...
    getTokenPostCount: () => tokenPostCount,
    getCaptured: () => {
...
function isBrowserAlive(page, context) {
  try {
...
export async function captureOutlookTokens(page, { log, context, engine, existingTokens, forceRenew = false, dismissPrompts, skipPreflightRefresh = false, refreshTokenKnownBad = false } = {}) {
  // Fresh login: cookie SSO first (ESTSAUTH live). TokenMan Outlook/Teams SPA is fallback only.
  const likelyFresh = !existingTokens?.refresh_token && !refreshTokenKnownBad;
  // Cookie-first path is usually <30s; keep room for TokenMan fallback when SSO fails.
  ...
  let cancelled = false;
  let listener = null;
  const isCancelled = () => cancelled || !isBrowserAlive(page, context);
  ...
  const work = async () => {
    ...
    listener = attachTokenListener(page, context);
    ...
    // Fresh password login: ESTSAUTH is already in the jar — try PKCE before Outlook/Teams SPA.
    if (likelyFresh && context && !isCancelled()) {
      log?.('token', 'Fresh login — cookie SSO PKCE first (Teams/Outlook, same Camoufox session)…');
      const earlyCookie = await tryContextRequestPkceAuthCode(context, log);
      if (acceptCapturedToken(earlyCookie)) {
        listener.stop?.();
        return earlyCookie;
      }
      log?.('token', 'Context cookie SSO not ready — landing Outlook, then retry / TokenMan fallback…');
    }

    // Land on Outlook mail — do NOT wait for full inbox UI (TokenMan path only needs MSAL traffic).
    ...
    const refreshFromMsal =
      (await readMsalRefreshToken(page)) ||
      (await readAnyMsalRefreshToken(page).catch(() => null)) ||
      collectMsalRefreshFromOrigins(storageState?.origins)[0] ||
      null;
    ...
    // TokenMan-first: HTTP Loki redeem beats in-browser SPA exchange.
    if (refreshToken) {
      log?.('token', 'TokenMan redeem — exchanging refresh_token (Loki scope)…');
      const loki = await tryLokiScopeRedeem(refreshToken, log, {
        clientId: listener.getClientId?.() || undefined,
      });
      if (acceptCapturedToken(loki)) {
        listener.stop?.();
        return { ...loki, refresh_token: loki.refresh_token || refreshToken };
      }
      if (isInvalidGrant(loki)) {
        sawInvalidGrant = true;
        invalidGrantMessage = formatTokenExchangeError(loki);
      ...
      log?.('token', 'Loki redeem missed — trying in-browser LiveProfile exchange…');
      ...
      if (isSpaOriginError(exchanged)) {
        log?.('token', 'SPA token endpoint — capturing from network instead…');
      }
    }
    ...
    if (freshLogin) {
      // Early cookie attempt already ran above; retry after Outlook land, then TokenMan.
      if (context && !isCancelled()) {
        log?.('token', 'Retry cookie SSO after Outlook land…');
      ...
      if (!acceptCapturedToken(tokens) && !isCancelled()) {
        log?.('token', 'Context cookie SSO miss — in-browser PKCE…');
        const inBrowser = await tryInBrowserPkceAuthCode(page, context, log);
        if (acceptCapturedToken(inBrowser)) tokens = inBrowser;
      }
      if (!acceptCapturedToken(tokens) && !isCancelled()) {
        log?.('token', 'Cookie SSO miss — TokenMan fallback (mint RT → Loki)…');
        tokens = await captureFreshLoginTokenMan(page, listener, log, dismissPrompts);
      }
      if (!acceptCapturedToken(tokens) && context && !isCancelled()) {
        log?.('token', 'TokenMan miss — cookie SSO retry…');
        const cookieRetry = await tryContextRequestPkceAuthCode(context, log);
        if (acceptCapturedToken(cookieRetry)) tokens = cookieRetry;
      }
    } else {
...
async function captureViaSavedSession(
  ...
  log,
  dismissPrompts,
  { sawInvalidGrant, invalidGrantMessage, context, refreshFromProfile, isCancelled = () => false }
) {
  log?.('token', 'Using saved session for MSAL token capture…');
  const navTimeout = navTimeoutMs();
  const alive = () => !isCancelled() && isBrowserAlive(page, context);
  ...
  if (alive() && (!acceptCapturedToken(tokens, { fresh: true }) || !networkRt)) {
  ...
  if (!networkRt && !(await isOutlookLoginPage(page).catch(() => false))) {
    ...
    networkRt = listener.getRefreshToken?.() || networkRt;
    clientId = listener.getClientId?.() || clientId;
  } else if (await isOutlookLoginPage(page).catch(() => false)) {
  ...
  const refreshFromMsal = await readMsalRefreshToken(page).catch(() => null);
  ...
  } else if (refreshFromProfile && !sawInvalidGrant && context && alive()) {
  ...
  if (!acceptCapturedToken(tokens, { fresh: true })) {
    const cached = await readMsalLiveProfileToken(page).catch(() => null);
    if (acceptCapturedToken(cached, { fresh: true })) tokens = cached;
  }

  if (!acceptCapturedToken(tokens, { fresh: true }) && alive()) {
...
/**
 * Fresh password login — TokenMan fallback (after cookie SSO miss):
 * Wait until Outlook MSAL is posting tokens, then RT → Loki.
 */
async function captureFreshLoginTokenMan(page, listener, log, dismissPrompts) {
  log?.('token', 'TokenMan fallback — mint RT → Loki redeem…');
  ...
  log?.('token', 'Waiting for Outlook MSAL shell…');
  await waitForOutlookMailReady(page, log, dismissPrompts, 20_000);
  await pokeOutlookMsal(page);

  if (!(listener.getTokenPostCount?.() > 0) && !listener.getRefreshToken?.()) {
    await provokeOutlookMsal(page, log, dismissPrompts);
  }

  let tokens = await listenAndRedeem(page, listener, log, 25_000, triedRts, 1_500);
  if (acceptCapturedToken(tokens)) {
    log?.('token', 'TokenMan success via Outlook MSAL RT → Loki');
    return tokens;
  }

  const cached = acceptCapturedToken(await readMsalLiveProfileToken(page).catch(() => null));
  if (cached) return cached;

  // One short Teams pass, then return so caller can cookie-SSO PKCE (same as smart-refresh).
  log?.('token', 'No RT yet — short Teams pass for refresh_token…');
  if (!page.isClosed?.()) {
    tokens = await captureViaTeamsQuick(page, listener, log, dismissPrompts, triedRts);
    if (acceptCapturedToken(tokens)) {
      log?.('token', 'TokenMan success via Teams MSAL RT → Loki');
      return tokens;
    }
  }

  log?.('token', `TokenMan miss — token posts=${listener.getTokenPostCount?.() || 0}, hasRt=${!!listener.getRefreshToken?.()}`);
...
async function listenAndRedeem(page, listener, log, listenMs, triedRts = new Set(), pollMs = 3_000) {
  ...
  while (Date.now() < end) {
    ...
    const slice = Math.min(pollMs, end - Date.now());
    if (slice <= 0) break;
...
async function captureViaTeamsQuick(page, listener, log, dismissPrompts, triedRts = new Set()) {
  if (page.isClosed?.()) return null;
  const navTimeout = Math.min(navTimeoutMs(), 40_000);
  await ensureTeamsPage(page, log, dismissPrompts, navTimeout);
  if (page.isClosed?.()) return null;
  ...
  let tokens = await listenAndRedeem(page, listener, log, 22_000, triedRts, 1_500);
  if (acceptCapturedToken(tokens)) return tokens;
  if (page.isClosed?.()) return null;

  const ok = await gotoWithProxyRetry(page, TEAMS_LIVEPROFILE.appUrl, log, { timeout: navTimeout }).catch(() => false);
  if (!ok || page.isClosed?.()) return acceptCapturedToken(tokens);
  await page.waitForTimeout(2500).catch(() => {});
  tokens = await listenAndRedeem(page, listener, log, 15_000, triedRts, 1_500);
  if (acceptCapturedToken(tokens)) return tokens;

  if (!listener.getRefreshToken?.() && triedRts.size === 0) {
...
/**
 * @param {{ forCookie?: boolean }} [opts]
 * forCookie=true → always mobile (login.live.com).
 * Hybrid OR residential-IPv4 login → Loki/MSAL on IPv6; cookie on mobile.
 */
async function playwrightProxyOpts({ forCookie = false } = {}) {
  if (!isProxyEnabled()) return undefined;

  try {
    const { isHybridProxyEnabled, isLoginResidentialIpv4 } = await import('./settings.js');
    if (isHybridProxyEnabled() || isLoginResidentialIpv4()) {
      const { getPlaywrightProxyConfigForLane } = await import('./proxy-local.js');
      const lane = forCookie ? 'cookie' : 'http-refresh';
      const cfg = await getPlaywrightProxyConfigForLane(lane);
      if (cfg?.server) {
        return cfg.mode === 'http-direct'
  ...
  // Browserless token POSTs: prefer HTTP CONNECT on mobile only.
  // Never use PROXY_HTTP_URL when residential — that is the iProxy phone, not the rotating SOCKS.
  try {
    const { getProxyHttpUrl, parseProxyUrl, isResidentialProxy } = await import('./settings.js');
    ...
    try {
      ...
      if (json?.error) {
        ...
        if (isInvalidGrant(json)) {
          ...
          // Same RT is dead for both clients — don't burn a second redeem.
          return lastError;
        }
...
export async function tryBrowserlessTokenRefresh(savedState, refreshToken, log) {
  ...
  // When RT is still good (e.g. before expiry), this succeeds in ~1–2s.
  const loki = await tryLokiScopeRedeem(refreshToken, log);
  if (loki?.access_token && isTokenValid(loki)) return loki;

  // Dead RT — MSAL LiveProfile exchange will also fail. Caller should use cookie SSO.
  if (isInvalidGrant(loki)) {
    return loki;
  }

  // Has SSO cookies → skip slow Outlook/Teams MSAL RT posts (often 8–18s each, rarely help
  // after Loki already failed). Cookie PKCE is the recovery path for this fleet.
  if (savedState?.cookies?.length >= 3) {
    if (loki?.error) {
      log?.('token', 'Loki redeem missed — skipping MSAL RT, trying cookie SSO next…');
    }
    return loki?.error ? loki : { error: 'loki_miss', error_description: 'Loki redeem failed' };
  }

  // No cookies — last RT hope via MSAL LiveProfile exchange.
...
async function gotoWithProxyRetry(page, url, log, { waitUntil = NAV_WAIT, timeout = navTimeoutMs() } = {}) {
  ...
  if (page.isClosed?.()) {
    log?.('proxy', 'Navigation skipped — browser already closed');
    return false;
  }
  try {
    ...
    // NS_BINDING_ABORTED = navigation superseded; retrying often makes it worse.
    if (/NS_BINDING_ABORTED|Target page, context or browser has been closed/i.test(msg)) {
      log?.('proxy', `Navigation aborted (${msg.split('\n')[0]}) — not retrying`);
      return false;
    }
    if (!isProxyEnabled()) throw err;
    if (page.isClosed?.()) return false;
    log?.('proxy', `Navigation failed (${msg.split('\n')[0]}) — retrying once`);
...
async function waitForOutlookMailReady(page, log, dismissPrompts, timeoutMs) {
  ...
  while (Date.now() < deadline) {
    await runDismissPrompts(page, dismissPrompts);
    // Interrupt / skip-7-days can block the Outlook shell forever if not clicked.
    if (isInterruptUrl(page.url()) || (await isBackupEmailScreen(page))) {
      const skipped = await clickSkipBackupEmail(page);
      if (skipped) log?.('prompt', `Skipped during Outlook wait: "${skipped}"`);
      await runDismissPrompts(page, dismissPrompts);
    }

    const url = page.url();
    if (/login\.(live|microsoftonline)\.com/i.test(url) && !(await isBackupEmailScreen(page))) {
      return false;
    }

    if (/outlook\.live\.com\/mail/i.test(url)) {
      // Require real mail chrome or MSAL cache — bare [role=main] matches empty shells
      // that never fire oauth2/token (token posts=0 in production).
      const ready = await page
        .evaluate((clientId) => {
          const want = String(clientId || '').toLowerCase();
          try {
            for (let i = 0; i < localStorage.length; i++) {
              const key = localStorage.key(i);
              if (key && key.toLowerCase().includes(want) && /accesstoken|refreshtoken/i.test(key)) {
                return true;
              }
            }
          } catch {
          ...
          const body = document.body?.innerText || '';
          if (/new mail|new message|inbox|focused|other/i.test(body)) return true;
          if (document.querySelector('[data-app-section="MailModule"]')) return true;
          const buttons = document.querySelectorAll('button[aria-label]');
          for (const b of buttons) {
            const a = (b.getAttribute('aria-label') || '').toLowerCase();
            if (a.includes('new mail') || a.includes('new message')) return true;
          }
          return false;
        }, LIVEPROFILE.clientId)
        .catch(() => false);
...
/**
 * When MSAL stays idle (token posts=0), soft focus events are not enough.
 * Force a mail-module route + UI actions that usually mint oauth2/token + RT.
 */
async function provokeOutlookMsal(page, log, dismissPrompts) {
  if (page.isClosed?.()) return;
  const navTimeout = Math.min(navTimeoutMs(), 40_000);

  log?.('token', 'Provoking Outlook MSAL (inbox route + UI click)…');
  await page
    .goto('https://outlook.live.com/mail/0/', { waitUntil: NAV_WAIT, timeout: navTimeout })
    .catch(() => {});
  await runDismissPrompts(page, dismissPrompts);
  await pokeOutlookMsal(page);
  ...
  const clicked = await page
    .evaluate(() => {
      const labelOf = (el) =>
        `${el.getAttribute?.('aria-label') || ''} ${el.innerText || ''} ${el.textContent || ''}`.trim();
      const nodes = [
        ...document.querySelectorAll('button, a, div[role="button"], span[role="button"]'),
      ];
      for (const el of nodes) {
        const t = labelOf(el);
        if (/new mail|new message|^compose$/i.test(t) && t.length < 40) {
          el.click();
          return 'compose';
        }
      }
      for (const el of nodes) {
        const t = labelOf(el);
        if (/^inbox$/i.test(t) || /\binbox\b/i.test(el.getAttribute?.('aria-label') || '')) {
          el.click();
          return 'inbox';
        }
      }
      // Folder tree / left nav fallback
      const inboxNav = document.querySelector(
        '[title="Inbox"], [aria-label="Inbox"], a[href*="/inbox"]'
      );
      if (inboxNav) {
        inboxNav.click();
        return 'inbox-nav';
  ...
  if (clicked) log?.('token', `Outlook UI poke: ${clicked}`);
  await pokeOutlookMsal(page);
  await page.waitForTimeout(2_000).catch(() => {});

  // Escape compose pane if we opened it — keep session on mail module.
  if (clicked === 'compose') {
...
export async function tryContextRequestPkceAuthCode(context, log) {
  ...
  try {
    if (context.browser?.()?.isConnected?.() === false) {
      log?.('token', 'Context cookie SSO skipped — browser already closed');
      return null;
    }
  } catch {
    return null;
...
export async function tryInBrowserPkceAuthCode(page, context, log) {
  ...
  if (!isBrowserAlive(page, context)) {
    log?.('token', 'In-browser cookie SSO skipped — browser already closed');
    return null;
  }
  const profiles = [TEAMS_LIVEPROFILE, LIVEPROFILE];
  ...
  for (const profile of profiles) {
    if (!isBrowserAlive(page, context)) return null;
    const { verifier, challenge } = makePkce();
...
export async function tryCookiePkceAuthCode(savedState, log) {
  ...
  return withCookieSsoSlot(async () => {
    // Cookie SSO hits login.live.com — must use mobile (residential SOCKS blocks *.live.com).
    let proxy;
    try {
      proxy = await playwrightProxyOpts({ forCookie: true });
    } catch {
      proxy = undefined;
    }
    const profiles = [TEAMS_LIVEPROFILE, LIVEPROFILE];
    ...
    for (const profile of profiles) {
      ...
      try {
        ...
        if (isCookieSsoNetworkError(err)) congested = true;
      } finally {
        await ctx.dispose().catch(() => {});
      }
    }

    if (congested) return { cookieSsoCongested: true };
    return null;
  });
}

function isCookieSsoNetworkError(err) {
  return /timed out|timeout|socket hang up|ECONNRESET|ECONNABORTED|EPIPE|ENOTFOUND|ECONNREFUSED|aborted|net::ERR/i.test(
 ...
 * During Camoufox login: Loki keep-alive stays on; cookie SSO (login.live.com) is blocked.
 * During GetCredentialType shield: all browserless HTTP is blocked briefly.
 * Returns { tokens, refreshTokenKnownBad }.
 */
export async function tryAllBrowserlessRefresh(
  savedState,
  log,
  { forceRefresh = false, lokiOnly = false } = {}
) {
  if (isBrowserlessHttpBlocked()) {
    return { tokens: null, refreshTokenKnownBad: false, fromCache: false, skippedHttp: true };
  }
  ...
  const blockCookie = lokiOnly || isLoginCookieSsoBlocked();
  const hasCookies = Array.isArray(savedState?.cookies) && savedState.cookies.length >= 3;

  // Cookie PKCE is the path that works at 17k+. Loki Teams+Outlook misses first and
  // holds Fast-lane slots for seconds — due pile grows even while SSO succeeds.
  if (hasCookies && !blockCookie) {
    ...
    if (cookieTokens?.cookieSsoCongested) {
      return {
        tokens: null,
        refreshTokenKnownBad: sawInvalidGrant,
        fromCache: false,
        skippedHttp: false,
        cookieSsoCongested: true,
      };
    }
  }

  // Dead stored RT: skip Loki/MSAL RT redeem, go straight to cookie SSO.
  if (!rtRejected) {
    const candidates = [];
    ...
    for (const rt of candidates) {
      ...
      if (isInvalidGrant(result)) {
        ...
        // Further RTs / MSAL won't help — cookie SSO (or Camoufox) is next.
        break;
      }
      // Loki miss with cookies: don't burn more RTs on the same slow path.
      if (hasCookies) break;
    }
  } else if (!blockCookie) {
  ...
  if (blockCookie) {
    if (lokiOnly || isLoginCookieSsoBlocked()) {
      log?.('token', 'Cookie SSO deferred — mobile IP rotating or Loki-only lane');
    }
    return {
      tokens: null,
      refreshTokenKnownBad: sawInvalidGrant,
  ...
  if (isBrowserlessHttpBlocked()) {
    return { tokens: null, refreshTokenKnownBad: sawInvalidGrant, fromCache: false, skippedHttp: true };
  }

  // Already tried cookie-first above when cookies exist.
  if (hasCookies && !blockCookie) {
    return {
      tokens: null,
      refreshTokenKnownBad: sawInvalidGrant,
  ...
  const cookieTokens = await tryCookiePkceAuthCode(savedState, log);
  if (cookieTokens?.access_token && isTokenValid(cookieTokens)) {
    // Cookie often mints a fresh RT — next cycle should try Loki again (HTTP keep-alive).
    // Only mark RT dead on real invalid_grant (sawInvalidGrant), not on timeout/miss.
    return {
      tokens: cookieTokens,
      refreshTokenKnownBad: sawInvalidGrant,
      fromCache: false,
  ...
  return {
    ...
    fromCache: false,
    skippedHttp: false,
    cookieSsoCongested: !!cookieTokens?.cookieSsoCongested,
  };
}
