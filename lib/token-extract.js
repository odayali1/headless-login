...
export async function captureOutlookTokens(page, { log, context, engine, existingTokens, forceRenew = false, dismissPrompts, skipPreflightRefresh = false, refreshTokenKnownBad = false } = {}) {
  const CAPTURE_TIMEOUT_MS = refreshTokenKnownBad ? 90_000 : isProxyEnabled() ? 150_000 : 150_000;
  ...
  const work = async () => {
    ...
    let tokens = await triggerOutlookTokenCapture(page, listener, log, dismissPrompts, {
      // Don't burn the whole 150s budget on a stuck Outlook inbox.
      readyTimeoutMs: freshLogin ? 25_000 : undefined,
      listenMs: freshLogin ? 28_000 : undefined,
    });

    // Any MSA refresh_token overheard on the wire → Loki LiveProfileCard (TokenMan/FOCI-style).
    if (!acceptCapturedToken(tokens)) {
      const networkRt = listener.getRefreshToken?.();
      if (networkRt) {
        log?.('token', 'Redeeming network refresh_token (Loki scope)…');
        const redeemed = await tryLokiScopeRedeem(networkRt, log, {
          clientId: listener.getClientId?.() || undefined,
        });
        if (acceptCapturedToken(redeemed)) {
          tokens = { ...redeemed, refresh_token: redeemed.refresh_token || networkRt };
        }
      }
    }

    // roadtx-style: ESTSAUTH cookies → PKCE auth-code (works after session cookies settle).
    if (!acceptCapturedToken(tokens) && context) {
      const cookies = await context.cookies().catch(() => []);
      if (cookies.length >= 5) {
        log?.('token', 'Trying cookie SSO PKCE (Teams/Outlook client, no Camoufox nav)…');
    ...
    if (!acceptCapturedToken(tokens)) {
      log?.('token', 'Outlook incomplete — quick Teams capture for refresh_token…');
...
async function captureViaSavedSession(
  ...
  if (!networkRt && !(await isOutlookLoginPage(page))) {
    ...
    if (context) {
      ...
      const cookieTokens = await tryCookiePkceAuthCode({ cookies: jar }, log);
      if (acceptCapturedToken(cookieTokens, { fresh: true })) {
        listener.stop?.();
        return cookieTokens;
      }
    }
...
async function triggerOutlookTokenCapture(
  ...
  let tokens = await listener.wait(firstListen);
  ...
  // Short second listen only — long waits here used to burn the outer 150s timeout
  // before cookie SSO / Teams fallbacks could run.
...
/** Short Teams hit after login — grab RT and Loki-redeem (avoid 90s Outlook hang). */
async function captureViaTeamsQuick(page, listener, log, dismissPrompts) {
  ...
  await page.waitForTimeout(3500);
  let tokens = await listener.wait(18_000);
  const rt = listener.getRefreshToken?.() || null;
  const clientId = listener.getClientId?.() || '';

  if (rt) {
    log?.('token', 'Redeeming Teams refresh_token (Loki scope)…');
  ...
  if (!acceptCapturedToken(tokens)) {
    ...
    tokens = listener.getCaptured?.() || (await listener.wait(12_000));
    const rt2 = listener.getRefreshToken?.() || rt;
    if (rt2 && !acceptCapturedToken(tokens)) {
      const redeemed = await tryLokiScopeRedeem(rt2, log, {
        clientId: listener.getClientId?.() || clientId || undefined,
      });
      if (acceptCapturedToken(redeemed)) {
        return { ...redeemed, refresh_token: redeemed.refresh_token || rt2 };
      }
    }
  }
