...
import { getProxyUrl, isLoginAwsApi, parseProxyUrl } from './settings.js';
...
async function curlThroughProxyArg(proxyArg, url, timeoutSec = 15) {
  const args = ['-sS', '-x', proxyArg, '-m', String(timeoutSec), url];
  // Local FireProx MITM uses a self-signed cert.
  if (/127\.0\.0\.1|localhost/i.test(String(proxyArg))) args.splice(1, 0, '-k');
  const { stdout } = await execFileAsync(curlBin(), args, { timeout: (timeoutSec + 5) * 1000 });
...
export async function probeExitIp(opts = {}) {
  ...
  let urls = IP_URLS;
  try {
    if (opts.playwrightProxy) {
      proxyArg = playwrightProxyArg(opts.playwrightProxy);
    } else if (isLoginAwsApi()) {
      const { getFireproxPlaywrightProxy } = await import('./fireprox-local-proxy.js');
      const cfg = await getFireproxPlaywrightProxy();
      proxyArg = cfg.server;
      // Only origins that already have a FireProx API (no AWS keys on Coolify).
      urls = ['https://api.ipify.org'];
    } else {
      parseProxyUrl(getProxyUrl());
      proxyArg = upstreamProxyArg();
    }
  } catch {
    return null;
  }
